...
request = HTTPXRequest(
    ...
    read_timeout=30.0,
    write_timeout=30.0,
    pool_limits=httpx.Limits(max_connections=40, max_keepalive_connections=20),
)
...
async def start_with_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # احترام وضع الصيانة
    if global_settings.get("maintenance_mode", False):
        await send_message_safe(context.bot, chat_id=update.effective_chat.id, "🛠️ نظام النشر تحت الصيانة والتحديث حاليًا.")
        return
    ...
    if not args:
        ...
        await send_message_safe(context.bot, chat_id=update.effective_chat.id, text=msg, parse_mode=ParseMode.MARKDOWN)
        return

    token = args[0]
    rec = start_tokens.get(token)
    if not rec:
        await send_message_safe(context.bot, chat_id=update.effective_chat.id, "⛔ رابط غير صالح أو منتهي.")
        return

    if user.id != rec["user_id"]:
        await send_message_safe(context.bot, chat_id=update.effective_chat.id, "⛔ هذا الرابط ليس لك. اطلب من المشرفين منحك تصريح نشر مؤقت.")
        return

    if datetime.utcnow() > rec["expires"]:
        await send_message_safe(context.bot, chat_id=update.effective_chat.id, "⌛ انتهت صلاحية الرابط.")
    ...
    await send_message_safe(context.bot, chat_id=update.effective_chat.id, text=msg_ok, parse_mode=ParseMode.MARKDOWN)

    start_tokens.pop(token, None)
    save_state()
# =============================
# Input collection
# =============================sess: Session, saved_type: str) -> str:
