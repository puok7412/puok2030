...
def save_state():
    global STATE_PATH
    try:
        # نبني البيانات: نستخدم snapshot إن كانت موجودة، وإلا نرجع لطريقتك القديمة
        try:
            data = build_persist_snapshot()  # إن كانت معرّفة في الملف
        except NameError:
            # === طريقتك القديمة (fallback) ===
            data = {}
    ...
    # ===== نسخة احتياطية خفيفة إلى تيليجرام (اختياري عبر متغير البيئة) =====
    try:
        chat_id_val = TG_BACKUP_CHAT_ID or int((os.getenv("TG_BACKUP_CHAT_ID") or "0").strip() or "0")
        if not chat_id_val:
            return  # لا توجد قناة نسخ احتياطي

        # حمّل نفس الـ data الذي كتبناه للتو
        ...
        async def _tg_backup():
            try:
                ...
                # تثبيت آخر نسخة
                try:
                    await application.bot.unpin_all_chat_messages(chat_id=chat_id_val)
                except Exception:
                    pass
                try:
