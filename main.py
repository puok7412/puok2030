...
def save_state():
    ...
    # ============== بناء لقطة الحالة وكتابتها للقرص ==============
    try:
        # جرّب اللقطة الموحّدة، وإلا ارجع للطريقة القديمة
        try:
            data = build_persist_snapshot()  # إن كانت معرّفة
        except NameError:
            # === fallback القديم ===
        ...
        return  # لا نكمّل في حال فشل الحفظ

    # ============== لا إرسال تلقائي لتليجرام ==============
    # يتم النسخ الاحتياطي يدويًا من زر لوحة التحكم فقط.
    if not BACKUP_ON_SAVE:
        return

    # (اختياري) إن فعّلت BACKUP_ON_SAVE=1 لاحقًا:
    try:
        chat_id_val = TG_BACKUP_CHAT_ID or int((os.getenv("TG_BACKUP_CHAT_ID") or "0").strip() or "0")
        if not chat_id_val:
            return

        payload_bytes = json.dumps(data, ensure_ascii=False).encode("utf-8")

        async def _tg_backup():
            ...
            try:
                ...
                # نثبّت بهدوء بدون unpin_all لتقليل احتمال Flood
                try:
                ...
                # إعادة محاولة واحدة فقط عند Rate Limit
                ...
                logger.exception("save_state: telegram backup failed")

        asyncio.create_task(_tg_backup())

    except Exception:
        logger.exception("save_state: scheduling telegram backup failed")

def _pack_tuple_keys(d: Dict[tuple, Any]) -> Dict[str, Any]:
...
def build_persist_snapshot() -> Dict[str, Any]:
    # تركيز على ما يهم نتائج الحملات والتفاعلات والوجهات
    return {
        "known_chats": known_chats,  # cid -> {"title":..., "type":...}

        # رسائل الحملات وربطها
        "campaign_messages": campaign_messages,                           # {camp_id: [(cid, mid), ...]}
        "campaign_base_msg": _pack_tuple_keys(campaign_base_msg),         # {(camp_id,cid)->base_mid} مغلّفة
        "message_to_campaign": _pack_tuple_keys(message_to_campaign),     # {(cid,mid)->camp_id} مغلّفة

        # العدّادات والستايلات
        "campaign_counters": campaign_counters,                           # {camp_id: {like, dislike, voters}}
        "reactions_counters": _pack_tuple_keys(reactions_counters),       # {(cid,base_mid)-> {...}} مغلّفة
        "campaign_styles": campaign_styles if "campaign_styles" in globals() else {},
        "reaction_style_by_message": _pack_tuple_keys(reaction_style_by_message) if "reaction_style_by_message" in globals() else {},

        # إعدادات النشر ذات الصلة
        "global_settings": {
            "reactions_feature_enabled": global_settings.get("reactions_feature_enabled", True),
            "pin_feature_enabled": global_settings.get("pin_feature_enabled", True),
            "scheduling_enabled": global_settings.get("scheduling_enabled", True),
            "schedule_locked": global_settings.get("schedule_locked", False),
            "rebroadcast_interval_seconds": global_settings.get("rebroadcast_interval_seconds", 3600),
            "rebroadcast_total": global_settings.get("rebroadcast_total", 0),
            "reaction_prompt_text": global_settings.get("reaction_prompt_text", "قيّم هذا المنشور 👇"),
        },
    }
...
async def backup_state_to_tg(bot) -> None:
    ...
    try:
        ...
        now = 0.0
    if now and (now - _LAST_BACKUP_TS) < 3.0:
        return
    _LAST_BACKUP_TS = now
    ...
    try:
        ...
        # ثبّت النسخة الأخيرة لتسهيل الاسترجاع
        try:
            await bot.unpin_all_chat_messages(chat_id=TG_BACKUP_CHAT_ID)
        except Exception:
            pass
        try:
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel:backup":
        try:
        # احفظ الحالة (سيُطلق النسخ الاحتياطي تلقائيًا كما في save_state)
            save_state()
        # محاولة فورية لإرسال نسخة احتياطية أيضًا لتنبيه المستخدم (إن كان المعرّف موجود)
            chat_id_val = TG_BACKUP_CHAT_ID or int((os.getenv("TG_BACKUP_CHAT_ID") or "0").strip() or "0")
            if chat_id_val:
                try:
                    payload = json.dumps(build_persist_snapshot() if 'build_persist_snapshot' in globals() else {}, ensure_ascii=False).encode("utf-8")
                    bio = io.BytesIO(payload); bio.name = "publisher_state.json"
                    msg = await context.bot.send_document(
                        chat_id=chat_id_val,
                        document=bio,
                        caption=f"publisher_state {datetime.utcnow().isoformat(timespec='seconds')}Z (manual)"
                    )
                    try:
                        await context.bot.unpin_all_chat_messages(chat_id=chat_id_val)
                    except Exception:
                        pass
                    try:
                        await context.bot.pin_chat_message(chat_id=chat_id_val, message_id=msg.message_id, disable_notification=True)
                    except Exception:
                        pass
                    await query.message.reply_text("✅ تم إنشاء نسخة احتياطية وإرسالها إلى قناة الأرشيف.")
                except Exception:
                    await query.message.reply_text("⚠️ تم حفظ الحالة محليًا، لكن فشل إرسال النسخة الاحتياطية إلى تيليجرام.")
            else:
                await query.message.reply_text("💾 تم حفظ الحالة محليًا.\nℹ️ لضبط النسخ إلى تيليجرام: ضع TG_BACKUP_CHAT_ID في Environment بقيمة chat_id لقناة الأرشيف.")
        except Exception:
            await query.message.reply_text("❌ تعذر إنشاء النسخة الاحتياطية الآن.")
        return
...
async def on_startup():
    ...
    try:
        chat_id_str = (os.getenv("TG_BACKUP_CHAT_ID") or "").strip()
        if chat_id_str:
            chat_id = int(chat_id_str)
            chat = await application.bot.get_chat(chat_id)
            pinned = getattr(chat, "pinned_message", None)
            if pinned and getattr(pinned, "document", None):
                # نحاول فقط إن كان اسم الملف JSON
                fname = pinned.document.file_name or ""
                if fname.endswith(".json"):
                    fobj = await application.bot.get_file(pinned.document.file_id)
                    # PTB v20: download_as_bytearray متاحة
                    data_bytes = await fobj.download_as_bytearray()
                    try:
                        json_data = json.loads(data_bytes.decode("utf-8"))
                        # فكّ التحويل وإسقاطها في المتغيرات العالمية
                        for bucket in _PERSIST_BUCKETS:
                            val = json_data.get(bucket, {})
                            if bucket in _TUPLEKEY_BUCKETS and isinstance(val, dict):
                                conv = {}
                                for kstr, v in val.items():
                                    # رجّع مفاتيح tuple من "a::b::c"
                                    parts = kstr.split("::")
                                    # حوّل للأعداد حيثما أمكن
                                    key = tuple(
                                ...
                                globals()[bucket] = val
                        # اكتب نسخة محلية فورًا
                        try:
                            with open(STATE_PATH, "w", encoding="utf-8") as f:
                                json.dump(json_data, f, ensure_ascii=False)
                        except Exception:
                            pass
                        logger.info("State restored from Telegram pinned backup.")
                    except Exception:
                        logger.exception("Failed to parse Telegram backup JSON")
    except Exception:
