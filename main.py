...
def status_text(sess: "Session") -> str:
    ...
    # التفاعلات (فقط لو الخدمة مفعّلة مركزيًا)
    if gs.get("reactions_feature_enabled", True):
        parts.append("✅ تفاعلات" if getattr(sess, "use_reactions", False) else "🚫 تفاعلات")

    # التثبيت (فقط لو الخدمة مفعّلة مركزيًا) — الافتراضي معطّل
    if gs.get("pin_feature_enabled", True):
        parts.append("📌 مفعّل تثبيت المنشور" if getattr(sess, "pin_enabled", False) else "📌 معطّل تثبيت المنشور")

    # الوجهات
    parts.append(f"🎯 وجهات محددة: {len(getattr(sess, 'chosen_chats', []))}")

    # الجدولة (اظهر تفاصيلها فقط إن كانت مفعّلة في الجلسة، وإلا صفر)
    if gs.get("scheduling_enabled", True) and getattr(sess, "schedule_active", False):
        interval_min = max(1, int(getattr(sess, "rebroadcast_interval_seconds", 0))) // 60
        total = int(getattr(sess, "rebroadcast_total", 0))
        parts.append(f"⏱️ الإعادة: كل {interval_min} دقيقة × {total}")
    else:
...
def keyboard_ready_options(sess: Session, settings: Dict[str, Any]) -> InlineKeyboardMarkup:
    ...
    btns.append([
        InlineKeyboardButton(
            "📌 تفعيل تثبيت المنشور" if not getattr(sess, "pin_enabled", False) else "📌 تعطيل تثبيت المنشور",
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith(("like:", "dislike:", "panel:", "perm:")):
        return
    ...
    if data == "toggle_pin":
        if not global_settings.get("pin_feature_enabled", True):
            ...
            return
        sess.pin_enabled = not bool(getattr(sess, "pin_enabled", False))
        await push_panel(context, user_id, sess, "✅ تم ضبط خيار تثبيت المنشور.")
        return
...
def build_status_block(sess, *, allow_schedule: bool | None = None) -> str:
    ...
    pin_line   = f"تثبيت المنشور: {'📌 مفعّل' if bool(getattr(sess, 'pin_enabled', True)) else '📌 معطّل'}"
...
def panel_settings_keyboard() -> InlineKeyboardMarkup:
    gs = global_settings
    rows = [
        [InlineKeyboardButton(f"📌 تثبيت المنشور: {'مفعل' if gs.get('pin_feature_enabled', True) else 'معطل'}",
...
def keyboard_ready_options(sess: 'Session', settings: dict) -> InlineKeyboardMarkup:
    ...
    # زر التثبيت
    if gs.get("pin_feature_enabled", True):
        btns.append([InlineKeyboardButton("📌 تفعيل تثبيت المنشور" if not getattr(sess, "pin_enabled", False) else "📌 تعطيل تثبيت المنشور",
                                          callback_data="toggle_pin")])
        ...
        for cid in target_chats:
            try:
                ...
                if mid:
                    ...
                    if gs.get("pin_feature_enabled", True) and getattr(sess, "pin_enabled", False) and not is_rebroadcast:
                        try:
...
async def answer_unknown_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # يوقف دوران "تحميل" حتى لو ما كان فيه هاندلر مطابق
...
# === تأكيد وجود التطبيق + تسجيل خطّاف الأخطاء (مرة واحدة) ===
try:
    application  # هل متغير التطبيق موجود أصلاً؟
except NameError:
    # إذا (لا) — أنشئه الآن
...
# مفاتيح سريعة في الخاص (رتّبناها عشان ok25s ما تنبلع)
...
# إدخال المحتوى في الخاص (نص/وسائط/ملفات) — يستثني ok/ok25s
application.add_handler(MessageHandler(
    filters.ChatType.PRIVATE & ~filters.COMMAND & ~filters.Regex(r"(?i)^\s*(ok|ok25s)\s*$"),
    handle_admin_input
), group=2)

# نصوص لوحة التحكم في الخاص (أسماء قوالب/نص دعوة التفاعل.. إلخ)
application.add_handler(MessageHandler(
...
# 4) أزرار واجهة جلسة النشر (catch-all) — مع استثناء (like/dislike/panel/perm)
application.add_handler(
    CallbackQueryHandler(on_button, pattern=r"^(?!like:|dislike:|panel:|perm:).+"),
    group=99
)

# 5) فول-باك لأي شيء سقط من الفلاتر (يوقف "تحميل")
application.add_handler(
