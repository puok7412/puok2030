...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    مزامنة أزرار التفاعل لجميع رسائل الحملة وفق الوضع المختار:
      ...
      - REACTIONS_ATTACH_MODE = "base_only"  : الأزرار على رسائل الأساس فقط
      - REACTIONS_ATTACH_MODE = "both"       : الأزرار على الاثنين
    """
    ...
    # وضع الإرفاق + حالة نص الدعوة من لوحة التحكم
    mode = globals().get("REACTIONS_ATTACH_MODE", "prompt_only")
    prompt_enabled = global_settings.get("reactions_prompt_enabled", True)

    # افتراضات
    attach_to_prompt = False
    attach_to_base = False

    if prompt_enabled:
        # السلوك العادي كما كان سابقًا
        attach_to_prompt = (mode in ("prompt_only", "both"))
        attach_to_base   = (mode in ("base_only",   "both"))
    else:
        # عند إخفاء نص الدعوة: نتعامل فعليًا كـ base_only
        attach_to_prompt = False
        attach_to_base   = True
    ...
    def _find_base_mid_for_chat(cid: int):
        try:
    ...
    def _kb(cid: int, mid_for_buttons: int, base_mid: int | None):
    ...
    # -------- وضع prompt_only مع ظهور نص الدعوة (السلوك القديم) --------
    if mode == "prompt_only" and prompt_enabled:
        # نظّف الأزرار من منشور الأساس (لمنع التكرار)
    ...
    # أولاً: حدّث رسائل الدعوة (لو مفعلة وتريد ذلك)
    ...
    # ثانياً: عند إخفاء نص الدعوة أو تفعيل base_only/both → حدّث منشورات الأساس نفسها
    if attach_to_base and base_map:
        for key, base_mid in list(base_map.items()):
            camp_k, cid = _parse_key_to_pair(key)
            if camp_k != campaign_id or not isinstance(base_mid, int):
                continue
