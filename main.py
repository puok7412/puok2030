...
def build_persist_snapshot() -> Dict[str, Any]:
    ...
    # ✅ تطبيع المفاتيح إلى نصوص حيثما كانت بدائية (int/str) لتفادي مشاكل JSON مستقبلًا
        ...
        if isinstance(d, dict):
            for k, v in d.items():
                try:
                    ...
                    # تجاهل أي مفتاح غير قابل للتحويل
                    continue
        return out

    # known_chats: نحول مفاتيح الدليل إلى أرقام
    known_chats.clear()
    known_chats.update(_int_keys(data.get("known_chats", {})))

    # campaign_messages: مفاتيح حملات كأرقام
    campaign_messages.clear()
    campaign_messages.update(_int_keys(data.get("campaign_messages", {})))

    # tuples مفكوكة إلى أرقام أصلًا عبر _unpack_tuple_keys
