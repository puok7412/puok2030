...
async def send_preview(update: Update, context: ContextTypes.DEFAULT_TYPE, sess: Session, *, hide_links: bool):
    ...
    # نفرض الإخفاء دائمًا في المعاينة
    hide_links = True

    # يحوّل أي نص إلى HTML مع روابط مخفية إن وُجدت، ويُعيد دائمًا ParseMode.HTML
    def _auto_htmlize_and_mode(txt: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
        if not txt:
            return txt, None
        return hidden_links_or_plain(txt, True), ParseMode.HTML
