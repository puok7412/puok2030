...
OK25S_REGEX= re.compile(r'(?i)^\s*ok25s\s*$')     # فتح لوحة التحكّم المركزية
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # أجب فوراً لتجنب "Query is too old"
    try:
        await query.answer()
    except BadRequest as e:
        msg = str(e).lower()
    ...
    if data == "panel:destinations":
        ...
        if not admin_chat_ids:
            ...
            return
        await query.message.reply_text("🗂️ الوجهات:", reply_markup=build_panel_chats_keyboard(admin_chat_ids, s))
        return
    ...
    if data.startswith("panel:toggle_chat:"):
        ...
        admin_chat_ids = await list_authorized_chats(context, user_id)
        try:
            await query.message.edit_reply_markup(reply_markup=build_panel_chats_keyboard(admin_chat_ids, s))
        except Exception:
            pass
        save_state()
        return
    ...
    if data == "panel:dest_save":
        # في نسختك القديمة كان الحفظ فوريًا ضمن التبديل؛ نكتفي بإشعار
    ...
    if data == "panel:schedule":
        if not sess:
            return
        await query.message.reply_text("⏱️ إعدادات الإعادة:", reply_markup=session_schedule_keyboard(sess))
        return

    if data.startswith("ssched_count:"):
        if sess:
            sess.rebroadcast_total = int(data.split(":")[1])
            save_state()
            try:
                await query.message.edit_reply_markup(reply_markup=session_schedule_keyboard(sess))
            except Exception:
                pass
        return
    ...
    if data.startswith("ssched_int:"):
        if sess:
            sess.rebroadcast_interval_seconds = int(data.split(":")[1])
            save_state()
            try:
                await query.message.edit_reply_markup(reply_markup=session_schedule_keyboard(sess))
            except Exception:
    ...
    if data == "sschedule_done":
        await query.message.reply_text("✅ تم حفظ إعدادات الإعادة.")
        save_state()
        return

    if data == "back_main":
        try:
            await query.message.reply_text("🛠️ *لوحة التحكّم الرئيسية*", parse_mode="Markdown", reply_markup=panel_main_keyboard())
        except Exception:
            pass
        return

    # ====== التفاعلات/القوالب/الأذونات/السجل/الإعدادات ======
    if data == "panel:reactions":
        await query.message.reply_text("🧩 التفاعلات: يمكن ضبطها من داخل جلسة المنشور أو مركزيًا لاحقًا.")
        return

    if data == "panel:templates":
        await query.message.reply_text("🧰 إدارة القوالب: (كما بالنسخة القديمة؛ موضع التخزين يعتمد على ملفك).")
        return

    if data == "panel:permissions":
        await query.message.reply_text("🔒 إدارة الأذونات:", reply_markup=permissions_root_keyboard())
        return

    if data.startswith("perm:chat:"):
        chat_id = int(data.split(":")[2])
        await refresh_admins_for_chat(context, chat_id)
        await query.message.reply_text("اختر المشرفين المسموح لهم بالنشر:", reply_markup=permissions_admins_keyboard(chat_id))
        return

    if data.startswith("perm:toggle:"):
        _, _, cid_str, uid_str = data.split(":")
        ...
        if uid in blocked:
            ...
            blocked.add(uid)
        try:
            await query.message.edit_reply_markup(reply_markup=permissions_admins_keyboard(cid))
        except Exception:
            pass
        save_state()
        return
    ...
    if data.startswith("perm:save:"):
        await query.message.reply_text("✅ تم حفظ أذونات المجموعة.")
        save_state()
        return

    if data == "panel:logs":
        await query.message.reply_text("🧾 السجلات: سيتم عرض آخر الأحداث/الأخطاء هنا (حسب إعدادات اللوج).")
        return

    if data == "panel:settings":
        await query.message.reply_text("⚙️ الإعدادات العامة: (بنفس أسلوب نسختك القديمة).")
        return
