...
# =============================
OK_REGEX = re.compile(r'(?i)^\s*(ok|ok25s)\s*$')
...
def save_state():
    ...
    try:
        ...
        # حاول إنشاء المجلد لمسار STATE_PATH الحالي
...
def build_session_schedule_keyboard(sess: "Session") -> InlineKeyboardMarkup:
    ...
    # 🔁 عدد مرات الإعادة (اتركه كما كان لديك؛ مثال شائع أدناه)
    rows.append([
    ...
    # زر تفعيل/إلغاء الجدولة (حسب منطقك الحالي)
    rows.append([
        InlineKeyboardButton("✅ تفعيل الإعادة" if not getattr(sess, "schedule_active", False) else "⛔️ إلغاء الإعادة",
                             callback_data="ssched_toggle")
    ])

    # شريط حالة صغير يُظهر الإعداد الحالي للمراجعة البصرية
    status = "—"
    try:
        parts = []
        ...
        if getattr(sess, "schedule_active", False):
            parts.append("الحالة: مفعّل")
        else:
            parts.append("الحالة: غير مفعّل")
        status = " | ".join(parts) if parts else status
    except Exception:
        pass

    # نعرض الحالة كسطر منفصل عبر زر غير تفاعلي (dummy)
...
async def handle_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.text:
        ...
        if raw == "ok":
            # اترك / ok للـ handler المخصص لبدء الجلسة (start_publishing_keyword)
            return
        if raw == "ok25s":
            # إن لم تكن هناك جلسة، ابدأ واحدة ثم افتح لوحة التحكم
            if not sess:
                await start_publishing_session(update.effective_user, context)
                sess = sessions.get(user_id)
            if not sess:
                return  # حماية إضافية
...
def register_handlers():
    ...
    # 👇👇 أضِف هذا الهاندلر قبل الهاندلر العام للرسائل الخاصة
    ...
    # Direct private messages (content intake) — يجب أن يبقى أخيرًا
