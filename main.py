...
async def handle_reactions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if campaign_id is not None:
        ...
        # ✅ محاولة نسخ احتياطي عتبي إن وُجدت الدالة
        try:
        ...
        # ✅ تحديث عدّاد هذه الوجهة/الرسالة لعرض التفصيل per chat
        ...
        # ✅ حفظ فوري قبل أي مزامنة/تعديلات واجهة
        try:
            save_state()
        except Exception:
        ...
        # 1) حدّث الزر في نفس الرسالة التي نُقر عليها الآن
        # في وضع prompt_only + الدعوة مفعّلة:
        #   إذا كانت هذه الرسالة هي رسالة المنشور الأساسي (base_or_msg_id) نتجاهل تحديثها
        #   لأن الأزرار الأساسية تكون على رسالة الدعوة فقط.
        # أما إذا كانت الدعوة معطّلة → الأزرار على المنشور نفسه ويجب تحديثها.
        try:
        ...
        # 2) مزامنة باقي رسائل الحملة (في حالة وجود رسائل دعوة فعالة فقط)
        # إذا كانت رسالة الدعوة معطّلة، لا نستدعي _sync_campaign_keyboards
        # حتى لا يعبث بأزرار المنشور في الوضع الجديد.
        if prompt_enabled:
            try:
        ...
        try:
            ...
            m = await context.bot.send_message(
                ...
                reply_to_message_id=base_or_msg_id,  # ← اقتباس المنشور نفسه
                parse_mode="HTML",
            )
...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    def _find_base_mid_for_chat(cid: int) -> Optional[int]:
        try:
    ...
    def _kb(cid: int, mid_for_buttons: int, base_mid: Optional[int]) -> InlineKeyboardMarkup:
    ...
    # ================================
    # ➊ وضع "بدون نص دعوة" (المهم لنا)
    # ================================
    if not prompt_enabled:
        # في هذا الوضع، الأزرار موجودة مباشرة على منشورات الأساس في كل المجموعات
        # نحدّث كل منشور أساس في الحملة بنفس العدّادات المجمّعة
        for key, base_mid in list(base_map.items()):
            ...
            try:
                ...
                # قد تكون الرسالة محذوفة أو قديمة؛ نتجاهلها
                continue

        # في حالات fallback النادرة، قد تكون هناك رسائل دعوة "صامتة" (نص فارغ) أنشأها send_post_one_chat
        # نحترمها ونحدّث أزرارها بنفس العدّادات
        for (cid, mid) in list(prompt_list):
            try:
                base_mid = _find_base_mid_for_chat(cid)
                kb = _kb(cid, mid, base_mid)
                await context.bot.edit_message_reply_markup(
                    chat_id=cid,
                    message_id=mid,
                    reply_markup=kb,
        ...
        return  # 👈 لا نطبق منطق REACTIONS_ATTACH_MODE في هذا الوضع

    # ================================
    # ➋ وضع "مع نص دعوة" (السلوك القديم)
    # ================================

    # ---- mode: prompt_only ----
    if mode == "prompt_only":
        # نظّف الأزرار من منشورات الأساس
        for key, base_mid in list(base_map.items()):
            camp_k, cid = _parse_key_to_pair(key)
            if camp_k != campaign_id or not isinstance(base_mid, int):
                continue
        ...
        # حدّث الأزرار في رسائل الدعوة فقط
        for (cid, mid) in prompt_list:
            try:
                base_mid = _find_base_mid_for_chat(cid)
                kb = _kb(cid, mid, base_mid)
                await context.bot.edit_message_reply_markup(
        ...
        return  # 👈 لا نلمس منشور الأساس نهائيًا في هذا الوضع

    # ---- الأوضاع الأخرى (both / base_only) ----
    # ملاحظة في تعليقك الأصلي: هذه الأوضاع معطّلة عمليًا لتجنب التكرار، لكن نترك الكود سليمًا
    if attach_to_prompt:
        for (cid, mid) in prompt_list:
            try:
                base_mid = _find_base_mid_for_chat(cid)
...
async def send_post_one_chat(
    ...
    *,
    ...
    kb = InlineKeyboardMarkup(
        [[
            InlineKeyboardButton(
                f"{pos_emo} {like_count}",
                callback_data=f"like:{chat_id}:{base_id_for_buttons}",
            ),
            InlineKeyboardButton(
                f"{neg_emo} {dislike_count}",
                callback_data=f"dislike:{chat_id}:{base_id_for_buttons}",
            ),
        ]]
    )

    # ربط رسالة الأساس بالحملة بالتفاعلات (مهم لاحتساب الحملة عند تعطيل نص الدعوة)
    if sess.campaign_id is not None:
        try:
    ...
    if not prompt_enabled:
        # لا نرسل رسالة دعوة
        # أولاً نحاول تركيب الكيبورد على نفس المنشور الجديد
        try:
            await context.bot.edit_message_reply_markup(
                chat_id=chat_id,
            ...
            # إذا فشل التعديل على نفس المنشور (نوع رسالة لا يقبل كيبورد مثلًا)
            # نرسل رسالة جديدة تحته بأزرار فقط كحل احتياطي
            try:
                prompt_msg = await context.bot.send_message(
                    chat_id=chat_id,
                    text=" ",  # نص بسيط جدًا حتى لا يظهر كدعوة منفصلة مزعجة
                    reply_markup=kb,
                    disable_web_page_preview=True,
                )
                if sess.campaign_id is not None:
    ...
    # === أين نضع الأزرار؟ حسب الوضع المختار (السلوك القديم مع رسالة الدعوة)
    mode = globals().get("REACTIONS_ATTACH_MODE", "prompt_only")
    attach_to_prompt = (mode in ("prompt_only", "both"))
    attach_to_base   = (mode in ("base_only",   "both"))
