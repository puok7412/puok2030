...
def keyboard_ready_options(sess: Session, settings: Dict[str, Any]) -> InlineKeyboardMarkup:
    ...
    # 👍 التفاعلات (ظاهر فقط إذا كانت الميزة مفعّلة مركزيًا)
    if global_settings.get("reactions_feature_enabled", True) and settings.get("default_reactions_enabled", True):
        btns.append([
            InlineKeyboardButton(
    ...
    # 📌 التثبيت (ظاهر فقط إذا الميزة مفعّلة مركزيًا)
    if global_settings.get("pin_feature_enabled", True):
        btns.append([InlineKeyboardButton(
...
async def start_publishing_keyword(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if global_settings.get("maintenance_mode", False):
        await update.message.reply_text("🛠️ البوت قيد التحديث والصيانة. يرجى المحاولة لاحقًا.")
        return

    # قائمة بيضاء (اختيارية لحسابك)
    s = get_settings(user_id)
...
# =============================
async def handle_control_buttons
