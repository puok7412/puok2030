...
def build_text_and_entities(text: str):
    """
    يحوّل أي نص يحوي URLs إلى نص “اضغط هنا (n)” مع كيانات TEXT_LINK.
    لا يستخدم HTML ولا Markdown.
    """
    if not text:
        return text, None

    txt = sanitize_text(text)  # نفس دالتك الحالية
    entities = []
    parts = []
    last = 0
    idx = 1
    for m in URL_RE.finditer(txt):
        ...
        url = m.group(1)
        if start > last:
            parts.append(txt[last:start])
        label = "اضغط هنا" if idx == 1 else f"اضغط هنا ({idx})"
        label_offset = sum(len(p) for p in parts)
        parts.append(label)
        entities.append(
            MessageEntity(
    ...
    out = "".join(parts)
    return out, entities if entities else None



# ========== Link-safe send helpers (TEXT_LINK entities, no parse_mode) ==========
async def _send_text_with_links(bot, *, chat_id: int, text: str, reply_markup=None, disable_web_page_preview: bool = True):
    safe_text, ents = build_text_and_entities(text)
...
async def _send_captioned(bot, method: str, *, chat_id: int, file_id: str, caption: str | None, extra_kwargs: dict | None = None):
    extra_kwargs = dict(extra_kwargs or {})
    if caption:
        cap_text, cap_ents = build_text_and_entities(caption)
        extra_kwargs["caption"] = cap_text
        if cap_ents:
            extra_kwargs["caption_entities"] = cap_ents

    if method == "photo":
        return await bot.send_photo(chat_id=chat_id, photo=file_id, **extra_kwargs)
    if method == "video":
        return await bot.send_video(chat_id=chat_id, video=file_id, **extra_kwargs)
    if method == "document":
        return await bot.send_document(chat_id=chat_id, document=file_id, **extra_kwargs)
    if method == "audio":
        return await bot.send_audio(chat_id=chat_id, audio=file_id, **extra_kwargs)
    if method == "voice":
        return await bot.send_voice(chat_id=chat_id, voice=file_id, **extra_kwargs)
...
def build_persist_snapshot() -> Dict[str, Any]:
    ...
    return {
        ...
        "snapshot_ts": int(time.time()),  # ← إضافة مطلوبة
        ...
        "campaign_prompt_msgs": cpm_norm,  # ←新增
        ...
        "global_settings": {
            ...
            "reaction_prompt_text": _gs.get("reaction_prompt_text", "✍️ شَارِكْنَا رَأْيَكَ عَبْرَ التَّفَاعُل أَدْنَاهُ 👇"),
...
global_settings = {
    ...
    "reaction_prompt_text": "✍️ شَارِكْنَا رَأْيَكَ عَبْرَ التَّفَاعُل أَدْنَاهُ 👇",  # ✅ الجديد
}
...
async def start_publishing_keyword(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if admin_ids:
        await start_publishing_session(user, context)
        return
    await send_message_safe(context.bot, chat_id=update.effective_chat.id, text="🔒 هذا الأمر متاح فقط بتصريح مؤقّت فعّال من مشرف المجموعة.")
...
async def handle_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # مُعين داخلي: إن كان النص HTML جاهز وفيه روابط <a> نُبقيه كما هو، وإلا نعقّمه
    def _keep_html_if_links_else_sanitize(raw: Optional[str]) -> Optional[str]:
        if raw is None:
            return None
        raw = raw.strip()
        if not raw:
            return raw
        # لو فيه روابط مضمّنة من كيانات تيليجرام
        if '<a href="' in raw:
            return raw
        # خلاف ذلك نستخدم التعقيم المعتاد
    ...
    if msg.text is not None and not getattr(msg, "media_group_id", None):
        # استخدم HTML إن وُجد (يحمل <a href="..."> من entities)، وإلا النص العادي
        raw_text = getattr(msg, "html_text", None) or msg.text
        clean = _keep_html_if_links_else_sanitize(raw_text)
    ...
    if getattr(msg, "media_group_id", None) and (getattr(msg, "photo", None) or getattr(msg, "video", None)):
        if getattr(msg, "photo", None):
            cap_raw = getattr(msg, "caption_html", None) or msg.caption
            cap_clean = _keep_html_if_links_else_sanitize(cap_raw) if cap_raw else None
            sess.media_list.append(("photo", msg.photo[-1].file_id, cap_clean))
            saved_type = "صورة ضمن ألبوم"
        elif getattr(msg, "video", None):
            cap_raw = getattr(msg, "caption_html", None) or msg.caption
            cap_clean = _keep_html_if_links_else_sanitize(cap_raw) if cap_raw else None
            sess.media_list.append(("video", msg.video.file_id, cap_clean))
    ...
    if getattr(msg, "document", None):
        cap_raw = getattr(msg, "caption_html", None) or msg.caption
        cap_clean = _keep_html_if_links_else_sanitize(cap_raw) if cap_raw else None
        sess.single_attachment = ("document", msg.document.file_id, cap_clean)
        saved_type = "مستند"
    if getattr(msg, "audio", None):
        cap_raw = getattr(msg, "caption_html", None) or msg.caption
        cap_clean = _keep_html_if_links_else_sanitize(cap_raw) if cap_raw else None
        sess.single_attachment = ("audio", msg.audio.file_id, cap_clean)
        saved_type = "ملف صوتي"
    if getattr(msg, "voice", None):
        # رسائل الصوت لا تحمل كابتشن عادةً
...
async def handle_reactions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if campaign_id is not None:
        ...
        # ✅ سطران: محاولة نسخ احتياطي عتبي إن وُجدت الدالة
        try:
        ...
        # ✅ جديد: تحديث عدّاد هذه الوجهة/الرسالة لعرض التفصيل per chat
        ...
        # ✅ حفظ فوري قبل أي مزامنة/تعديلات واجهة (حتى لو حدث تعثّر لاحقًا تبقى الزيادة محفوظة)
        try:
            save_state()
        except Exception:
        ...
        # حفظ إضافي كإجراء أمان (لا يضر ولو تكرر)
        try:
            save_state()
        except Exception:
    ...
    # ✅ حفظ فوري أيضًا للرسائل المفردة
    try:
        save_state()
    except Exception:
...
async def send_preview(update: Update, context: ContextTypes.DEFAULT_TYPE, sess: Session, *, hide_links: bool):
    # ⛔ في المعاينة نجبر إخفاء بطاقة المعاينة فقط، لكن الروابط تبقى قابلة للنقر عبر TEXT_LINK
    hide_links = True
    ...
    if not getattr(sess, "chosen_chats", None):
        await context.bot.send_message(
            chat_id=user_id,
            text="⚠️ لا توجد وجهة للنشر.يرجى اختيار مجموعة أو قناة من «اختيار الوجهات».",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ رجوع", callback_data="back_main")]])
        )
    ...
    if getattr(sess, "single_attachment", None):
        ...
        c_raw = caption or a_caption
        await _send_captioned(context.bot, "document" if a_type=="document" else a_type, chat_id=user_id, file_id=file_id, caption=c_raw)
        m = await _send_text_with_links(context.bot, chat_id=user_id, text=status_block, reply_markup=action_kb)
        sess.preview_action_msg_id = m.message_id if m else None
        save_state()
        return
    ...
    if getattr(sess, "media_list", None):
        if len(sess.media_list) > 1:
            ...
            for idx, (t, fid, cap) in enumerate(sess.media_list):
                c_raw = (caption if idx == 0 else (cap or None))
                if c_raw:
                    cap_text, cap_ents = build_text_and_entities(c_raw)
                else:
                    cap_text, cap_ents = None, None
                if t == "photo":
                    media_group.append(InputMediaPhoto(media=fid, caption=cap_text, caption_entities=cap_ents))
                else:
                    media_group.append(InputMediaVideo(media=fid, caption=cap_text, caption_entities=cap_ents))
            await context.bot.send_media_group(chat_id=user_id, media=media_group)
        else:
            t, fid, cap = sess.media_list[0]
            c_raw = caption or cap
            await _send_captioned(context.bot, t, chat_id=user_id, file_id=fid, caption=c_raw)
        m = await _send_text_with_links(context.bot, chat_id=user_id, text=status_block, reply_markup=action_kb)
        sess.preview_action_msg_id = m.message_id if m else None
        save_state()
        return

    # نص فقط (تعطيل معاينة المواقع هنا فقط)
    if getattr(sess, "text", None):
        await _send_text_with_links(context.bot, chat_id=user_id, text=sess.text, disable_web_page_preview=True)
        m = await _send_text_with_links(context.bot, chat_id=user_id, text=status_block, reply_markup=action_kb)
        sess.preview_action_msg_id = m.message_id if m else None
        save_state()
        return
    ...
    if getattr(sess, "single_attachment", None):
        a_type, file_id, a_caption = sess.single_attachment
        c_raw = caption or a_caption
        c_txt, c_entities = build_text_and_entities(c_raw or "")
        if a_type == "document":
            await context.bot.send_document(chat_id=user_id, document=file_id, caption=c_txt, caption_entities=c_entities)
        elif a_type == "audio":
            await context.bot.send_audio(chat_id=user_id, audio=file_id, caption=c_txt, caption_entities=c_entities)
        elif a_type == "voice":
            await context.bot.send_voice(chat_id=user_id, voice=file_id, caption=c_txt, caption_entities=c_entities)
    ...
    if getattr(sess, "media_list", None):
        if len(sess.media_list) > 1:
            ...
            for idx, (t, fid, cap) in enumerate(sess.media_list):
                c_raw = (caption if idx == 0 else (cap or None))
                if c_raw:
                    c_txt, c_entities = build_text_and_entities(c_raw)
                else:
                    c_txt, c_entities = None, None
                if t == "photo":
                    media_group.append(InputMediaPhoto(media=fid, caption=c_txt, caption_entities=c_entities))
                else:
                    media_group.append(InputMediaVideo(media=fid, caption=c_txt, caption_entities=c_entities))
            await context.bot.send_media_group(chat_id=user_id, media=media_group)
        else:
            t, fid, cap = sess.media_list[0]
            c_raw = caption or cap
            c_txt, c_entities = build_text_and_entities(c_raw or "")
            if t == "photo":
                await context.bot.send_photo(chat_id=user_id, photo=fid, caption=c_txt, caption_entities=c_entities)
            else:
                await context.bot.send_video(chat_id=user_id, video=fid, caption=c_txt, caption_entities=c_entities)
        m = await context.bot.send_message(chat_id=user_id, text=status_block, reply_markup=action_kb)
        sess.preview_action_msg_id = m.message_id
        save_state()
        return

    # --- نص فقط (نعطل كرت المعاينة فقط) ---
    if getattr(sess, "text", None):
        txt, ents = build_text_and_entities(sess.text)
        await context.bot.send_message(
            chat_id=user_id,
            text=txt,
            entities=ents,
...
async def send_post_one_chat(
    ...
    *,
    ...
    if sess.single_attachment:
        ...
        if hide_links:
            c = hidden_links_or_plain(c, True)
            if a_type == "document":
                m = await context.bot.send_document(chat_id=chat_id, document=file_id, caption=c, parse_mode="HTML")
            elif a_type == "audio":
                m = await context.bot.send_audio(chat_id=chat_id, audio=file_id, caption=c, parse_mode="HTML")
            elif a_type == "voice":
                m = await context.bot.send_voice(chat_id=chat_id, voice=file_id, caption=c, parse_mode="HTML")
            else:
                m = None
        else:
            m = await _send_captioned(context.bot, a_type, chat_id=chat_id, file_id=file_id, caption=c)
        if m:
    ...
    if sess.media_list:
        if len(sess.media_list) > 1:
            ...
            for idx, (t, fid, cap) in enumerate(sess.media_list):
                ...
                if hide_links:
                    c = hidden_links_or_plain(c, True) if c else None
                    if t == "photo":
                        media_group.append(InputMediaPhoto(media=fid, caption=c, parse_mode="HTML"))
                    else:
                        media_group.append(InputMediaVideo(media=fid, caption=c, parse_mode="HTML"))
                else:
                    # يترك الكابتشن None لإرسالها لاحقًا فرديًا إن لزم
                    media_group.append(InputMediaPhoto(media=fid) if t == "photo" else InputMediaVideo(media=fid))
            ...
            if hide_links:
                c = hidden_links_or_plain(c, True)
                if t == "photo":
                    m = await context.bot.send_photo(chat_id=chat_id, photo=fid, caption=c, parse_mode="HTML")
                else:
                    m = await context.bot.send_video(chat_id=chat_id, video=fid, caption=c, parse_mode="HTML")
            else:
                m = await _send_captioned(context.bot, t, chat_id=chat_id, file_id=fid, caption=c)
    ...
    if caption:
        if hide_links:
            m = await context.bot.send_message(
                chat_id=chat_id,
                text=hidden_links_or_plain(caption, True),
                parse_mode="HTML",
                disable_web_page_preview=True
            )
        else:
            m = await _send_text_with_links(context.bot, chat_id=chat_id, text=caption)
        first_message_id = first_message_id or (m.message_id if m else None)
    ...
    if attach_to_prompt:
        if sess.campaign_id is not None and is_rebroadcast:
            ...
            ensure_prompt_ok = not still_any_for_chat

        if ensure_prompt_ok:
            try:
                prompt_msg = await _send_text_with_links(context.bot, chat_id=chat_id, text=reaction_prompt)
                if sess.campaign_id is not None:
...
async def _safe_send_one(context, cid, sess, *, is_rebroadcast, hide_links, reaction_prompt, sem):
    import random
    max_attempts = 3          # محاولة أولى + محاولتان احتياط
    base_backoff = 2.0        # 1x, 2x, 4x تقريباً

    async with sem:
        # Jitter بسيط قبل البدء يخفف ضغط الإرسال الجماعي على تيليجرام
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                # احترام مهلة تيليجرام + زيادة طفيفة لتجنب التصادم
                wait_s = float(getattr(e, "retry_after", 3)) + random.uniform(0.2, 0.6)
                await asyncio.sleep(wait_s)

            except (TimedOut, NetworkError):
                # باك أوف أُسّي خفيف للمحاولات التالية
                backoff = (base_backoff ** (attempt - 1)) + random.uniform(0.1, 0.4)
                await asyncio.sleep(backoff)

            except Exception:
                # أخطاء أخرى: جرّب باك أوف ثم أعد المحاولة
                backoff = (base_backoff ** (attempt - 1)) + random.uniform(0.1, 0.4)
                await asyncio.sleep(backoff)
...
async def publish_to_chats(context: ContextTypes.DEFAULT_TYPE, user_id: int, sess: Session, *, is_rebroadcast: bool, hide_links: bool):
    ...
    # أرسل بالتوازي مع تسطير بسيط لتخفيف الاندفاعة
    sem = asyncio.Semaphore(max(1, MAX_CONCURRENCY))
    tasks = []
    maxc = max(1, MAX_CONCURRENCY)

    for idx, cid in enumerate(target_chats):
        # stagger صغير يوزّع البداية على دفعات
        await asyncio.sleep(0.2 * (idx % maxc))
        tasks.append(asyncio.create_task(
            _safe_send_one(
                ...
                reaction_prompt=global_settings.get(
                    ...
                    "✍️ شاركنا برأيك عبر التفاعل أدناه 👇"
                ),
                sem=sem,
            )
        ))
...
async def handle_campaign_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("show_stats:"):
        ...
        if per_chat_lines:
            ...
            # لا تفصيل ولا تفاعلات إجمالية
            text += "\n(لا توجد تفاعلات حتى الآن)"
        else:
            # يوجد إجمالي لكن لا توجد رسائل مسجّلة لتفصيل الوجهات
            text += "\n(لا توجد رسائل مسجّلة لعرض التفصيل لكل وجهة حتى الآن)"
