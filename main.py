...
# يدعم http/https و روابط تبدأ بـ www.
URL_RE = re.compile(
...
def make_html_with_hidden_links(text: str) -> str:
    ...
    for m in URL_RE.finditer(text):
        ...
        # لو الرابط يبدأ بـ www. نضيف له https:// علشان تيليجرام يتعامل معه كرابط صحيح
        url = raw
...
def get_settings(user_id: int) -> Dict[str, Any]:
    # السجل الأساسي (مثل ما هو عندك الآن)
    ...
    # ✅ ضمان وجود المفاتيح حتى لو كان السجل قديم من ملف الحفظ
    s.setdefault("hide_links_default", True)
    s.setdefault("default_reactions_enabled", True)
    s.setdefault("permissions_mode", "all")
    s.setdefault("whitelist", set())
    s.setdefault("disabled_chats", set())
    if "templates" not in s:
        s["templates"] = s.get("templates", []) or []
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # معاينة
    if data == "preview":
        await send_preview(update, context, sess, hide_links=True)
        return
    # ====== النشر ======
...
async def _publish_and_report(context: ContextTypes.DEFAULT_TYPE,
    user_id: int,
    sess: "Session",
    allow_schedule: bool,
    hide_links: bool
...
async def handle_reactions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if campaign_id is not None:
        ...
        # محاولة نسخ احتياطي بعد التصويت
        try:
        ...
        # تحديث عدّاد هذه الوجهة/الرسالة (تفصيلي لكل مجموعة)
        ...
        # 1) تحديث زر الرسالة التي تم الضغط عليها الآن
        try:
            ...
            if current_mid and not skip_update_current:
                kb = InlineKeyboardMarkup([[
                    InlineKeyboardButton(
                        pos_label,
                        callback_data=f"like:{chat_id}:{base_or_msg_id}",
                    ),
                    InlineKeyboardButton(
                        neg_label,
                        callback_data=f"dislike:{chat_id}:{base_or_msg_id}",
                    ),
                ]])
        ...
        # 2) مزامنة باقي رسائل الحملة
        # الآن تُستدعى دائمًا، سواء كان نص الدعوة مفعّل أو مخفي
        try:
...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    # وضع الإرفاق القديم (يُستخدم فقط عندما تكون رسالة الدعوة مفعّلة)
    mode = globals().get("REACTIONS_ATTACH_MODE", "prompt_only")
    attach_to_prompt = (mode in ("prompt_only", "both"))
    attach_to_base   = (mode in ("base_only",   "both"))

    # حالة تفعيل / تعطيل نص الدعوة من لوحة التحكم
    prompt_enabled = global_settings.get("reactions_prompt_enabled", True)
    ...
    # مصدر الدعوات الصحيح: campaign_prompt_msgs
    alt_prompt_store = globals().get("campaign_prompt_msgs") or {}
    prompt_list = (
        list(alt_prompt_store.get(campaign_id, []))
        or list(alt_prompt_store.get(str(campaign_id), []))
        or []
    )
    ...
    # دالة مساعدة موحدة لبناء الكيبورد بعرض أكبر
    def _kb(cid: int, mid_for_buttons: int, base_mid: int):
        use_id = base_mid if isinstance(base_mid, int) else mid_for_buttons
        pos_label = f"\u00A0\u00A0{pos_emo} {tot_like}\u00A0\u00A0"
        neg_label = f"\u00A0\u00A0{neg_emo} {tot_dislike}\u00A0\u00A0"
        return InlineKeyboardMarkup([[
            InlineKeyboardButton(
                pos_label,
                callback_data=f"like:{cid}:{use_id}",
            ),
            InlineKeyboardButton(
                neg_label,
                callback_data=f"dislike:{cid}:{use_id}",
            ),
        ]])

    # إذا كانت نصوص الدعوة معطّلة من الإعدادات الحديثة:
    #    → نستخدم "وضع الأساس فقط" مهما كان REACTIONS_ATTACH_MODE
    ...
    # ===== هنا السلوك القديم عندما تكون نصوص الدعوة مفعّلة =====

    # 1) مزامنة رسائل الدعوة (إن كان attach_to_prompt مفعّلًا)
    if attach_to_prompt:
        for (cid, mid) in list(prompt_list):
            try:
                kb = _kb(cid, mid, base_map.get((campaign_id, cid), mid))
                await context.bot.edit_message_reply_markup(
                    chat_id=cid,
                    message_id=mid,
                    reply_markup=kb,
    ...
    # 2) مزامنة رسائل الأساس (إن كان attach_to_base مفعّلًا)
    if attach_to_base:
        for key, base_mid in list(base_map.items()):
            camp_k, cid = _parse_key_to_pair(key)
            if camp_k != campaign_id or not isinstance(base_mid, int):
...
async def send_preview(update: Update, context: ContextTypes.DEFAULT_TYPE, sess: Session, *, hide_links: bool):
    ...
    if caption and not getattr(sess, "single_attachment", None) and not getattr(sess, "media_list", None):
        ...
        await send_message_safe(
            ...
            disable_web_page_preview=True,  # نفس سلوك النشر الفعلي
        )
        something_sent = True
...
async def send_post_one_chat(
    ...
    *,
    ...
    # === السلوك القديم عندما تكون رسالة الدعوة مفعّلة ===
    mode = globals().get("REACTIONS_ATTACH_MODE", "prompt_only")
    attach_to_prompt = (mode in ("prompt_only", "both"))
    attach_to_base   = (mode in ("base_only",   "both"))
