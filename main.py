...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    مزامنة أزرار التفاعل لجميع رسائل الحملة وفق الوضع المختار:
      - REACTIONS_ATTACH_MODE = "prompt_only": الأزرار على رسائل الدعوة فقط (ويتم تنظيف الأزرار من منشور الأساس)
    ...
    # وضع prompt_only: نظّف كيبورد منشور الأساس وحدث الدعوات فقط
    if mode == "prompt_only":
        try:
        ...
        if prompt_list:
            for (cid, mid) in list(prompt_list):
                try:
                    ...
                    continue
        return  # لا تلمس منشور الأساس في هذا الوضع بعد التنظيف

    # الأوضاع الأخرى:
    # 1) حدّث رسائل الدعوة إن لزم
    ...
    # 2) وحدّث كيبورد منشور الأساس إن attach_to_base
    if attach_to_base:
        for key, base_mid in list(base_map.items()):
            camp_k, cid = _parse_key_to_pair(key)
            if camp_k != campaign_id or not isinstance(base_mid, int):
                continue
            try:
                kb = _kb_for(cid, base_mid, base_mid)
                await context.bot.edit_message_reply_markup(chat_id=cid, message_id=base_mid, reply_markup=kb)
            except Exception:
                continue
...
async def send_post_one_chat(
    ...
    *,
    ...
    if sess.campaign_id:
        ...
        # مهم: نربط منشور الأساس بالحملة حتى لو ما أرسلنا رسالة دعوة
    ...
    # أولاً: لو attach_to_base → نعلّق الأزرار على نفس منشور الأساس
    if attach_to_base and base_id_for_buttons:
        try:
            ...
            # لو فشل (رسالة قديمة أو غير ذلك) نكمّل بدون كسر الدالة
            pass

    # ثانياً: لو attach_to_prompt → نرسل/نحدّث رسالة الدعوة كما في السابق
