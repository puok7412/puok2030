...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    if campaign_styles.get(campaign_id) != style:
        ...
        try: save_state()
        except Exception: logger.exception("save_state (campaign_styles) failed")
    pos_emo, neg_emo = _get_reaction_pair(style)
    ...
    # ✅ أولاً: نظّف أي كيبورد على رسائل الأساس دائماً إن كان الوضع prompt_only
    if mode == "prompt_only":
        try:
        ...
        if prompt_list:
            for (cid, mid) in list(prompt_list):
                try:
                    ...
                    continue
        # 🔚 خروج مبكّر — لا تلمس الأساس إطلاقًا في هذا الوضع
        return

    # 👇 الأوضاع الأخرى (base_only / both) تبقى كما هي:
