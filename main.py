...
# ====== Handlers Registration (FINAL ORDER) ======
...
# Group-only command: grant temp publish
...
# Panel text inputs (editing reaction prompt / template name, إلخ)
application.add_handler(
    MessageHandler(
...
# Content capture in DM (text/media/docs/audio/voice) excluding ok/ok25s and commands
application.add_handler(
    MessageHandler(
...
# Post-campaign controls (stats / stop rebroadcast)
application.add_handler(CallbackQueryHandler(handle_campaign_buttons, pattern=r"^(show_stats:|stop_rebroadcast:)"), group=11)

# Admin control panel callbacks (panel:* / perm:*) — non-blocking so it لا يحجب باقي الأزرار
application.add_handler(CallbackQueryHandler(handle_control_buttons, pattern=r"^(panel:|perm:)", block=False), group=12)

# Catch-all for the publishing session UI (choose_chats / select_all / done_chats / back_main / preview / publish / schedule_menu / ssched_* / toggle_pin ...)
application.add_handler(CallbackQueryHandler(on_button), group=99)

# ---- Membership & chat discovery ----
