...
async def send_preview(update: Update, context: ContextTypes.DEFAULT_TYPE, sess: Session, *, hide_links: bool):
    ...
    if not getattr(sess, "chosen_chats", None):
        await context.bot.send_message(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ رجوع", callback_data="back_main")]]),
            disable_web_page_preview=True,  # لا نحتاج معاينة هنا
        )
        return
    ...
    if getattr(sess, "text", None):
        await context.bot.send_message(
            ...
            parse_mode=("HTML" if hide_links else None),
            disable_web_page_preview=True,  # هنا المهم: نص + روابط بدون معاينة
        )
        m = await context.bot.send_message(
            chat_id=user_id,
            text=status_block,
    ...
    *,
    ...
    if caption:
        m = await context.bot.send_message(
            ...
            parse_mode=("HTML" if hide_links else None),
            disable_web_page_preview=True,  # هنا المهم في النشر للمجموعات
        )
    ...
    # توحيد رسالة الأساس للأزرار ضمن الحملة (كما هو في كودك الأصلي)
    ...
    like_count, dislike_count = get_reaction_counts(chat_id, base_id_for_buttons, sess)
    style = getattr(sess, "reactions_style", "thumbs")
    pos_emo, neg_emo = _get_reaction_pair(style)
