...
async def send_stats_to_admin(app_or_ctx, owner_id: int, campaign_id: Optional[int]):
    """
    يرسل ملخص الإحصاءات في الخاص بعد كل إعادة بث:
    - إلى مالك الحملة (owner_id)
    - وإلى أي مستخدم مُنح تصريحًا مؤقتًا من هذا المالك وما زال تصريحه ساريًا ومستخدمًا.
    يتضمن الملخص: الإجمالي + تفصيل كل وجهة + أزرار (عرض التقييم / إيقاف الإعادة).
    """
    try:
        ...
        for (cmp_id, cid), base_mid in campaign_base_msg.items():
            if cmp_id != campaign_id:
                continue
            rec = reactions_counters.get((cid, base_mid), {}) or {}
            title = known_chats.get(cid, {}).get("title", str(cid))
            lines.append(f"• {title} — 👍 {int(rec.get('like', 0))} | 👎 {int(rec.get('dislike', 0))}")
        ...
        # أزرار سريعة: عرض التقييم + إيقاف الإعادة
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("📊 عرض التقييم", callback_data=f"show_stats:{campaign_id}")],
        ...
        # أرسل للمالك
        try:
            await bot.send_message(chat_id=owner_id, text=text, reply_markup=kb)
        except Exception:
            pass

        # ابحث عن مستخدم مُنح تصريحًا من هذا المالك وما زال تصريحه ساريًا وتم استخدامه
        # (قد يكون هناك أكثر من مستخدم؛ نرسل للأحياء فقط)
        now = datetime.utcnow()
        notified = set()
        for uid, grant in list(temp_grants.items()):
            try:
                if grant.get("granted_by") == owner_id and grant.get("used") and grant.get("expires") and grant["expires"] > now:
                    if uid not in notified and uid != owner_id:
                        try:
    ...
    if data == "panel:stats":
        ...
        for key in campaign_base_msg.keys():
            if isinstance(key, tuple) and len(key) >= 2:
                ...
                if "|" in ss:
                    try:
                        chat_ids.add(int(ss.split("|", 1)[1].strip().lstrip("-")))
                    except Exception:
                        pass
                elif "::" in ss:
                    try:
        ...
        rows = []
        # نعرض بحد أقصى 10 لائحة حديثة/مرتّبة (عدّلها إلى sorted(chat_ids) لو أردت الكل)
        ...
        for key, base_mid in campaign_base_msg.items():
            try:
                if isinstance(key, tuple) and len(key) >= 2:
                    ...
                    ss = key.strip()
                    if "|" in ss:
                        camp_id, cid = map(lambda x: int(x.strip().lstrip("-")), ss.split("|", 1))
                    elif "::" in ss:
                        camp_id, cid = map(lambda x: int(x.strip().lstrip("-")), ss.split("::", 1))
                    else:
                        continue
        ...
        if not pairs:
            ...
            for k, bm in list(campaign_base_msg.items()):
                ...
                if isinstance(k, tuple) and len(k) >= 2 and int(k[0]) == camp_id:
                    ...
                    if "|" in sss:
                        a, b = sss.split("|", 1)
                        if a.isdigit() and int(a) == camp_id and b.strip().lstrip("-").isdigit():
                            cid = int(b)
                    elif "::" in sss:
                        a, b = sss.split("::", 1)
                        if a.isdigit() and int(a) == camp_id and b.strip().lstrip("-").isdigit():
