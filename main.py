...
async def send_message_safe(bot, *, chat_id: int, text: str, parse_mode=None, reply_markup=None, disable_web_page_preview: bool = True):
    """إرسال رسالة بآلية إعادة محاولة بسيطة للتعامل مع بطء الشبكة/التأخيرات.
    يُعيد كائن الرسالة عند النجاح أو None عند الفشل بعد محاولات."""
    backoff = 1.0
    for _attempt in range(4):
        try:
            ...
            try:
                delay = float(getattr(e, "retry_after", backoff))
            except Exception:
                delay = backoff
            ...
            logger.exception("send_message_safe: unexpected error")
            break
    return None
...
# عميل HTTPX بطقم مهلات أكبر + pool_timeout لتجنّب PoolTimeout وقت الإقلاع
request = HTTPXRequest(
    connect_timeout=20.0,
    read_timeout=60.0,
    write_timeout=60.0,
    pool_timeout=20.0,   # ✅ الجديد والمهم
)
...
# أعلى الملف مرة واحدة:
_SAVE_LOCK = asyncio.Lock()
_SAVE_DEBOUNCE = float(os.getenv("SAVE_DEBOUNCE_SEC", "1.0"))  # يمكنك ضبطها مثلاً إلى 0.3
_LAST_SAVE_REQ = 0.0
...
def save_state(*, sync: bool = False):
    """يحفظ الحالة.
    افتراضيًا: غير حاجز بالخلفية مع دمج الطلبات (debounce).
    للنسخ اليدوي الفوري (زر النسخ الاحتياطي): save_state(sync=True)
    """
    global STATE_PATH, _LAST_SAVE_REQ

    def _build_snapshot():
        # —— بناء اللقطة كما في نسختك ——
        try:
    ...
    def _write_snapshot(data):
        # ضمان مسار الكتابة — بدون أي fallback إلى /tmp
        dirpath = os.path.dirname(STATE_PATH) or "."
        try:
            os.makedirs(dirpath, exist_ok=True)
        except PermissionError:
            logger.exception("save_state: لا يمكن إنشاء المجلد للكتابة على STATE_PATH=%s", STATE_PATH)
            return
        except Exception:
            # قد يكون المجلد موجودًا بالفعل
            logger.exception("save_state: os.makedirs فشل لمسار %s", dirpath)
    ...
    async def _bg_save():
        # دمج (debounce): نفّذ آخر طلب فقط ضمن نافذة SAVE_DEBOUNCE
        stamp = time.time()
        globals()["_LAST_SAVE_REQ"] = stamp
        await asyncio.sleep(_SAVE_DEBOUNCE)
        if globals().get("_LAST_SAVE_REQ") != stamp:
            return  # جاء طلب أحدث؛ دع الأحدث ينفّذ

        async with _SAVE_LOCK:
            data = _build_snapshot()
            # I/O على ثريد منفصل حتى لا نحجز حلقة الحدث
            await asyncio.to_thread(_write_snapshot, data)

    try:
        if sync:
            # حفظ فوري متزامن — استخدمه عند زر النسخ الاحتياطي اليدوي
            _write_snapshot(_build_snapshot())
            return

        # حفظ غير حاجز تلقائيًا لبقية الاستدعاءات
        loop = asyncio.get_running_loop()
        loop.create_task(_bg_save())

    except RuntimeError:
        # لا توجد حلقة asyncio حالياً (مثلاً أثناء الإقلاع المبكر): احفظ متزامنًا
        _write_snapshot(_build_snapshot())
    except Exception:
...
def _current_total_votes() -> int:
    """حساب إجمالي التفاعلات (لايك + ديسلايك) لكل الحملات والرسائل الفردية."""
    total = 0
    try:
        for rec in (globals().get("campaign_counters") or {}).values():
            try:
                total += int(rec.get("like", 0)) + int(rec.get("dislike", 0))
            except Exception:
                continue
        for rec in (globals().get("reactions_counters") or {}).values():
            try:
                total += int(rec.get("like", 0)) + int(rec.get("dislike", 0))
            except Exception:
                continue
...
def build_persist_snapshot() -> Dict[str, Any]:
    ...
    _campaign_prompt_msgs  = (globals().get("campaign_prompt_msgs") or {})
    _active_rebroadcasts   = (globals().get("active_rebroadcasts") or {})  # ✅ جديد: حفظ الجدولة النشطة
    _gs                    = (globals().get("global_settings") or {})

    known_chats_norm       = {str(k): v for k, v in _known_chats.items()}
    campaign_messages_norm = {str(k): v for k, v in _campaign_messages.items()}
    campaign_counters_norm = {str(k): v for k, v in _campaign_counters.items()}

    # تطبيع وحذف التكرارات في campaign_prompt_msgs
    cpm_norm: Dict[str, List[List[int]]] = {}
    for camp_id, pairs in _campaign_prompt_msgs.items():
        key = str(camp_id)
        seen = set()
        acc: List[List[int]] = []
        for p in (pairs or []):
    ...
    return {
        "total_votes": _current_total_votes(),
        "snapshot_ts": int(time.time()),
        "known_chats": known_chats_norm,
        "campaign_messages": campaign_messages_norm,
        ...
        "campaign_prompt_msgs": cpm_norm,
        "campaign_counters": campaign_counters_norm,
        ...
        # ✅ جديد: حفظ الحملات المجدولة/النشطة لإعادة تركيب JobQueue بعد الإقلاع
        # (نحوّل لأي قيم غير JSONable إن وُجدت عبر _walk_to_jsonable)
        "active_rebroadcasts": _walk_to_jsonable(_active_rebroadcasts),
        ...
        "global_settings": {
            ...
            "reaction_prompt_text": _gs.get(
                "reaction_prompt_text",
                "✍️ شَارِكْنَا رَأْيَكَ عَبْرَ التَّفَاعُل أَدْنَاهُ 👇",
            ),
            # ✅ الخيار الجديد: حفظ حالة تفعيل/تعطيل نص الدعوة
            "reactions_prompt_enabled": _gs.get("reactions_prompt_enabled", True),
        },
    }


def apply_persist_snapshot(data: Dict[str, Any]) -> None:
    """يطبّق لقطة الحالة المحفوظة إلى المتغيّرات العالمية، مع تطبيع المفاتيح إلى int."""
    if not data:
        return
    ...
    def _int_keys(d):
        ...
        if isinstance(d, dict):
            for k, v in d.items():
                try:
                    out[int(str(k).strip())] = v
                except Exception:
                    continue
        return out

    def _pairs_to_unique_tuples(lst):
        out = []
        seen = set()
        for p in (lst or []):
            try:
    ...
    # known_chats
    known_chats.clear()
    known_chats.update(_int_keys(data.get("known_chats", {})))

    # campaign_messages
    campaign_messages.clear()
    campaign_messages.update(_int_keys(data.get("campaign_messages", {})))

    # جداول مفاتيحها tuple
    ...
    campaign_counters.update(_int_keys(data.get("campaign_counters", {})))

    reactions_counters.clear()
    reactions_counters.update(_unpack_tuple_keys(data.get("reactions_counters", {})))

    _normalize_voters_keys_inplace()

    # أنماط التفاعلات على مستوى الحملة ← طَبِّع المفاتيح إلى int
    if "campaign_styles" in globals():
        cs = data.get("campaign_styles", {})
        campaign_styles.clear()
        campaign_styles.update(_int_keys(cs))

    # أنماط التفاعلات على مستوى الرسالة
    if "reaction_style_by_message" in globals():
        reaction_style_by_message.clear()
        reaction_style_by_message.update(
            _unpack_tuple_keys(data.get("reaction_style_by_message", {}))
        )

    # ✅ استعادة قوائم رسائل الدعوات لكل حملة (ضروري للمزامنة عبر كل المجموعات) بمفاتيح int
    campaign_prompt_msgs.clear()
    _raw_cpm = data.get("campaign_prompt_msgs", {})
    if isinstance(_raw_cpm, dict):
        for camp_k, pairs in _raw_cpm.items():
            try:
                camp_id = int(str(camp_k).strip())
            except Exception:
                continue
            campaign_prompt_msgs[camp_id] = _pairs_to_unique_tuples(pairs)

    # ✅ جديد: استعادة الحملات المجدولة/النشطة (ليتمكن startup من إعادة جدولة JobQueue)
    try:
        active_rebroadcasts.clear()
        _raw_ar = data.get("active_rebroadcasts", {})
        if isinstance(_raw_ar, dict):
            active_rebroadcasts.update(_raw_ar)
    except Exception:
        pass
...
async def _resync_all_keyboards(bot):
    ...
    # ✅ 0) لا تلمس إلا الحملات "النشطة" فقط (حتى لا يركّب أزرار على كل منشورات قديمة بعد الإقلاع)
    active_campaign_ids = set()
    try:
        for _name, info in (active_rebroadcasts or {}).items():
            try:
                payload = (info or {}).get("payload") or {}
                cid = payload.get("campaign_id")
                if cid is not None:
                    active_campaign_ids.add(int(cid))
            except Exception:
                continue
    except Exception:
        active_campaign_ids = set()
    ...
    try:
        ...
        # ✅ إذا لا توجد حملات نشطة، لا تزامن أي حملة إطلاقًا
        ...
        for camp_id in (campaign_messages or {}).keys():
            ...
            # ✅ تجاهل الحملات القديمة غير النشطة
            if camp_i not in active_campaign_ids:
                continue

            try:
                await _sync_campaign_keyboards(ctx, camp_i)
            except Exception:
                continue
    except Exception:
    ...
    # ✅ نفس الفكرة: لا تعيد تركيب أزرار قديمة بعد الإقلاع (وخصوصًا لو عندك تاريخ طويل)
    #    نخليها تعمل فقط للرسائل غير التابعة لحملة "نشطة" (وتتخطّى أي حملة)
    try:
        for k, rec in (reactions_counters or {}).items():
            try:
                ...
                camp = message_to_campaign.get((chat_id, message_id))
                if camp is not None:
                    # ✅ إذا كانت حملة غير نشطة، لا تلمسها (حتى ما نركّب على القديم)
                    try:
                        if int(camp) not in active_campaign_ids:
                            continue
                    except Exception:
                        continue
                    # لو نشطة، اتركها (لأن مزامنتها تتم في الجزء الأول)
                    continue

                # ✅ إذا لا توجد حملات نشطة أصلاً: لا تركب شيئًا على رسائل فردية قديمة
                if not active_campaign_ids:
                    continue
...
def load_state():
    ...
    try:
        ...
        for bucket in _PERSIST_BUCKETS:
            ...
            # ✅ تطبيع مفاتيح campaign_counters إلى int (كانت محفوظة كسلاسل)
            if bucket == "campaign_counters" and isinstance(loaded, dict):
                fixed = {}
                for k, v in loaded.items():
                    try:
                        ...
                        # نتجاهل مفاتيح غير قابلة للتحويل
                        continue
                loaded = fixed

            # ✅ إعادة فك مفاتيح الـ tuple للبكتات التي تُحفظ كمفاتيح مجمّعة داخل JSON
            # هذا يمنع اختلاف عدادات الحملات بين المجموعات بعد الإعادة/التحميل
            try:
                if bucket in _TUPLEKEY_BUCKETS and isinstance(loaded, dict):
                    loaded = _unpack_tuple_keys(loaded)
            except Exception:
                pass
        ...
        if not loaded_any:
            ...
            # (اختياري) نحفظ فورًا بعد التطبيع لضمان بقاء المفاتيح بالهيئة الصحيحة
            try:
                save_state()
            except Exception:
                logger.exception("post-load save_state failed")
...
global_settings = {
    ...
    "reaction_prompt_text": "✍️ شَارِكْنَا رَأْيَكَ عَبْرَ التَّفَاعُل أَدْنَاهُ 👇",
    "reactions_prompt_enabled": True,  # رسالة الدعوة مفعلة افتراضيا
}
...
def new_campaign_id() -> int:
    """يولّد معرّف حملة فريدًا دائمًا حتى بعد إعادة التشغيل."""
...
def sanitize_text(text: Optional[str]) -> Optional[str]:
    ...
    # توحيد الأسطر الفارغة: أكثر من 3 أسطر متتالية → سطرين
    txt = re.sub(r'\n{3,}', '\n\n', txt)
    # توحيد الفراغات: مسافتين أو Tab أو أكثر → مسافة واحدة
    txt = re.sub(r'[ \t]{2,}', ' ', txt)
    return txt.strip()
...
def make_html_with_hidden_links(text: str) -> str:
    ...
    for m in URL_RE.finditer(text):
        ...
        # 🔥 النص الجديد للرابط المخفي
        base_label = "اضغط هنا لعرض التفاصيل"
...
async def start_publishing_keyword(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ⛔ امنع OK اثناء الصيانة (مع صورة)
    if global_settings.get("maintenance_mode", False):
        await send_maintenance_notice(context.bot, user_id)
        return

    # 1) لو عنده تصريح موقت فعال
    if _user_has_active_grant(user_id):
        await start_publishing_session(user, context)
        return

    # 2) حاول اولاً من الكاش العادي
    admin_ids = await chats_where_user_is_admin(context, user_id)
    if admin_ids:
        await start_publishing_session(user, context)
        return

    # 3) في حالة الكاش فاضي او ناقص (بعد الاقلاع مثلاً) افحص مباشرة كل القروبات المعروفة
    try:
        from_chat_ids = list(known_chats.keys())
    except Exception:
        from_chat_ids = []

    if from_chat_ids:
        found_admin = False
        for cid in from_chat_ids:
            try:
                member = await context.bot.get_chat_member(cid, user_id)
                if member.status in ("administrator", "creator"):
                    # حدث الكاش محلياً
                    known_chats_admins.setdefault(cid, {})[user_id] = member.user.full_name or ""
                    found_admin = True
            except Exception:
                continue
    ...
    # 4) لو لا تصريح ولا مشرف في اي قروب معروف → نفس الرسالة القديمة
    await send_message_safe(
        context.bot,
        chat_id=update.effective_chat.id,
        text="🔒 هذا الامر متاح فقط بتصريح موقت فعال من مشرف المجموعة."
    )
...
async def start_with_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if global_settings.get("maintenance_mode", False):
        await send_message_safe(
            context.bot,
            chat_id=update.effective_chat.id,
            text="🛠️ نظام النشر تحت الصيانة والتحديث حاليًا."
        )
        return
    ...
    if not rec:
        await send_message_safe(
            context.bot,
            chat_id=update.effective_chat.id,
            text="⛔ رابط غير صالح أو منتهي."
        )
        return

    # التحقق من الملكية
    if user.id != rec.get("user_id"):
        await send_message_safe(
            context.bot,
            chat_id=update.effective_chat.id,
            text="⛔ هذا الرابط ليس لك. اطلب من المشرفين منحك تصريح نشر مؤقت."
        )
        return

    # التحقق من الصلاحية
    if datetime.utcnow() > rec.get("expires", datetime.utcnow()):
        await send_message_safe(
            context.bot,
            chat_id=update.effective_chat.id,
            text="⌛ انتهت صلاحية الرابط."
        )
...
_last_backup_votes_mark = int((globals().get("global_settings") or {}).get("last_backup_votes", 0))


async def _maybe_backup_after_vote(bot, *, reason: str = "+5"):
    """ينفّذ نسخًا احتياطيًا تلقائيًا عندما يزيد الإجمالي بمقدار 5 تفاعلات منذ آخر نسخ موثّق."""
    global _last_backup_votes_mark
    try:
        cur = _current_total_votes()
        if cur - _last_backup_votes_mark >= 5:
            fn = globals().get("backup_to_tg")
            if callable(fn):
                ok = await fn(bot, reason=reason)
                if ok:
                    _last_backup_votes_mark = cur
    except Exception:
        logger.exception("_maybe_backup_after_vote failed")
...
async def handle_reactions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # like:cid:mid  (mid هنا غالباً هو base_mid في الحملة)
    try:
    ...
    # حالة تفعيل/تعطيل رسالة الدعوة من الإعدادات العامة
    prompt_enabled = global_settings.get("reactions_prompt_enabled", True)

    # ✅ تحديد الحملة بشكل أقوى (حتى لو كان الربط ناقص عند بعض الرسائل القديمة)
    campaign_id = message_to_campaign.get((chat_id, base_or_msg_id))
    current_mid = getattr(query.message, "message_id", None)
    if campaign_id is None and current_mid:
        campaign_id = message_to_campaign.get((chat_id, current_mid))

    # ✅ قفل خفيف لمنع تداخل التصويت (Campaign أو Message)
    try:
        import asyncio
        _locks = globals().setdefault("_reaction_locks", {})
        lock_key = ("camp", int(campaign_id)) if campaign_id is not None else ("msg", int(chat_id), int(base_or_msg_id))
    ...
    # --- حالة حملة: نحدّث إجمالي الحملة + عدّاد الوجهة/الرسالة ---
    if campaign_id is not None:
        # ✅ تحديث العدادات داخل القفل فقط، ثم تنفيذ الشبكة خارج القفل
        if lock:
            async with lock:
                cc = campaign_counters.setdefault(
                    campaign_id,
                    {"like": 0, "dislike": 0, "voters": {}}
                )
                if not isinstance(cc.get("voters"), dict):
                    cc["voters"] = {}

                # ✅ ثبت الربط للحملة على الأقل على (base_or_msg_id) وعلى رسالة الضغط الحالية (إن وجدت)
                try:
                    message_to_campaign[(chat_id, base_or_msg_id)] = campaign_id
                    if current_mid:
                        message_to_campaign[(chat_id, current_mid)] = campaign_id
                    save_state()
                except Exception:
                    pass
                ...
                if user.id in cc["voters"]:
                    ...
                    # تحديث عدّاد هذه الوجهة/الرسالة (تفصيلي لكل مجموعة) — اختياري
                    rec = reactions_counters.setdefault(
                        (chat_id, base_or_msg_id),
                        {"like": 0, "dislike": 0, "voters": {}},
                    )
                    if not isinstance(rec.get("voters"), dict):
                        rec["voters"] = {}
                    if user.id not in rec["voters"]:
                        if action == "like":
                            rec["like"] += 1
                        else:
                            rec["dislike"] += 1
                        rec["voters"][user.id] = action

                    # حفظ فوري
                    try:
                        save_state()
                    except Exception:
                        logger.exception("save_state after reaction (immediate) failed")

                # snapshot للعرض خارج القفل
        ...
        # ✅ إذا سبق وصوّت: رد سريع ولا تكمل
        if already_voted:
            try:
                await query.answer(
                    f"سجّلنا تفاعلك على هذا المنشور مسبقًا. شكرًا {first_name} 💙",
                    show_alert=True,
                )
            except BadRequest:
                pass
        ...
        # محاولة نسخ احتياطي بعد التصويت (خارج القفل)
        try:
            maybe = globals().get("_maybe_backup_after_vote")
            if callable(maybe):
                context.application.create_task(maybe(context.bot))
        except Exception:
            pass

        # نصوص الأزرار (مسافات غير منكسرة لتكون أعرض)
        pos_label = f"\u00A0\u00A0{pos_emo} {like_cnt}\u00A0\u00A0"
        neg_label = f"\u00A0\u00A0{neg_emo} {dislike_cnt}\u00A0\u00A0"

        # 1) تحديث زر الرسالة التي تم الضغط عليها الآن (إن كانت رسالة تصويت/دعوة)
        try:
            skip_update_current = (
                mode == "prompt_only"
                and prompt_enabled
                and current_mid == base_or_msg_id
            )
            if current_mid and not skip_update_current:
                kb = InlineKeyboardMarkup([[
                    InlineKeyboardButton(pos_label, callback_data=f"like:{chat_id}:{base_or_msg_id}"),
                    InlineKeyboardButton(neg_label, callback_data=f"dislike:{chat_id}:{base_or_msg_id}"),
                ]])
        ...
        # 2) ✅ مزامنة كل رسائل الحملة (الأهم لحل اختلاف الأرقام بين المجموعات)
        try:
        ...
        async with lock:
            ...
            if user.id in voters:
                ...
                try:
                    ...
                    logger.exception("save_state after reaction (single/immediate) failed")
    ...
    if already_voted:
        try:
            await query.answer(
                f"عزيزي {first_name}، لقد سجّلنا تفاعلك على هذا المنشور سابقًا. شكرًا لك 💙",
                show_alert=True,
            )
        except BadRequest:
            pass
...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    """
    مزامنة أزرار التفاعل للحملة مع إصلاح جذري:
    ✅ تحديث "آخر" رسالة تصويت فقط لكل مجموعة
    ✅ إزالة الكيبورد من الرسائل القديمة (حتى لا تركب على منشور سابق بعد الإعادة)
    ✅ توحيد الأرقام بين كل المجموعات دائماً (إجمالي الحملة)
    """
    # ✅ قفل خفيف للحملة (Snapshot تحت القفل ثم تحديثات خارج القفل)
    try:
        import asyncio
        _locks = globals().setdefault("_reaction_locks", {})
        lock_key = ("camp", int(campaign_id))
        lock = _locks.get(lock_key)
        if lock is None:
            lock = asyncio.Lock()
            _locks[lock_key] = lock
    ...
    if lock:
        async with lock:
            ...
            prompt_list = list((campaign_prompt_msgs.get(campaign_id, []) or []))
    ...
    def _kb(cid: int, use_id: int):
        pos_label = f"\u00A0\u00A0{pos_emo} {tot_like}\u00A0\u00A0"
        neg_label = f"\u00A0\u00A0{neg_emo} {tot_dislike}\u00A0\u00A0"
        return InlineKeyboardMarkup([[
            InlineKeyboardButton(pos_label, callback_data=f"like:{cid}:{use_id}"),
            InlineKeyboardButton(neg_label, callback_data=f"dislike:{cid}:{use_id}"),
        ]])

    def _base_mid_for(cid: int):
        try:
            v = base_map.get((campaign_id, cid))
            return int(v) if isinstance(v, int) else None
        except Exception:
            return None

    # ✅ إن كانت الدعوة معطلة: نضع الأزرار على الأساس فقط
    if not prompt_enabled:
        for k, base_mid in list(base_map.items()):
            try:
                if not (isinstance(k, tuple) and len(k) >= 2 and int(k[0]) == int(campaign_id)):
                    continue
                cid = int(k[1])
                if not isinstance(base_mid, int):
                    continue
                kb = _kb(cid, int(base_mid))
                await context.bot.edit_message_reply_markup(
                    chat_id=cid,
                    message_id=int(base_mid),
                    reply_markup=kb,
                )
                message_to_campaign[(cid, int(base_mid))] = campaign_id
            except Exception:
                continue
        try:
            save_state()
    ...
    # ===== عندما تكون رسالة الدعوة مفعّلة =====

    # 1) ✅ dedupe: احتفظ بآخر رسالة لكل مجموعة
    latest_by_chat = {}
    for (cid, mid) in list(prompt_list):
        try:
            cid_i = int(cid); mid_i = int(mid)
        except Exception:
            continue
        latest_by_chat[cid_i] = mid_i

    # إزالة الكيبورد من الرسائل القديمة
    try:
        for (cid, mid) in list(prompt_list):
            try:
                cid_i = int(cid); mid_i = int(mid)
                ...
                continue
            if latest_by_chat.get(cid_i) != mid_i:
                try:
                    await context.bot.edit_message_reply_markup(
                        chat_id=cid_i,
                        message_id=mid_i,
    ...
    # 2) ✅ حدّث آخر رسالة دعوة لكل مجموعة
    if attach_to_prompt:
        for cid_i, prompt_mid in list(latest_by_chat.items()):
            try:
                base_mid = _base_mid_for(cid_i)
                use_id = base_mid if isinstance(base_mid, int) else prompt_mid
                kb = _kb(cid_i, int(use_id))
                await context.bot.edit_message_reply_markup(
                    chat_id=cid_i,
                    message_id=int(prompt_mid),
                    reply_markup=kb,
                )
                message_to_campaign[(cid_i, int(use_id))] = campaign_id
                message_to_campaign[(cid_i, int(prompt_mid))] = campaign_id
            except Exception:
                continue

    # 3) (اختياري) مزامنة الأساس إن كان وضع both/base_only مفعّل
    if attach_to_base:
        for k, base_mid in list(base_map.items()):
            try:
                if not (isinstance(k, tuple) and len(k) >= 2 and int(k[0]) == int(campaign_id)):
    ...
    # 4) ✅ أعد حفظ قائمة الدعوات بعد تنظيف القديم (نخزن آخر رسالة فقط لكل مجموعة)
    try:
        campaign_prompt_msgs[campaign_id] = [(cid, mid) for cid, mid in latest_by_chat.items()]
        save_state()
    except Exception:
        pass
...
async def send_preview(update: Update, context: ContextTypes.DEFAULT_TYPE, sess: Session, *, hide_links: bool):
    query = update.callback_query
    user_id = query.from_user.id
    caption = sess.text  # نص المنشور الأساسي
    ...
    # --- أولاً: الوسائط (ألبوم/صور/فيديو) تظهر أعلى المعاينة دائماً إن وجدت ---
    if getattr(sess, "media_list", None):
        if len(sess.media_list) > 1:
            media_group = []
            for idx, (t, fid, cap) in enumerate(sess.media_list):
                # لو ما فيه ملف مرفق، ممكن نستعمل نص المنشور ككابشن لأول وسائط
                if caption and not getattr(sess, "single_attachment", None) and idx == 0:
                    c = caption
                else:
                    c = cap or None
                if c:
                ...
                if t == "photo":
                    media_group.append(
                        InputMediaPhoto(
                            media=fid,
                            caption=c,
                            parse_mode=("HTML" if (hide_links and c) else None),
                        )
                    )
                else:
                    media_group.append(
            ...
            if caption and not getattr(sess, "single_attachment", None):
                c = caption
            else:
                c = cap or None
            if c:
    ...
    # --- ثانياً: الملف (صوت / مستند / فويس) + نص المنشور ككابشن تحته ---
    if getattr(sess, "single_attachment", None):
        a_type, file_id, a_caption = sess.single_attachment
        # نعطي الأولوية لنص المنشور ليكون هو الكابشن
    ...
    # --- ثالثاً: نص فقط لو ما فيه لا وسائط ولا ملف ---
    if caption and not getattr(sess, "single_attachment", None) and not getattr(sess, "media_list", None):
        await context.bot.send_message(
            chat_id=user_id,
...
async def send_post_one_chat(
    ...
    *,
    ...
    # --- أولاً: الوسائط (ألبوم/صور/فيديو) ترسل في الأعلى دائماً إن وجدت ---
    if sess.media_list:
        if len(sess.media_list) > 1:
            media_group = []
            for idx, (t, fid, cap) in enumerate(sess.media_list):
                # لو ما فيه ملف مرفق، ممكن استخدام نص المنشور ككابشن لأول وسائط
                if caption and not sess.single_attachment and idx == 0:
                    c = caption
                else:
                    c = cap or None
                if c:
            ...
            t, fid, cap = sess.media_list[0]
            if caption and not sess.single_attachment:
                c = caption
            else:
                c = cap or None
            if c:
    ...
    if sess.single_attachment:
        ...
        if a_type == "document":
            ...
            m = await context.bot.send_audio(
                chat_id=chat_id,
                audio=file_id,
                caption=c,
                parse_mode=("HTML" if (hide_links and c) else None),
            ...
            m = None
        if m and not first_message_id:
            first_message_id = m.message_id

    # --- ثالثاً: نص فقط لو ما فيه لا وسائط ولا ملف ---
    if caption and not sess.single_attachment and not sess.media_list:
        m = await context.bot.send_message(
            chat_id=chat_id,
            text=hidden_links_or_plain(caption, hide_links),
            parse_mode=("HTML" if hide_links else None),
    ...
    # توحيد رسالة الأساس للأزرار ضمن الحملة (لأجل العدّادات والـ callback فقط)
    ...
    # الرسالة التي سنركّب عليها الكيبورد فعليًا في هذا النشر (جديد كل مرة)
    keyboard_target_mid = first_message_id

    # --- التفاعلات ---
    # ✅ إذا التصويت معطل: نظّف أي أزرار قديمة (خصوصاً في الإعادة) ثم اخرج بدون أي تصويت
    if (not global_settings.get("reactions_feature_enabled", True)) or (not getattr(sess, "use_reactions", True)):
        try:
            if sess.campaign_id is not None:
                lst = campaign_prompt_msgs.get(sess.campaign_id, []) or []
                kept = []
                for (c, mid) in list(lst):
                    if c == chat_id:
                        try:
                            await context.bot.edit_message_reply_markup(
                                chat_id=chat_id,
                        ...
                        kept.append((c, mid))
                campaign_prompt_msgs[sess.campaign_id] = kept
                save_state()
        except Exception:
            pass
    ...
    try:
        ...
        if sess.campaign_id is not None:
            ...
            if style is None:
                style = reaction_style_by_message.get(
                    (chat_id, base_id_for_buttons),
                    "thumbs",
                )
    ...
    # نصوص الأزرار بعرض أكبر (مسافات غير منكسرة)
    like_label = f"\u00A0\u00A0{pos_emo} {like_count}\u00A0\u00A0"
    dislike_label = f"\u00A0\u00A0{neg_emo} {dislike_count}\u00A0\u00A0"

    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton(
            like_label,
            callback_data=f"like:{chat_id}:{base_id_for_buttons}",
        ),
        InlineKeyboardButton(
            dislike_label,
            callback_data=f"dislike:{chat_id}:{base_id_for_buttons}",
        ),
    ]])

    # ربط رسالة الأساس بالحملة بالتفاعلات
    if sess.campaign_id is not None:
        try:
    ...
    # هل نستخدم رسالة دعوة أم نركّب الأزرار مباشرة؟
    prompt_enabled = global_settings.get("reactions_prompt_enabled", True)
    ...
    if not prompt_enabled:
        try:
            await context.bot.edit_message_reply_markup(
                chat_id=chat_id,
                message_id=keyboard_target_mid,
                reply_markup=kb,
            )
            return first_message_id
        except Exception:
            try:
                ...
                if sess.campaign_id is not None:
                    message_to_campaign[(chat_id, base_id_for_buttons)] = sess.campaign_id
                    # ✅ اربط أيضًا رسالة الدعوة نفسها بالحملة (أمان)
                    message_to_campaign[(chat_id, prompt_msg.message_id)] = sess.campaign_id
    ...
    # === سلوك ثابت عند تفعيل رسالة الدعوة ===
    mode = globals().get("REACTIONS_ATTACH_MODE", "prompt_only")
    attach_to_prompt = (mode in ("prompt_only", "both"))
    attach_to_base   = (mode in ("base_only",   "both"))

    # (اختياري) تركيب على المنشور نفسه عند both/base_only
    if attach_to_base:
        try:
            await context.bot.edit_message_reply_markup(
                chat_id=chat_id,
    ...
    # ✅ أهم إصلاح: لا تعدّل رسالة دعوة قديمة لنفس المجموعة (هذا سبب انتقال الأزرار للمنشور السابق)
    if attach_to_prompt and sess.campaign_id is not None:
        # 1) افصل أي كيبورد عن رسائل دعوة قديمة في نفس المجموعة داخل نفس الحملة
        try:
            lst = campaign_prompt_msgs.setdefault(sess.campaign_id, [])
            kept = []
            for (c, mid) in list(lst):
            ...
            # 2) أرسل رسالة دعوة جديدة دائماً تحت المنشور الحالي
            try:
                prompt_msg = await context.bot.send_message(
                    chat_id=chat_id,
                ...
                # ✅ اربط رسالة الدعوة الجديدة بالحملة (أمان للـ callback)
                message_to_campaign[(chat_id, prompt_msg.message_id)] = sess.campaign_id

                # خزّن آخر دعوة لهذه المجموعة فقط
                lst = [(c, mid) for (c, mid) in lst if c != chat_id]
                lst.append((chat_id, prompt_msg.message_id))
                campaign_prompt_msgs[sess.campaign_id] = lst
                save_state()
            except Exception:
                pass

            # 3) تحسين مزامنة الأرقام بين المجموعات وقت النشر/الإعادة:
            #    حدّث reply_markup لبقية رسائل الدعوة المسجلة للحملة
            try:
                for (c, mid) in list(campaign_prompt_msgs.get(sess.campaign_id, []) or []):
                    if c == chat_id and mid == (prompt_msg.message_id if 'prompt_msg' in locals() and prompt_msg else None):
                        continue
                    try:
                        await context.bot.edit_message_reply_markup(
                            chat_id=c,
...
async def publish_to_chats(
    ...
    *,
    ...
    # ✅ ثبّت disabled_chats كـ set دائمًا (قد ترجع list من state)
    disabled = s.get("disabled_chats", set())
    try:
        if not isinstance(disabled, set):
            disabled = set(disabled or [])
        s["disabled_chats"] = disabled
    except Exception:
        disabled = set()
        s["disabled_chats"] = disabled

    # ✅ ثبّت chosen_chats كقائمة أرقام + إزالة التكرار بدون تغيير ترتيب الإرسال كثيرًا
    raw_chats = list(getattr(sess, "chosen_chats", []) or [])
    norm_chats = []
    seen = set()
    for x in raw_chats:
        try:
        ...
        if cid in seen:
            continue
        seen.add(cid)
        norm_chats.append(cid)

    target_chats = [cid for cid in norm_chats if cid not in disabled]
        ...
        for cid in target_chats:
            ...
            try:
                if not isinstance(blocked, set):
                    ...
                    group_permissions.setdefault(cid, {})["blocked_admins"] = blocked
            except Exception:
                blocked = set()
                group_permissions.setdefault(cid, {})["blocked_admins"] = blocked
    ...
    tasks = [
        _safe_send_one(
            ...
            is_rebroadcast=is_rebroadcast,
            hide_links=hide_links,
            # ✅ خذ نص الدعوة من global_settings مع ديفولت جديد
            reaction_prompt=global_settings.get(
                "reaction_prompt_text",
                "✍️ شاركنا رأيك عبر التفاعل أدناه 👇"
            ),
            sem=sem,
        )
        for cid in target_chats
...
async def rebroadcast_job(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ total/left بأمان (حتى لو صارت نص/None)
    try:
    ...
    tmp = Session(
        ...
        single_attachment=data.get("single_attachment"),
        use_reactions=bool(data.get("use_reactions", True)),
        chosen_chats=set(data.get("chosen_chats") or []),
    ...
    # ✅ تحديث العدّ وإيقاف الجدولة عند الانتهاء (آمن)
    left = max(0, left - 1)
    data["left"] = left
...
async def schedule_rebroadcast(app_or_ctx, user_id: int, sess: Session, interval_seconds: int = 7200, total_times: int = 12):
    ...
    # ✅ امنع تكرار جدولة نفس الحملة (لو موجودة قبل)
    try:
        jq0 = None
        if hasattr(app_or_ctx, "job_queue"):
            jq0 = getattr(app_or_ctx, "job_queue", None)
        if jq0 is None and hasattr(app_or_ctx, "application"):
            jq0 = getattr(app_or_ctx.application, "job_queue", None)
        if jq0 is not None:
            old_jobs = jq0.get_jobs_by_name(name) or []
            for j in old_jobs:
                try:
                    j.schedule_removal()
                except Exception:
    ...
    # ✅ fallback loop مع تخزين الـ task لإيقافه لاحقًا من زر stop_rebroadcast
    ...
    t = asyncio.create_task(_fallback_loop())
    active_rebroadcasts[name] = {"interval": int(interval_seconds), "payload": payload, "task": t}
    save_state()
...
async def send_stats_to_admin(
    ...
    """
    يرسل ملخص الإحصاءات بعد كل إعادة بث إلى:
    - مالك الحملة (owner_id) دائماً.
    - المنفّذ الفعلي للنشر (publisher_id) إذا كان مختلفاً عن المالك.
    لا يوجّه الإشعار لأي مشرفين آخرين أو لجميع حاملي التصاريح المؤقتة.
    """
    try:
        # احصل على bot من السياق أو التطبيق
        bot = getattr(app_or_ctx, "bot", None)
        if bot is None and hasattr(app_or_ctx, "application"):
            bot = app_or_ctx.application.bot
        if bot is None:
        ...
        # لا يوجد حملة؟ أرسل تنبيهًا خفيفًا
        if not campaign_id:
            try:
                await bot.send_message(chat_id=owner_id, text="📊 لا توجد حملة مرتبطة لعرض الإحصاءات.")
            except Exception:
                pass
            return

        # إجماليات الحملة
        cc = campaign_counters.get(campaign_id) or {}
        tot_like = int(cc.get("like", 0))
        tot_dislike = int(cc.get("dislike", 0))
        voters_count = len(cc.get("voters", {}) or {})

        # ===== تفصيل كل وجهة (per_chat_lines) =====
        per_chat_lines = []

        # المصدر الأساسي: campaign_base_msg (يدعم المفاتيح tuple و"camp|chat" و"camp::chat")
        used_cids = set()
        for k, bm in list(campaign_base_msg.items()):
            cid = None
            try:
                if isinstance(k, tuple) and len(k) >= 2 and int(k[0]) == campaign_id:
                    cid = int(k[1])
                elif isinstance(k, str):
                    ks = k.strip()
                    if "|" in ks:
                        a, b = ks.split("|", 1)
                        if a.strip().isdigit() and int(a) == campaign_id:
                            cid = int(b.strip())  # ← لا نحذف السالب
                    elif "::" in ks:
                        a, b = ks.split("::", 1)
                        if a.strip().isdigit() and int(a) == campaign_id:
                            cid = int(b.strip())  # ← لا نحذف السالب
            ...
            if cid is None or cid in used_cids:
                continue
            used_cids.add(cid)

            base_mid = campaign_base_msg.get((campaign_id, cid))
            if base_mid is None:
                base_mid = campaign_base_msg.get(f"{campaign_id}|{cid}") or campaign_base_msg.get(f"{campaign_id}::{cid}")

            rec = reactions_counters.get((cid, base_mid), {}) if base_mid is not None else {}
            like = int(rec.get("like", 0))
            dislike = int(rec.get("dislike", 0))
            title = known_chats.get(cid, {}).get("title", str(cid))
            per_chat_lines.append(f"• {title} — 👍 {like} | 👎 {dislike}")

        # fallback: campaign_messages إن لم نجد شيئًا
        if not per_chat_lines:
            pairs = campaign_messages.get(campaign_id, []) or []
            seen = set()
        ...
        if per_chat_lines:
            text += "\n" + "\n".join(per_chat_lines)
        elif (tot_like + tot_dislike) == 0:
            text += "\n(لا توجد تفاعلات حتى الآن)"
        else:
            text += "\n(لا توجد رسائل مسجّلة لعرض التفصيل لكل وجهة حتى الآن)"

        # أزرار سريعة
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("📊 عرض التقييم", callback_data=f"show_stats:{campaign_id}")],
            [InlineKeyboardButton("⏹️ إيقاف الإعادة", callback_data=f"stop_rebroadcast:{owner_id}:{campaign_id}")]
        ])

        # المستلمون: المالك دائمًا + المنفّذ (إن كان مختلفًا)
        ...
        # الإرسال
        for rid in list(recipients):
            try:
                await bot.send_message(chat_id=rid, text=text, reply_markup=kb)
            except Exception:
                pass

    except Exception:
        # لا تعطل الجدولة إن فشل الإرسال
        try:
            logger.exception("send_stats_to_admin failed")
        except Exception:
            pass
...
async def handle_campaign_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("show_stats:"):
        ...
        # إجماليات الحملة (حتى لو ما فيه رسائل pairs)
        cc = campaign_counters.get(campaign_id) or {}
        tot_like = int(cc.get("like", 0))
        tot_dislike = int(cc.get("dislike", 0))
        voters_count = len(cc.get("voters", {}) or {})

        # قراءة رسائل الحملة (إن وُجدت) وتجهيز التفصيل لكل وجهة
        pairs = campaign_messages.get(campaign_id, []) or []
        per_chat_lines = []
        if pairs:
            seen = set()
            for (cid, _mid) in pairs:
                ...
                rec = reactions_counters.get((cid, base_mid), {}) if base_mid is not None else {}
                title = known_chats.get(cid, {}).get("title", str(cid))
                per_chat_lines.append(
                    f"• {title} — 👍 {int(rec.get('like', 0))} | 👎 {int(rec.get('dislike', 0))}"
                )

        # نص نظيف بأسطر حقيقية
        text = (
            f"📊 إحصاءات المنشورات #{campaign_id}\n"
            f"——————————————\n"
            f"👍 الإجمالي: {tot_like}\n"
    ...
    if data.startswith("stop_rebroadcast:"):
        ...
        # ✅ أوقف fallback task إن كان موجود
        try:
            info = active_rebroadcasts.get(name) or {}
            t = info.get("task")
...
async def webhook_handler(secret: str, request: Request):
    ...
    # ✋ لا تُعالج أي تحديث قبل اكتمال تهيئة التطبيق
...
def panel_main_keyboard() -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [InlineKeyboardButton("💾 نسخ احتياطي الآن", callback_data="panel:backup")],  # ← كان panel:backup_now
        [InlineKeyboardButton(
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ====== الإحصاءات (اختيار مجموعة أولاً) ======
    if data == "panel:stats":
        # استخرج كل المجموعات التي لديها أي منشور مسجّل في campaign_base_msg
        chat_ids = set()
        for key in campaign_base_msg.keys():
            if isinstance(key, tuple) and len(key) >= 2:
                chat_ids.add(int(key[1]))
            elif isinstance(key, str):
                ss = key.strip()
                # دعم صيغ النسخ الاحتياطي: "camp|chat" أو "camp::chat"
                if "|" in ss:
                    try:
                        chat_ids.add(int(ss.split("|", 1)[1].strip()))
                    except Exception:
                        pass
                elif "::" in ss:
                    try:
        ...
        if not chat_ids:
            try:
                await _panel_replace(
                    query,
                    "لا توجد مجموعات تحتوي منشورات بعد.",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 القائمة الرئيسية", callback_data="panel:back")]])
                )
            except Exception:
                pass
            return
        ...
        for cid in sorted(chat_ids):
            title = known_chats.get(cid, {}).get("title", str(cid))
            rows.append([InlineKeyboardButton(f"👥 {title}", callback_data=f"panel:stats:chat:{cid}")])

        rows.append([InlineKeyboardButton("⬅️ رجوع", callback_data="panel:back")])
        try:
            await _panel_replace(query, "اختر مجموعة لعرض أحدث منشور وإحصاءاته:", reply_markup=InlineKeyboardMarkup(rows))
        except Exception:
            pass
        return

    if data.startswith("panel:stats:chat:"):
        try:
            chat_id = int(data.split(":")[3])
        except Exception:
            return

        # ابحث عن كل الحملات التي لها base_msg لهذه المجموعة
        camp_ids_for_chat = set()
        for key, base_mid in campaign_base_msg.items():
            try:
                if isinstance(key, tuple) and len(key) >= 2:
                    camp_id, cid = int(key[0]), int(key[1])
                elif isinstance(key, str):
                    ss = key.strip()
                    if "|" in ss:
                        camp_id, cid = map(lambda x: int(x.strip()), ss.split("|", 1))
                    elif "::" in ss:
                        camp_id, cid = map(lambda x: int(x.strip()), ss.split("::", 1))
                    else:
                        continue
                    ...
                    continue
                if cid == chat_id:
                    camp_ids_for_chat.add(camp_id)
            except Exception:
                continue

        if not camp_ids_for_chat:
            # احتياط: قد تكون الرسائل في campaign_messages فقط
            try:
                await _panel_replace(
                    query,
                    "لا توجد منشورات مسجّلة لهذه المجموعة.",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ رجوع للمجموعات", callback_data="panel:stats")]])
                )
            except Exception:
                pass
            return

        # اختر أحدث حملة (أكبر رقم)
        camp_id = max(camp_ids_for_chat)

        # احضر base_mid لهذه الحملة في هذه المجموعة
        base_mid = campaign_base_msg.get((camp_id, chat_id))
        if base_mid is None:
            # دعم صيغ النص من النسخ الاحتياطي
            base_mid = campaign_base_msg.get(f"{camp_id}|{chat_id}") or campaign_base_msg.get(f"{camp_id}::{chat_id}")

        # إجمالي الحملة (كل الوجهات) للمعلومة العامة
        cc = campaign_counters.get(camp_id) or {}
        tot_like = int(cc.get("like", 0))
        tot_dislike = int(cc.get("dislike", 0))
        voters_count = len(cc.get("voters", {}) or {})

        # الإحصاء الخاص بهذه المجموعة فقط
        rec = reactions_counters.get((chat_id, base_mid), {}) if base_mid is not None else {}
        like = int(rec.get("like", 0))
        dislike = int(rec.get("dislike", 0))

        title = known_chats.get(chat_id, {}).get("title", str(chat_id))
        text = (
            f"📊 أحدث منشور في {title}\n"
            f"منشور #{camp_id}\n"
            f"——————————————\n"
            f"👍 هذه المجموعة: {like}\n"
            f"👎 هذه المجموعة: {dislike}\n"
            f"———\n"
            f"📦 إجمالي الحملة #{camp_id}\n"
            f"👍 الإجمالي: {tot_like}\n"
            f"👎 الإجمالي: {tot_dislike}\n"
            f"👥 المصوّتون الفريدون: {voters_count}\n"
        )

        # أزرار بسيطة: (1) اختيار منشور آخر لنفس المجموعة (اختياري) (2) الجدولة الحالية (بدون منطق جديد)
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("📊 عرض التقييم الكامل", callback_data=f"show_stats:{camp_id}")],
            [InlineKeyboardButton("📦 اختيار منشور آخر لهذه المجموعة",
                                  callback_data=f"panel:stats:list_camps:{chat_id}")],
            [InlineKeyboardButton("⬅️ رجوع للمجموعات",
                                  callback_data="panel:stats")]
        ])

        try:
            await _panel_replace(query, text, reply_markup=kb)
        except Exception:
            pass
        return

    if data.startswith("panel:stats:list_camps:"):
        try:
            chat_id = int(data.split(":")[3])
        except Exception:
            return
        ...
        if not camp_ids_for_chat:
            try:
                await _panel_replace(
                    query,
                    "لا توجد منشورات أخرى لهذه المجموعة.",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ رجوع", callback_data=f"panel:stats:chat:{chat_id}")]])
                )
            except Exception:
                pass
            return
        ...
        title = known_chats.get(chat_id, {}).get("title", str(chat_id))
        rows = [[InlineKeyboardButton(f"📦 المنشور #{cid}", callback_data=f"panel:stats:camp:{cid}")]
                for cid in sorted(camp_ids_for_chat, reverse=True)[:10]]
        rows.append([InlineKeyboardButton("⬅️ رجوع", callback_data=f"panel:stats:chat:{chat_id}")])
        try:
            await _panel_replace(query, f"اختر منشورًا من {title}:", reply_markup=InlineKeyboardMarkup(rows))
        except Exception:
            pass
        return

    # ====== قائمة الإحصاءات: تفاصيل حملة محددة (حسب الوجهة) ======
    if data.startswith("panel:stats:camp:"):
        ...
        # جرّب الأزواج من campaign_messages
        pairs = list(campaign_messages.get(camp_id, []) or [])

        # إن لم توجد، استخرج الوجهات مباشرة من campaign_base_msg لنفس الحملة
        if not pairs:
            used = set()
            for k, bm in list(campaign_base_msg.items()):
                cid = None
                if isinstance(k, tuple) and len(k) >= 2 and int(k[0]) == camp_id:
                    cid = int(k[1])
                elif isinstance(k, str):
                    sss = k.strip()
                    if "|" in sss:
        ...
        if not pairs:
            try:
                await _panel_replace(
                    query,
                    "لا توجد رسائل مسجّلة لهذه الحملة.",
        ...
        for cid, _mid in pairs:
            ...
            if base_mid is None:
                base_mid = (campaign_base_msg.get(f"{camp_id}|{cid}")
                            or campaign_base_msg.get(f"{camp_id}::{cid}"))
    ...
    # ====== الوجهات (لوحة المسؤول) ======
    if data == "panel:destinations":
        # لائحة كل الوجهات المعروفة مع تفعيل/تعطيل
        disabled = s.setdefault("disabled_chats", set())
        rows = []

        # اعرض كل known_chats (أو صِفِّها حسب حاجتك)
        for cid in sorted(known_chats.keys()):
            info = known_chats.get(cid, {})
            title = info.get("title", str(cid))
            badge = "⛔" if cid in disabled else "✅"
            rows.append([InlineKeyboardButton(f"{badge} {title}", callback_data=f"panel:toggle_chat:{cid}")])

        rows.append([InlineKeyboardButton("💾 حفظ إعداد الوجهات", callback_data="panel:dest_save")])
        rows.append([InlineKeyboardButton("⬅️ رجوع", callback_data="panel:back")])

        try:
            await _panel_replace(query, "فعّل/عطّل الوجهات المطلوبة ثم احفظ:", reply_markup=InlineKeyboardMarkup(rows))
        except Exception:
            pass
        return
    ...
    if data.startswith("panel:toggle_chat:"):
        ...
        # أعد بناء نفس القائمة بسرعة
        rows = []
        for xcid in sorted(known_chats.keys()):
            info = known_chats.get(xcid, {})
            title = info.get("title", str(xcid))
            badge = "⛔" if xcid in disabled else "✅"
            rows.append([InlineKeyboardButton(f"{badge} {title}", callback_data=f"panel:toggle_chat:{xcid}")])
        rows.append([InlineKeyboardButton("💾 حفظ إعداد الوجهات", callback_data="panel:dest_save")])
        rows.append([InlineKeyboardButton("⬅️ رجوع", callback_data="panel:back")])
    ...
    # ====== الجدولة / الإعادة (لوحة المسؤول) ======
    if data == "panel:schedule":
        on = global_settings.get("scheduling_enabled", True)
        kb = InlineKeyboardMarkup([
        ...
        try:
            await _panel_replace(query, f"⏱️ الإعادة: *{'مفعّلة' if on else 'معطّلة'}*", reply_markup=kb, parse_mode="Markdown")
        except Exception:
            pass
        return
    ...
    if data == "panel:sched_toggle":
        gs = global_settings
        now_flag = not gs.get("scheduling_enabled", True)
        gs["scheduling_enabled"] = now_flag
        add_log(user_id, f"{'تشغيل' if now_flag else 'إيقاف'} خدمة الجدولة مركزيًا")
        save_state()
        ...
        try:
            await _panel_replace(query, f"✅ تم {'تشغيل' if now_flag else 'إيقاف'} خدمة الجدولة.\n\n⏱️ الإعادة: *{'مفعّلة' if on else 'معطّلة'}*",
    ...
    if data == "panel:reactions":
        gs = global_settings
        on = gs.get("reactions_feature_enabled", True)
        prompt_on = gs.get("reactions_prompt_enabled", True)

        rows = [
            [InlineKeyboardButton(
                f"{'⏹️ إيقاف' if on else '▶️ تشغيل'} التفاعلات",
                callback_data="panel:react_toggle"
            )],
            [InlineKeyboardButton(
                f"{'👁️‍🗨️ إخفاء' if prompt_on else '💬 إظهار'} رسالة الدعوة",
                callback_data="panel:reactions:prompt_toggle"
            )],
        ...
        text = (
            ...
            f"💬 رسالة الدعوة: *{'ظاهرة' if prompt_on else 'مخفية (ستظهر الأزرار تحت المنشور مباشرة)'}*"
        )
    ...
    if data == "panel:react_toggle":
        ...
        msg = (
            f"✅ تم {'تشغيل' if now else 'إيقاف'} خدمة التفاعلات مركزيًا.\n\n"
            f"👍 الحالة: *{'مفعّلة' if on else 'معطّلة'}*\n"
            f"💬 رسالة الدعوة: *{'ظاهرة' if prompt_on else 'مخفية'}*"
        )
    ...
    if data == "panel:reactions:prompt_toggle":
        gs = global_settings
        now = not gs.get("reactions_prompt_enabled", True)
        gs["reactions_prompt_enabled"] = now
        add_log(user_id, f"{'إظهار' if now else 'إخفاء'} رسالة الدعوة")
        save_state()
        ...
        msg = (
            "✅ تم إخفاء رسالة الدعوة.\n\n"
            "سيتم إرفاق أزرار التفاعل مباشرة أسفل المنشور الجديد."
            if not now else
            "✅ تم إظهار رسالة الدعوة من جديد."
        )
        msg += f"\n\n👍 حالة التفاعلات: *{'مفعّلة' if on else 'معطّلة'}*"
    ...
    if data == "panel:reactions:edit":
        ...
        try:
            await context.bot.send_message(
                chat_id=user_id,
                text="أرسل الآن نص دعوة التفاعل الجديد."
            )
        except Exception:
            pass
        return
    ...
    if data == "panel:backup":
        ...
        if now_ts - last < 10:
            try:
                ...
                pass
            return
        context.user_data["last_backup_click_ts"] = now_ts
        ...
        try:
            ...
            when = _dt.utcnow().strftime("%Y-%m-%d %H:%M:%SZ")
            if ok:
                lines = [
...
async def backup_to_tg(bot, *, reason: str = "manual") -> bool:
    ...
    يضيف snapshot_ts في الـ caption ويخزّن أيضًا file_id + TS في وصف المجموعة.
    """
    ...
    try:
        # ✅ فحص صلاحيات البوت قبل الإرسال (مفيد للتشخيص)
        try:
            me = await bot.get_me()
            member = await bot.get_chat_member(chat_id, me.id)
            can_pin = getattr(member, "can_pin_messages", None)
            logger.info("backup: bot status=%s, can_pin=%s", getattr(member, "status", None), can_pin)
        except Exception as e:
            logger.warning("backup: failed to check bot member status (%s)", e)

        snap = build_persist_snapshot()
        ts = int(snap.get("snapshot_ts") or time.time())
        ...
        bio.name = BACKUP_FILENAME  # نُبقي الاسم ثابتًا كما هو
        caption = f"backup|ts={ts}|utc={datetime.utcnow().isoformat()}Z|reason:{reason}"
        ...
        try:
            ...
            logger.warning("backup: pin failed (%s)", e)
        except Exception as e:
            logger.warning("backup: pin failed (unexpected %s)", e)

        # خزّن file_id + TS في وصف المجموعة لمسار بديل سريع
        try:
            desc = f"LAST_BACKUP_FILE_ID={msg.document.file_id}|TS={ts}"
            await bot.set_chat_description(chat_id, desc[:255])
        except TelegramError as e:
            logger.warning("backup: set_chat_description failed (%s)", e)
        except Exception as e:
            logger.warning("backup: set_chat_description failed (unexpected %s)", e)

        logger.info("backup: sent to %s (pinned=%s, ts=%s)", chat_id, pinned_ok, ts)
        # — تحديث مؤشرات آخر نسخ احتياطي وحفظ متزامن —
        try:
            ts = int(time.time())
            gs = globals().get("global_settings") or {}
            gs["last_backup_ts"] = ts
            gs["last_backup_votes"] = _current_total_votes()
            gs["last_snapshot_ts"] = ts
            globals()["global_settings"] = gs
            save_state(sync=True)
        except Exception:
            logger.exception("backup_to_tg: stamping globals/save_state failed")
        return True
    except Exception:
        logger.exception("backup_to_tg failed")
        return False

def _normalize_voters_keys_inplace():
    # حملات
    for rec in (campaign_counters or {}).values():
        voters = rec.get("voters")
        if isinstance(voters, dict):
            rec["voters"] = {int(str(k)): v for k, v in voters.items() if v in ("like", "dislike")}
    # رسائل مفردة
    for rec in (reactions_counters or {}).values():
        voters = rec.get("voters")
        if isinstance(voters, dict):
            rec["voters"] = {int(str(k)): v for k, v in voters.items() if v in ("like", "dislike")}
...
async def restore_state_from_tg(bot) -> bool:
    ...
    try:
        ...
        # — مقارنة حداثة اللقطة: تخطّ الأقدم —
        try:
            remote_ts = int((data or {}).get("snapshot_ts") or 0)
        except Exception:
            remote_ts = 0
        try:
            local_ts = int((globals().get("global_settings") or {}).get("last_snapshot_ts", 0))
        except Exception:
            local_ts = 0

        if local_ts and remote_ts and remote_ts < local_ts:
            logger.warning(
                "restore_state_from_tg: remote snapshot older than local (remote=%s < local=%s) → skip",
                remote_ts, local_ts
            )
            return False
        ...
        # ختم آخر لقطة وتخزينها محليًا
        try:
            gs = globals().get("global_settings") or {}
            gs["last_snapshot_ts"] = remote_ts or int(time.time())
            globals()["global_settings"] = gs
            save_state()
        except Exception:
            logger.exception("restore_state_from_tg: stamping last_snapshot_ts failed")

        # ✅ مزامنة الكيبوردات مباشرة بعد الاسترجاع
        try:
            await _resync_all_keyboards(bot)
            logger.info("restore: keyboards resynced.")
        except Exception:
            logger.exception("restore: resync failed")
...
async def _startup():
    ...
    # 8) جدولة نسخة احتياطية يومية خفيفة (بدون نفخ الكود)
    try:
        maybe = globals().get("_daily_backup_job")
        if callable(maybe):
            jq = application.job_queue
            # إزالة أي جدولة قديمة بنفس الاسم لمنع التكرار
            for j in jq.get_jobs_by_name("daily_backup"):
                j.schedule_removal()
            # تكرار كل 24 ساعة، وأول تشغيل بعد 10 دقائق من الإقلاع
            jq.run_repeating(maybe, interval=24 * 3600, first=600, name="daily_backup")
    except Exception:
        logger.exception("startup: schedule daily backup failed")

    # — جدولة نسخ احتياطي يومي 03:00 بتوقيت الرياض —
    try:
        import zoneinfo, datetime as _dt
        riyadh = zoneinfo.ZoneInfo("Asia/Riyadh")
        async def _daily_backup_job(ctx):
            try:
                fn = globals().get("backup_to_tg")
                if callable(fn):
                    await fn(ctx.bot, reason="daily")
            except Exception:
                logger.exception("daily backup failed")
        application.job_queue.run_daily(_daily_backup_job, time=_dt.time(3, 0, 0, tzinfo=riyadh), name="daily_backup_03KSA")
    except Exception:
        logger.exception("startup: schedule daily backup failed")
...
async def on_error(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if isinstance(err, RetryAfter):
        logger.warning("RetryAfter: sleeping %s", getattr(err, "retry_after", "?"))
        return
