...
async def send_post_one_chat(
    ...
    *,
    ...
    # لم نعد نعتمد على HTML؛ سنبني كيانات TEXT_LINK مباشرة
    ...
    if sess.single_attachment:
        ...
        c_txt, c_entities = build_text_and_entities(c_raw or "")
        if a_type == "document":
            m = await context.bot.send_document(chat_id=chat_id, document=file_id, caption=c_txt, caption_entities=c_entities)
        elif a_type == "audio":
            m = await context.bot.send_audio(chat_id=chat_id, audio=file_id, caption=c_txt, caption_entities=c_entities)
        elif a_type == "voice":
            m = await context.bot.send_voice(chat_id=chat_id, voice=file_id, caption=c_txt, caption_entities=c_entities)
        else:
            m = None
        if m:
    ...
    if sess.media_list:
        if len(sess.media_list) > 1:
            ...
            c_txt, c_entities = build_text_and_entities(c_raw or "")
            if t == "photo":
                m = await context.bot.send_photo(chat_id=chat_id, photo=fid, caption=c_txt, caption_entities=c_entities)
            else:
                m = await context.bot.send_video(chat_id=chat_id, video=fid, caption=c_txt, caption_entities=c_entities)
            first_message_id = first_message_id or (m.message_id if m else None)
            caption = None

    # --- نص فقط (تعطيل معاينة المواقع فقط) ---
    if caption:
        txt, ents = build_text_and_entities(caption)
        m = await context.bot.send_message(
            chat_id=chat_id,
            text=txt,
            entities=ents,
    ...
    # ===== باقي منظومتك كما هي (تثبيت/تفاعلات/حملات) =====
