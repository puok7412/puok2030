...
async def send_post_one_chat(
    ...
    *,
    ...
    if sess.single_attachment:
        ...
        # بما أننا استخدمنا النص كـ caption مع المرفق المفرد
        caption = None

    # --- وسائط متعددة (ألبوم صور / فيديو) ---
    if sess.media_list:
        if len(sess.media_list) > 1:
            # تيليجرام يسمح بحد أقصى 10 عناصر في كل media_group
            all_media = sess.media_list
            chunk_size = 10
            total = len(all_media)

            for start in range(0, total, chunk_size):
                chunk = all_media[start:start + chunk_size]
                media_group = []

                for idx, (t, fid, cap) in enumerate(chunk):
                    # أول عنصر في أول دفعة يأخذ نص المنشور كـ caption
                    ...
                    if t == "photo":
                        ...
                        # في حال وُضع نوع غير مدعوم بالخطأ في media_list نتجاهله
                        continue
                ...
                # نعتبر أول رسالة في أول دفعة هي الأساس للتفاعلات
                if first_message_id is None and msgs:
                    first_message_id = msgs[0].message_id

            # بعد إرسال الألبوم لا نريد إرسال نص مستقل مرة أخرى
            caption = None

        else:
            # صورة / فيديو مفرد من media_list
            t, fid, cap = sess.media_list[0]
            c = caption or cap
            c = hidden_links_or_plain(c, hide_links)
    ...
    # نستخدم الحملة دائمًا (حتى مع الألبوم) حتى تتزامن التفاعلات بين المجموعات
    effective_campaign_id = sess.campaign_id if (sess.campaign_id is not None) else None
    ...
    # --- أزرار تفاعل عريضة وكبيرة وجنب بعض ---
    # نستخدم مسافات داخل النص لتوسيع عرض الأزرار قدر الممكن
    pos_label = f"{pos_emo} {like_count}      "
    neg_label = f"{neg_emo} {dislike_count}      "
    ...
    # 🔹 تركيب أزرار التفاعل
    try:
        # المحاولة الأولى: تحت نفس الرسالة (نص / صورة واحدة / فيديو واحد / أول رسالة من الألبوم)
        await context.bot.edit_message_reply_markup(
            chat_id=chat_id,
        ...
        # في بعض الحالات (مثل ألبوم وسائط) تيليجرام لا يقبل تعديل الكيبورد على الرسالة نفسها
        # هنا نرسل رسالة جديدة أسفل المنشور تحتوي فقط على الأزرار
        try:
            await context.bot.send_message(
                chat_id=chat_id,
                text="\u2060",  # حرف غير مرئي لكن يمنع خطأ "message text is empty"
                reply_markup=kb,
            )
        except Exception:
            pass
