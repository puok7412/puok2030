...
try:
    _PERSIST_BUCKETS  # موجودة أصلًا
    if "campaign_styles" not in _PERSIST_BUCKETS:
        _PERSIST_BUCKETS.append("campaign_styles")
except NameError:
    _PERSIST_BUCKETS = ["campaign_styles"]  # احتياط لو لم تكن معرّفة (نادرًا)

campaign_styles: Dict[int, str] = globals().get("campaign_styles", {})
globals()["campaign_styles"] = campaign_styles  # تأكيد توفّرها عالميًا
...
def status_text(sess: Session) -> str:
    ...
    lines.append(f"*📦 الحملة*: ID `{campaign_id if campaign_id is not None else '—'}` • حفظ تلقائي {check}")
...
def build_next_hint(sess: Session, saved_type: str) -> str:
    ...
    return (
        f"*إضافة متاحة*: {' • '.join(can_add)} — اضغط *تم* للخيارات."
        if can_add else
...
async def rebroadcast_job(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # (اختياري) مزامنة لوحات التفاعل في جميع الرسائل التابعة للحملة بنفس الإيموجي المختار
    try:
        if tmp.campaign_id:
