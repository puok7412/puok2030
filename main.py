...
async def send_preview(update: Update, context: ContextTypes.DEFAULT_TYPE, sess: Session, *, hide_links: bool):
    # === يحوّل أي URL إلى <a> ويحدد parse_mode تلقائيًا، ويحترم النص HTML الجاهز ===
    def _auto_htmlize_and_mode(txt: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
        if not txt:
            return txt, None
        # لو النص أصلاً HTML (جاء من entities عبر واجهة تيليجرام)
        if '<a href="' in txt:
            return txt, "HTML"
        # لو فيه أي URL صريح
        if URL_RE.search(txt or ""):
            return make_html_with_hidden_links(txt), "HTML"
        # خيار الإخفاء القسري
        if hide_links:
            return make_html_with_hidden_links(txt), "HTML"
        return txt, None
    ...
    if getattr(sess, "single_attachment", None):
        ...
        c, c_mode = _auto_htmlize_and_mode(c_raw)
        if a_type == "document":
            await context.bot.send_document(chat_id=user_id, document=file_id, caption=c, parse_mode=c_mode)
        elif a_type == "audio":
            await context.bot.send_audio(chat_id=user_id, audio=file_id, caption=c, parse_mode=c_mode)
        elif a_type == "voice":
            await context.bot.send_voice(chat_id=user_id, voice=file_id, caption=c, parse_mode=c_mode)
        ...
        if len(sess.media_list) > 1:
            ...
            for idx, (t, fid, cap) in enumerate(sess.media_list):
                ...
                if c_raw:
                    ...
                    c, c_mode = None, None
                if t == "photo":
                    media_group.append(InputMediaPhoto(media=fid, caption=c, parse_mode=c_mode))
                else:
                    media_group.append(InputMediaVideo(media=fid, caption=c, parse_mode=c_mode))
            ...
            c, c_mode = _auto_htmlize_and_mode(c_raw)
            if t == "photo":
                await context.bot.send_photo(chat_id=user_id, photo=fid, caption=c, parse_mode=c_mode)
            else:
                await context.bot.send_video(chat_id=user_id, video=fid, caption=c, parse_mode=c_mode)
        m = await context.bot.send_message(chat_id=user_id, text=status_block, reply_markup=action_kb)
        sess.preview_action_msg_id = m.message_id
        save_state()
        return
    ...
    if getattr(sess, "text", None):
        txt, pmode = _auto_htmlize_and_mode(sess.text)
        await context.bot.send_message(
            chat_id=user_id,
            text=txt,
            parse_mode=pmode,
...
async def send_post_one_chat(
    ...
    *,
    ...
    # يحوّل أي URL إلى <a> عند الحاجة ويُرجِع دومًا ParseMode.HTML للكابتشن/النص
    def _auto_htmlize_and_mode(txt: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
        if not txt:
            return txt, None
        t = txt
        # لو النص يحتوي <a href> جاهزة نرسلها كما هي
        if '<a href="' in t:
            return t, ParseMode.HTML
        # لو مطلوب إخفاء الروابط أو وُجد URL خام → حوّله إلى <a>
        if hide_links or URL_RE.search(t or ""):
            return make_html_with_hidden_links(t), ParseMode.HTML
        # لا روابط: نهرب أي أحرف قد تكسر HTML ونرسل كـ HTML نظيف
        return _html.escape(t, quote=False), ParseMode.HTML
    ...
    if sess.single_attachment:
        ...
        c, c_mode = _auto_htmlize_and_mode(c_raw)
        if a_type == "document":
            m = await context.bot.send_document(chat_id=chat_id, document=file_id, caption=c, parse_mode=c_mode)
        elif a_type == "audio":
            m = await context.bot.send_audio(chat_id=chat_id, audio=file_id, caption=c, parse_mode=c_mode)
        elif a_type == "voice":
            m = await context.bot.send_voice(chat_id=chat_id, voice=file_id, caption=c, parse_mode=c_mode)
        else:
            m = None
        if m:
    ...
    if sess.media_list:
        if len(sess.media_list) > 1:
            ...
            c, c_mode = _auto_htmlize_and_mode(c_raw)
            if t == "photo":
                m = await context.bot.send_photo(chat_id=chat_id, photo=fid, caption=c, parse_mode=c_mode)
            else:
                m = await context.bot.send_video(chat_id=chat_id, video=fid, caption=c, parse_mode=c_mode)
            first_message_id = first_message_id or (m.message_id if m else None)
            caption = None

    # --- نص فقط (تعطيل معاينة المواقع فقط) ---
    if caption:
        txt, pmode = _auto_htmlize_and_mode(caption)
        m = await context.bot.send_message(
            chat_id=chat_id,
            text=txt,
            parse_mode=pmode,                 # HTML دائمًا إن وُجد نص
            disable_web_page_preview=True     # يلغي الكرت فقط، الرابط يظل قابلًا للنقر
        )
