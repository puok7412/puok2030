...
def keyboard_ready_options(sess: "Session", settings: Dict[str, Any]) -> InlineKeyboardMarkup:
    ...
    if settings.get("default_reactions_enabled", True):
        # عرض نمط التفاعلات الحالي بدل "إضافة/إزالة"
        cur = getattr(sess, "reactions_style", None)
        if not cur:
            # محاولة استرداده من تفضيلات المشرف (آخر اختيار)
            cur = get_settings(settings.get("owner_id", 0)).get("last_reactions_style", "thumbs") if hasattr(settings, "get") else "thumbs"
        pos, neg = _get_reaction_pair(cur)
...
async def start_publishing_session(user, context: ContextTypes.DEFAULT_TYPE):
    ...
    # رسالة ترحيب مختصرة وفاخرة — بدون نصائح مطوّلة
    welcome = (
        f"👋 *مرحبًا* _{name}_\n"
        f"{sep}"
        f"*وضع النشر*\n"
...
async def start_with_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # دالة بسيطة لتأمين Markdown للأسماء
    def _md_escape(s: str) -> str:
        return s.replace("\\", "\\\\").replace("_", "\\_").replace("*", "\\*").replace("`", "\\`").replace("[", "\\[").replace("]", "\\]")

    if not args:
        # رسالة قصيرة غير مزعجة
        msg = (
    ...
    # رسالة قصيرة: تم منحك تصريحًا مؤقتًا مع وقت الانتهاء
    msg_ok = (
    ...
    # (اختياري) منع إعادة استخدام الرابط بعد بدء الجلسة
    start_tokens.pop(token, None)
    save_state()
...
async def handle_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ألبوم (صور/فيديو) ضمن media_group
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تبديل التفاعلات (متروك للتوافق – ما زال يعمل بجانب قائمة الاختيار)
    ...
    if data == "publish":
        ...
        # حفظ نمط التفاعلات للحملة (إن وُجدت البنية)
        try:
...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    # استرجاع نمط الحملة
    style = campaign_styles.get(campaign_id, "thumbs")
    pos_emo, neg_emo = _get_reaction_pair(style)
...
async def send_post_one_chat(
    ...
    *,
    ...
    if getattr(sess, "use_reactions", True):
        ...
        # عدّ موحّد للحملة إن وُجدت
        if sess.campaign_id is not None:
            cc = campaign_counters.setdefault(sess.campaign_id, {"like": 0, "dislike": 0, "voters": {}})
            like_count = cc["like"]
            dislike_count = cc["dislike"]

        # 👇 اختيار الإيموجي حسب النمط
        try:
            style = getattr(sess, "reactions_style", None)
            if not style and sess.campaign_id is not None:
                style = campaign_styles.get(sess.campaign_id, "thumbs")
            pos_emo, neg_emo = _get_reaction_pair(style or "thumbs")
        except Exception:
            pos_emo, neg_emo = "👍", "👎"
...
async def rebroadcast_job(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # إشعار تقدّم الإعادة — رسالة واحدة تُحدَّث كل مرة
    try:
        done = data["total"] - data["left"] + 1
        progress = data.setdefault("progress_msgs", {})  # {chat_id: message_id}

        for cid in data["chosen_chats"]:
            text = f"🔁 إعادة النشر {done}/{data['total']}"
            # محاولة ربط رسالة التقدّم بالمنشور الأساسي (إن وُجد)
            base_id = None
            try:
                if tmp.campaign_id:
                    base_id = campaign_base_msg.get((tmp.campaign_id, cid))
            except Exception:
                pass
            ...
            try:
                if cid in progress:
                    # تحديث نفس الرسالة
                    await ctx.bot.edit_message_text(chat_id=cid, message_id=progress[cid], text=text)
                else:
                    # إرسال أول مرة ثم حفظ الـ message_id
                    if base_id:
                        m = await ctx.bot.send_message(chat_id=cid, text=text, reply_to_message_id=base_id)
                    else:
                        m = await ctx.bot.send_message(chat_id=cid, text=text)
                    progress[cid] = m.message_id
            except Exception:
                pass
    except Exception:
        pass
