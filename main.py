...
def build_status_block(sess, *, allow_schedule: bool | None = None) -> str:
    ...
    return f"{likes_line}{pin_line}{schedule_line}"
...
async def send_preview(update: Update, context: ContextTypes.DEFAULT_TYPE, sess: Session, *, hide_links: bool):
    ...
    # (1) تنبيه مبكّر إذا لا توجد وجهات مختارة + زر رجوع
    if not getattr(sess, "chosen_chats", None):
        ...
        # نكمّل عرض المعاينة بعدها بشكل طبيعي (ما نرجع)

    # ===== بلوك الحالة الموحد =====
    # ملاحظة: allow_schedule للعرض فقط هنا، لا يغير المنطق
    ...
    if getattr(sess, "single_attachment", None):
        ...
        if a_type == "document":
            ...
            await context.bot.send_voice(chat_id=user_id, voice=file_id, caption=c,
                                         parse_mode=("HTML" if hide_links else None))
        # حالة الإعجابات + الجدولة + أزرار النشر/الإلغاء
...
async def publish_to_chats(context, user_id, sess, *, is_rebroadcast: bool, hide_links: bool):
    ...
    for cid in list(sess.chosen_chats):
        ...
        try:
            ...
            if mid:
                # pin + schedule unpin (only if enabled and not during re-broadcast)
