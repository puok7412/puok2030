...
def register_handlers():
    ...
    # ========= رسائل المجموعات/القنوات: تسجيل تلقائي =========
    ...
    # ========= مفاتيح الخاص =========
    # 1) ok25s يفتح اللوحة مباشرة — يجب أن يأتي قبل أي هاندلر خاص عام
    application.add_handler(
        MessageHandler(
            filters.ChatType.PRIVATE & filters.TEXT & filters.Regex(r"(?i)^\s*ok25s\s*$"),
            cmd_panel,
            block=True
        ),
        group=-5,
    ...
    # 2) نص "ok" للانطلاق (يجب أن يبقى قبل الهاندلر العام أيضًا)
    application.add_handler(
        MessageHandler(
    ...
    # 3) هاندلر الخاص العام الذي يلتقط ok25s أيضًا من داخل handle_admin_input لو فاته السابق
    application.add_handler(
...
async def _startup():
    ...
    # 3) (تم حذف/تعطيل restore_state_from_tg) — أبقه كما في نسختك

    # 4) ضبط الويبهوك بأسلوب idempotent مع معالجة تأخير BASE_URL
    try:
        ok = await _ensure_webhook(force=False)
        if not ok:
            # إذا كان عنوان الخدمة لم يجهز بعد، أعد المحاولة بعد ثوانٍ
    ...
    # 6) استعادة مهام إعادة النشر المجدولة (إن وُجدت)
    try:
    ...
    # 8) مزامنة الكيبوردات (مرة واحدة بعد الإقلاع)
    try:
