...
async def handle_reactions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if campaign_id is not None:
        ...
        # --- خريطة رسائل الأساس لكل حملة/مجموعة ---
        ...
        like_label = f"{pos_emo} {like_cnt} | إعجاب"
        dislike_label = f"{neg_emo} {dislike_cnt} | غير مناسب"
...
async def send_post_one_chat(
    ...
    *,
    ...
    # ✅ مهم: نستخدم campaign_id حتى لو كان المنشور ألبوم
    # حتى تبقى العدادات موحّدة بين كل المجموعات
    ...
    like_label = f"{pos_emo} {like_count} | إعجاب"
    dislike_label = f"{neg_emo} {dislike_count} | غير مناسب"
    ...
    # 🔹 تركيب أزرار التفاعل
    try:
        # المحاولة الأولى: تحت نفس الرسالة (نص / صورة واحدة / فيديو واحد)
        await context.bot.edit_message_reply_markup(
            chat_id=chat_id,
        ...
        try:
            await context.bot.send_message(
                chat_id=chat_id,
                text="\u2060",  # حرف غير مرئي يمنع خطأ "message text is empty"
                reply_markup=kb,
            )
        except Exception:
            pass
