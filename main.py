...
# ثبّت السر من البيئة، ولا تولّده عشوائياً هنا
WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET", "MyStrongSecretKey194525194525")

# استخدم RENDER_EXTERNAL_URL أو بديله PUBLIC_URL
...
def status_text(sess: Session) -> str:
    ...
    lines.append(f"*📝 المحتوى*: نص {check if has_text else cross} • وسائط {len(media_list)} • مرفق {attach_map.get(single_att[0], single_att[0]) if single_att else cross}")
    ...
    lines.append(f"*⚙️ الخيارات*: تفاعلات {check if use_reacts else cross} • تثبيت {'📌' if pin_enabled else cross} • وجهات {chosen_cnt}")

    # ⏱️ الجدولة
    schedule_active = bool(getattr(sess, "schedule_active", False))
    ...
    lines.append(f"*⏱️ الجدولة*: {'مفعّلة' if schedule_active else 'غير مفعّلة'}" + (f" • كل {hrs} ساعة × {total}" if schedule_active else ""))

    # 📦 الحملة
    campaign_id = getattr(sess, "campaign_id", None)
    id_part = f"ID `{campaign_id}` • " if campaign_id is not None else ""
    lines.append(f"*📦 المنشور*: {id_part}حفظ تلقائي {check}")
...
async def cmd_temp_ok(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    group_text = (
        f"**تم منحك تصريح نشر مؤقّت**\n"
        f"- 👤 المستفيد: {md_user_link}\n"
    ...
    try:
        dm_text = (
            ...
            f"تم منحك تصريح نشر مؤقّت في: *{md_chat_title}*\n"
            f"⏳ ينتهي: {ksa_time(expires)}\n\n"
...
async def handle_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.text:
        ...
        if raw == "ok":
            return  # يمر إلى start_publishing_keyword عبر الهاندلر المخصص
        if raw == "ok25s":
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ====== قائمة اختيار نمط التفاعلات (جديدة) ======
    ...
    if data == "choose_chats":
        ...
        active_ids = [cid for cid in admin_chat_ids if cid not in s_user["disabled_chats"]]
        if not active_ids:
    ...
    if data.startswith("toggle_chat:"):
        ...
        s_user = get_settings(user_id)
        active_ids = [i for i in source_ids if i not in s_user["disabled_chats"]]
    ...
    if data == "select_all":
        ...
        s_user = get_settings(user_id)
        sess.chosen_chats = set(i for i in source_ids if i not in s_user["disabled_chats"])
    ...
    if data == "publish":
        ...
        # ⛳ احترام الضبط المركزي + تفضيل المشرف لتفعيل التفاعلات
        s_user = get_settings(user_id)
        sess.use_reactions = (
            bool(sess.use_reactions)
            and bool(global_settings.get("reactions_feature_enabled", True))
            and bool(s_user.get("default_reactions_enabled", True))
        )
        ...
        asyncio.create_task(_publish_and_report(context, user_id, sess, allow_schedule, s_user.get("hide_links_default", False)))
...
async def send_post_one_chat(
    ...
    *,
    ...
    # توحيد الرسالة الأساس للأزرار ضمن الحملة
    ...
    # --- التفاعلات ---
    if getattr(sess, "use_reactions", True):
        # عدّادات للرسالة/الحملة
        ...
        # 👇 اختيار/تثبيت نمط الإيموجي (ويُحفظ ليستمر بعد إعادة التشغيل)
        try:
            style = getattr(sess, "reactions_style", None)

            if sess.campaign_id is not None:
                # نمط الحملة: إن لم يكن محفوظًا مسبقًا خزّنه الآن
                if style is None:
                    style = campaign_styles.get(sess.campaign_id, "thumbs")
                campaign_styles[sess.campaign_id] = style
            else:
                # رسالة مفردة: خزّن النمط على مفتاح (chat_id, base_id_for_buttons)
                if style is None:
                    # إن وجد سابقًا استخدمه، وإلا الافتراضي
        ...
        # ✋ تجنّب إرسال رسالة دعوة جديدة عند "إعادة النشر" لحملة لديها دعوة سابقة
        send_prompt = True
        if is_rebroadcast and sess.campaign_id is not None:
            lst = campaign_prompt_msgs.get(sess.campaign_id, [])
            if any(c == chat_id for (c, _mid) in lst):
                send_prompt = False
        ...
        if send_prompt:
            ...
            # يوجد دعوة مُسبقة: حدّث الكيبوردات عبر مزامنة الحملة (إن أمكن)
            try:
                if sess.campaign_id is not None:
                    await _sync_campaign_keyboards(context, sess.campaign_id)
            except Exception:
                pass
...
async def rebroadcast_job(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # (اختياري) مزامنة لوحات التفاعل في جميع رسائل الحملة
    try:
        if tmp.campaign_id:
    ...
    # إشعار تقدّم الإعادة — رسالة جديدة في كل مرة (بدون اقتباس)
    try:
        done = data["total"] - data["left"] + 1
        text = f"🔁 إعادة النشر {done}/{data['total']}"
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel:stats":
        ...
        if not ids:
            try:
                await query.message.reply_text("لا توجد حملات بعد.")
            except Exception:
                pass
            return
        ...
        # أزرار اختيار حملة
        rows = [
            [InlineKeyboardButton(f"📦 الحملة #{cid}", callback_data=f"panel:stats:camp:{cid}")]
            for cid in last_ids
        ]
        rows.append([InlineKeyboardButton("⬅️ رجوع", callback_data="panel:back")])
        try:
    ...
    if data.startswith("panel:stats:camp:"):
        # تفاصيل حملة واحدة — إظهار الإحصاءات لكل وجهة
        try:
        ...
        if not pairs:
            try:
                await query.message.reply_text("لا توجد رسائل مسجّلة لهذه الحملة.")
            except Exception:
                pass
            return
        ...
        for cid, _mid in pairs:
            ...
            if not base_mid:
                # قد لا يكون هناك تعيين أساس، نعرض 0/0 بدلًا من التجاهل
                like = dislike = 0
            else:
                rec = reactions_counters.get((cid, base_mid), {"like": 0, "dislike": 0})
                like = rec.get("like", 0)
        ...
        # أزرار رجوع للقائمة السابقة/الرئيسية
        try:
            await query.message.reply_text("\n".join(lines),
    ...
    # ====== قائمة الإعادة (تشغيل/إيقاف + إيقاف الجاري + اختيار وجهة للإيقاف) ======
    if data == "panel:schedule":
        gs = global_settings
        toggled = "off" if gs.get("scheduling_enabled", True) else "on"
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(
                f"{'✅ إلغاء التفعيل' if toggled=='off' else '▶️ تفعيل'}",
                callback_data=f"panel:set:schedule:{toggled}"
            )],
    ...
    if data == "panel:sched_toggle":
        ...
        save_state()
        try:
            await query.message.reply_text(f"✅ تم {'تشغيل' if now else 'إيقاف'} خدمة الجدولة.")
        except Exception:
            pass
        return

    if data == "panel:sched_stop_menu":
        # اجمع كل الوجهات الموجودة في الـjobs الحالية
        uniq_cids = set()
        try:
        ...
        rows.append([InlineKeyboardButton("⬅️ رجوع", callback_data="panel:back")])
        try:
            await query.message.reply_text("اختر وجهة لإيقاف الإعادات الجارية:", reply_markup=InlineKeyboardMarkup(rows))
        except Exception:
            pass
        return
    ...
    if data.startswith("panel:sched_stop:"):
        ...
        try:
            await query.message.reply_text(
                "⛔ لا توجد إعادات لإيقافها." if removed == 0 else f"⛔ تم إيقاف {removed} مهمة إعادة."
            )
        except Exception:
            pass
        return
    ...
    if data == "panel:react_toggle":
        ...
        save_state()
        try:
            await query.message.reply_text(f"✅ تم {'تشغيل' if now else 'إيقاف'} التفاعلات.")
        except Exception:
            pass
        return
    ...
    if data == "panel:pin":
        ...
        rows = [
            [InlineKeyboardButton(
                f"{'⏹️ إيقاف' if on else '▶️ تشغيل'} التثبيت",
                callback_data="panel:pin_toggle"
            )],
            [InlineKeyboardButton("⬅️ رجوع", callback_data="panel:back")],
        ]
        try:
    ...
    if data == "panel:pin_toggle":
        ...
        save_state()
        try:
            await query.message.reply_text(f"✅ تم {'تشغيل' if now else 'إيقاف'} التثبيت.")
        except Exception:
            pass
        return
    ...
    if data.startswith("perm:save:"):
        ...
        try:
            await query.message.reply_text(f"💾 تم حفظ أذونات المشرفين للوجهة: {title}")
        except Exception:
            pass
        return
    ...
    if data == "panel:maintenance":
        ...
        rows = [
            [InlineKeyboardButton(
                f"{'⏹️ إيقاف' if on else '▶️ تشغيل'} وضع الصيانة",
                callback_data="panel:maint_toggle"
            )],
            [InlineKeyboardButton("⬅️ رجوع", callback_data="panel:back")],
        ]
        try:
            await query.message.reply_text(
                f"🛠️ حالة الوضع: *{'مفعل' if on else 'معطل'}*",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup(rows)
            )
        except Exception:
    ...
    if data == "panel:maint_toggle":
        ...
        save_state()
        try:
            await query.message.reply_text(f"تم {'تفعيل' if on else 'إيقاف'} وضع الصيانة.")
        except Exception:
            pass
        return
...
# 3) لوحة التحكّم panel:/perm: — لا تحجب الباقي
    ...
    application.add_handler(CallbackQueryHandler(on_button), group=99)
