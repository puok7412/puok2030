...
async def handle_reactions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    data = query.data  # الشكل: like:chat_id:message_id | dislike:chat_id:message_id

    # أجب فورًا لمنع خطأ "Query is too old..."
    try:
        await query.answer()
    except BadRequest:
        return
    ...
    # خزّن/اجلب السجل لهذا المنشور (أنشئه إن لم يكن موجودًا)
    ...
    # منع تكرار التصويت لنفس المستخدم
    if user.id in voters:
        return  # ممكن تغيّرها لإظهار تنبيه "صوّتت سابقًا"
    ...
    # رسالة شكر مهذّبة مع اقتباس للمنشور الأصلي (تُعيد إظهاره آخر الشاشة)
    try:
