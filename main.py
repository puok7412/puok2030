...
def get_settings(user_id: int) -> Dict[str, Any]:
    ...
    if not s:
        s = {
            ...
            "reaction_prompt_text": "💬 رأيكم يهمّنا! فضلاً قيّموا هذا المنشور:",
...
async def start_with_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🎯 ملاحظة: إذن النشر مؤقت ومقيد بهذه المجموعة فقط."
    )

    # ثم تفعيل الجلسة وإظهار الترحيب
    await start_publishing_session(user, context)

    # لا نحذف التوكن هنا: نسمح له بالبقاء حتى يتم النشر أو انتهاء المهلة
...
async def _schedule_unpin(context: ContextTypes.DEFAULT_TYPE, chat_id: int, message_id: int, delay_seconds: int):
    async def _unpin_inner(ctx: ContextTypes.DEFAULT_TYPE):
        try:
    ...
    context.application.job_queue.run_once(
        lambda c: context.application.create_task(_unpin_inner(c)),
        when=delay_seconds,
        name=f"unpin_{chat_id}_{message_id}"
    )
...
# جدولة إعادة البث
# =============================
async def schedule_rebroadcast(application, user_id: int, sess: Session, interval_seconds: int = 60, total_times: int = 12):
    payload = {
        "text": sess.text,
        "media_list": sess.media_list[:],
        "single_attachment": sess.single_attachment,
...
def main():
    ...
    # استقبال نصوص لوحة التحكم
    application.add_handler(
        MessageHandler(filters.ChatType.PRIVATE & filters.TEXT, handle_panel_text),
        group=0
    )

    # ok في الخاص — الآن محجوب إلا بتصريح فعّال
    application.add_handler(
        MessageHandler(
            filters.ChatType.PRIVATE & filters.Regex(r"(?i)^ok$"),
            start_publishing_keyword
        ),
        group=1
    )

    # إدخال المحتوى في الخاص
    content_filter = (
        filters.ChatType.PRIVATE
        & ~filters.COMMAND                # ⬅️ جديد: تجاهل /start وأي أوامر
        & (
    ...
    application.add_handler(MessageHandler(content_filter, handle_admin_input), group=2)
...
# مسارات أساسية: الصفحة الرئيسية + هيلث + UptimeRobot
...
@app.get("/uptime")
async def uptime():
    # استخدم هذا الرابط في UptimeRobot: https://<render-app>.onrender.com/uptime
    return {"ok": True, "ts": datetime.utcnow().isoformat()}

# Webhook بمسار سري (لا تشارك WEBHOOK_SECRET)
ALLOWED_UPDATES = None  # أو ["message","callback_query", ...] لو حبيت
...
async def on_startup():
    ...
    # 2) حذف أي Webhook قديم (احتياطي)
    try:
        await application.bot.delete_webhook(drop_pending_updates=True)
    except Exception:
        pass

    # 3) ضبط الويبهوك إذا توفّر عنوان Render الخارجي
    if not BASE_URL:
        logger.warning("RENDER_EXTERNAL_URL غير متاح بعد — سيتم ضبط الويبهوك عند توفره. أعد النشر لو لزم.")
        return
    ...
    # 4) (اختياري) جدولة مهام متكررة كل 5 دقائق مثل تنظيف جلسات أو أي عمل دوري
    if application.job_queue:
        # مثال: مهمة كل 5 دقائق
        # application.job_queue.run_repeating(some_job_fn, interval=5*60, first=5*60)
        pass
