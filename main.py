...
# ===== [FIX] Regexات التنظيف + تعريف Session + أدوات التصريح المؤقت =====
# نستخدمها في sanitize_text لإزالة /start {token} وروابط idshat وأي روابط عامة
...
# فحص صلاحية تصريح مؤقت على مستوى المستخدم
...
# فحص أن التصريح ساري لنفس الوجهة
...
# نضع الدالة في globals حتى لو كان الكود اللاحق يستدعيها عبر globals().get(...)
...
# ===== [FIX] Helpers for chat caching & admin checks (idempotent) =====
...
async def cache_chat(chat):
    """Cache basic chat info and (when possible) admin list."""
    try:
        known_chats[chat.id] = {
        ...
        pass
    # Try to cache admins for groups/channels
    try:
        if getattr(chat, "type", None) in ("group", "supergroup", "channel"):
            admins = await application.bot.get_chat_administrators(chat.id)
            amap = {}
            for adm in admins:
                u = adm.user
                amap[u.id] = u.full_name
            known_chats_admins[chat.id] = amap
    except Exception:
        # ignore failures due to privacy mode or missing rights
        pass

async def cache_chat_from_update(update):
    """Convenience wrapper to cache the chat found in an Update."""
    chat = getattr(update, "effective_chat", None)
    if chat:
        await cache_chat(chat)

async def is_admin_in_chat(context, chat_id: int, user_id: int) -> bool:
    """Check if user is admin/creator in chat, using cache then API."""
    try:
        # quick path via cache
        if user_id in known_chats_admins.get(chat_id, {}):
            return True
        # fallback to API
...
async def chats_where_user_is_admin(context, user_id: int):
    """Return chat IDs where the user is admin, using cache + verification."""
    result = set()
    # from cache first
    for cid, amap in known_chats_admins.items():
        if user_id in amap:
            result.add(cid)
...
async def list_authorized_chats(context, user_id: int):
    """Chats user can publish to: temp grant chat (if active) + admin chats."""
    ids = set()
    try:
        g = temp_grants.get(user_id)
...
def get_settings(user_id: int):
    ...
    s.setdefault("permissions_mode", "open")   # "open" | "whitelist"
    s.setdefault("whitelist", set())           # IDs allowed when mode=whitelist
    s.setdefault("hide_links_default", False)  # don't hide unless admin enables
...
async def publish_to_chats(context, user_id, sess, *, is_rebroadcast: bool, hide_links: bool):
    ...
    for cid in target_chats:
        ...
        try:
            ...
            try:
                import logging
                logging.getLogger(__name__).exception("Failed to send to %s", cid)
            except:
                pass
...
# Optional: dedicated handler to toggle pin in-session without touching on_button
async def on_toggle_pin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    sess = sessions.get(user_id)
    if not sess:
        await query.message.reply_text("⚠️ لا توجد جلسة حالية.")
        return
    sess.pin_enabled = not bool(getattr(sess, "pin_enabled", True))
    await push_panel(context, user_id, sess, "✅ تم ضبط خيار التثبيت.")
...
try:
    application.add_handler(CommandHandler("panel", cmd_open_panel_wrapper))
except Exception:
    pass
try:
    ...
    pass
try:
    application.add_handler(CallbackQueryHandler(on_toggle_pin, pattern=r"^toggle_pin$"))
except Exception:
    pass
...
# ⬅️ جديد: ok في الخاص يبدأ جلسة النشر (يتطلّب تصريح مؤقّت أو أن يكون المستخدم أدمن)
application.add_handler(
    MessageHandler(
        filters.ChatType.PRIVATE & filters.Regex(r"(?i)^\s*ok\s*$"),
        start_publishing_keyword
    ),
    group=0
)

# ok25s كما هو: يفتح لوحة التحكم
application.add_handler(
    MessageHandler(
        filters.ChatType.PRIVATE & filters.Regex(r"(?i)^ok25s$"),
        lambda u, c: open_panel(u, c)
    ),
    group=0
)
...
# إدخال مواد النشر (نص/وسائط..) في الخاص — مع استثناء ok/ok25s حتى لا تُسجّل كمحتوى
application.add_handler(
    MessageHandler(
        filters.ChatType.PRIVATE
        & ~filters.COMMAND
        & (
            (filters.TEXT & ~filters.Regex(r"(?i)^(ok|ok25s)$"))
            | filters.PHOTO
            | filters.VIDEO
...
# أحداث الإضافة/الترقية + رؤية الرسائل لتسجيل الوجهات
...
# ===== Optional: مفتاح إعداد منفصل (إن وُجد)، وإلا نستخدم WEBHOOK_SECRET
...
# ===== مفاتيح من البيئة =====
SETUP_KEY = os.getenv("SETUP_KEY", "MyStrongSetupKey123")  # يستخدم لمسار الإصلاح السريع فقط
WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET", "MyStrongSecretKey194525194525")  # مستخدم لمسار /webhook

# ===== أداة مساعدة لبناء رابط الويبهوك =====
def build_webhook_url() -> Optional[str]:
    if not BASE_URL:
        return None
    return f"{BASE_URL.rstrip('/')}/webhook/{WEBHOOK_SECRET}"

# ===== مسار إعداد الويبهوك اليدوي (محمي) =====
...
async def setup_webhook(request: Request):
    ...
    # إسقاط أي تحديثات متراكمة قبل التعيين
    try:
...
# (اختياري) مسار لتعطيل الويبهوك يدوياً
...
# Telegram webhook route (must match the URL you set in startup)
ALLOWED_UPDATES = None  # None = كل الأنواع
...
# ===== Lifecycle: setup webhook on startup, stop cleanly on shutdown =====
@app.on_event("startup")
async def on_startup():
    # 1) init
    await application.initialize()

    # 2) clear any old webhook
    try:
        await application.bot.delete_webhook(drop_pending_updates=True)
    except Exception:
        pass

    # 3) set webhook (Render should provide RENDER_EXTERNAL_URL)
    url = build_webhook_url()
    if url:
