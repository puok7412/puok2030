...
def status_text(sess: Session) -> str:
    ...
    lines.append(f"- نص: {check if has_text else cross}")
    lines.append(f"- وسائط: {len(media_list)}")
    if single_att:
        t = single_att[0]
        lines.append(f"- مرفق: {attach_map.get(t, t)}")
    else:
        lines.append(f"- مرفق: {cross}")

    # ⚙️ الخيارات
    lines.append(sep)
    lines.append("*⚙️ الخيارات*")
    ...
    lines.append(f"- التفاعلات: {check if use_reacts else cross}")
    lines.append(f"- التثبيت: {'📌' if pin_enabled else cross}")
    lines.append(f"- الوجهات: {chosen_cnt}")

    # ⏱️ الجدولة
    lines.append(sep)
    lines.append("*⏱️ الجدولة*")
    ...
    if schedule_active:
        lines.append(f"- الحالة: {check}")
        lines.append(f"- الفاصل: كل {hrs} ساعة")
        lines.append(f"- مرات الإعادة: {total}")
    else:
        lines.append(f"- الحالة: {cross}")

    # 📦 الحملة
    lines.append(sep)
    lines.append("*📦 الحملة*")
    campaign_id = getattr(sess, "campaign_id", None)
    lines.append(f"- المعرف: `{campaign_id if campaign_id is not None else '—'}`")
    lines.append(f"- الحفظ التلقائي: {check}")
...
def _md_escape(s: str) -> str:
    # تفادي كسر الـ Markdown لأسماء فيها رموز خاصة
...
async def start_publishing_session(user, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome = (
        f"👋 *مرحبًا* _{name}_\n"
        f"{sep}"
        f"أنت الآن في *وضع النشر*.\n"
        f"- أرسل *نصًا* أو *وسائط* (صورة/فيديو/ألبوم/مستند/صوت/فويس) **بأي ترتيب**.\n"
        f"- بعد أول إدخال سيتم *حفظ الجلسة تلقائيًا* وستظهر لك لوحة الخيارات.\n"
        f"{sep}"
        f"*نصائح سريعة*\n"
        f"- يمكنك دمج النص مع الوسائط في نفس الحملة.\n"
        f"- من *الخيارات* تستطيع تفعيل التثبيت 👍📌، التفاعلات، والجدولة ⏱️.\n"
    )
