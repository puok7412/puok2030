...
async def handle_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg and msg.text:
        ...
        if raw == "ok25s" or raw == "ok":
            # تم التقاطهما بهاندلرات منفصلة (Regex) لذلك نتجاهل هنا
            return
    # --- نهاية السلوك ---
...
def register_handlers():
    ...
    # ========= PRIVATE shortcuts =========
    # ok25s: افتح لوحة التحكم مباشرة في الخاص (أولوية أعلى)
    application.add_handler(
        MessageHandler(
    ...
    # ok: ابدأ جلسة النشر في الخاص (يأتي بعد ok25s)
    application.add_handler(
        MessageHandler(
