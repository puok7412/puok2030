...
async def handle_done_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sess = sessions.get(user_id)
    if not sess:
        await q.message.reply_text("⚠️ لا توجد جلسة حالية. أرسل ok لبدء جلسة نشر جديدة.")
        return

    # في حالة التصريح المؤقّت: ثبّت الوجهة تلقائيًا
    if getattr(sess, "is_temp_granted", False) and getattr(sess, "allowed_chats", set()):
        sess.chosen_chats = set(sess.allowed_chats)
...
async def answer_unknown_callback(update, context):
...
# مفاتيح سريعة في الخاص (رتّبناها لتجنّب التداخل)
...
# (زر تم) — خليه قبل أي كاتش-أول
application.add_handler(
    CallbackQueryHandler(handle_done_button, pattern=r"^done$"),
    group=9
)
...
# 4) واجهة جلسة النشر (catch-all) — استثنِ like/dislike/panel/perm
application.add_handler(
    CallbackQueryHandler(on_button, pattern=r"^(?!like:|dislike:|panel:|perm:).+"),
    group=99
)

# 5) فول-باك لأي شيء يسقط من الفلاتر (يوقف "تحميل")
application.add_handler(
