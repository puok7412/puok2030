...
BACKUP_ON_SAVE = os.getenv("BACKUP_ON_SAVE", "0") == "1"  # اتركها 0
...
def apply_persist_snapshot(data: Dict[str, Any]) -> None:
    ...
    if "reaction_style_by_message" in globals():
        reaction_style_by_message.update(_unpack_tuple_keys(data.get("reaction_style_by_message", {})))
...
_LAST_BACKUP_TS = 0.0  # اتركه كما هو أو فوق هذه الدالة

async def backup_state_to_tg(bot) -> bool:
    ...
    try:
        ...
        now = 0.0
    if now and (now - _LAST_BACKUP_TS) < 10.0:
        return False  # ضغط مزدوج سريع
    _LAST_BACKUP_TS = now

    try:
        payload = json.dumps(
            build_persist_snapshot(),
            ensure_ascii=False,
            separators=(",", ":")
        ).encode("utf-8")

        bio = io.BytesIO(payload)
        bio.name = "publisher_state.json"

        msg = await bot.send_document(
            chat_id=TG_BACKUP_CHAT_ID,
            document=bio,
            caption=f"publisher_state {datetime.utcnow().isoformat(timespec='seconds')}Z"
        )
        try:
        ...
        logger.exception("backup_state_to_tg failed")
        return False

async def restore_state_from_tg(bot) -> None:
    """استرجاع الحالة من الرسالة المثبّتة في مجموعة النسخ + مزامنة الأزرار بعد الاسترجاع."""
    if not TG_BACKUP_CHAT_ID:
        return
    try:
        chat = await bot.get_chat(TG_BACKUP_CHAT_ID)
        pm = getattr(chat, "pinned_message", None)
        doc = getattr(pm, "document", None) if pm else None
        if not doc:
            return

        fobj = await bot.get_file(doc.file_id)
        buf = io.BytesIO()
        await fobj.download_to_memory(out=buf)
        buf.seek(0)

        data = json.loads(buf.read().decode("utf-8"))
        apply_persist_snapshot(data)
        logger.info("State restored from pinned Telegram backup")

        # ⬅️ بعد الاسترجاع مباشرة: أعد مزامنة الأزرار لتظهر الأرقام الصحيحة
        try:
            await _resync_all_keyboards(bot)
        except Exception:
            logger.warning("Keyboard resync after restore failed")

    except Exception:
        logger.exception("restore_state_from_tg failed")
...
async def handle_reactions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if campaign_id is not None:
        ...
        try:
            ...
            # لا تحدّث كيبورد المنشور الأساسي عندما يكون الوضع prompt_only  # <--
        ...
        # 2) مزامنة باقي رسائل الحملة (وتنظيف كيبورد المنشور إذا كنا prompt_only حسب تعديلك في _sync)  # <--
        try:
            await _sync_campaign_keyboards(context, campaign_id)
        except Exception:
            pass

        # إشعار خفيف + رسالة شكر
        try:
        ...
        try:
            ...
            m = await context.bot.send_message(
                ...
                reply_to_message_id=(getattr(query.message, "message_id", base_or_msg_id) or base_or_msg_id),
                parse_mode="HTML",
            )
    ...
    # --- حالة رسالة مفردة (ليست حملة): استخدم reactions_counters للرسالة نفسها ---
    ...
    try:
        ...
        m = await context.bot.send_message(
            chat_id=chat_id,
            text=f"🙏 شكرًا {user.mention_html()} على تفاعلك {emoji}!",
            reply_to_message_id=current_mid,
            parse_mode="HTML",
        )
        context.application.job_queue.run_once(
            _delete_msg_job,
...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    # لو نعمل "prompt_only" ننظّف أي أزرار على المنشور لتفادي التكرار
    clean_base_when_prompt_only = True
    ...
    # مصادر رسائل الدعوة/التذكير (قد تُخزّن بمفتاح int أو str، وقد يكون الاسم campaign_messages أو campaign_prompt_msgs)
    camp_msgs_store = (globals().get("campaign_messages") or {})
    prompt_list = list(camp_msgs_store.get(campaign_id, [])) or list(camp_msgs_store.get(str(campaign_id), [])) or []

    # بعض المشاريع تحفظ الدعوات في campaign_prompt_msgs
    alt_prompt_store = (globals().get("campaign_prompt_msgs") or {})
    prompt_list_alt = list(alt_prompt_store.get(campaign_id, [])) or list(alt_prompt_store.get(str(campaign_id), [])) or []

    # دمج فريد للدعوات
    seen = set()
    for tup in prompt_list_alt:
        if tup not in prompt_list:
            prompt_list.append(tup)
    prompt_list = [t for t in prompt_list if not (t in seen or seen.add(t))]

    # خريطة رسائل الأساس: قد تكون المفاتيح (camp_id, cid) أو "camp|cid" أو "camp::cid"
    base_map = globals().get("campaign_base_msg") or {}

    def _parse_key_to_pair(k):
        """إرجاع (camp_id, chat_id) من أي مفتاح محتمل."""
        try:
            if isinstance(k, tuple) and len(k) >= 2:
    ...
    def _find_base_mid_for_chat(cid: int) -> int | None:
        """أحضِر base_mid لهذه الحملة وهذا الـ chat."""
        try:
            # 1) مفاتيح tuple
            bm = base_map.get((campaign_id, cid))
            if isinstance(bm, int):
                return bm
            # 2) مفاتيح نصية "camp|cid" أو "camp::cid"
    ...
    # لوحة الأزرار بالإجمالي: نُمرّر دائمًا base_mid في callback لو توفر
    ...
    # 0) إن كنا في "prompt_only" نظّف الأزرار من المنشور الأساسي (لو كانت موجودة من نشرات/إصدارات سابقة)
    if (mode == "prompt_only") and clean_base_when_prompt_only:
        try:
            for key, base_mid in list(base_map.items()):
                try:
                    ...
                    if camp_k != campaign_id or not isinstance(base_mid, int):
                        continue
                    # إزالة الكيبورد من رسالة الأساس
    ...
    # 1) حدّث الرسالة الأساسية لكل وجهة (فقط إذا الوضع يسمح)
    if attach_to_base:
        try:
            for key, base_mid in list(base_map.items()):
                try:
                    ...
                    if camp_k != campaign_id or not isinstance(base_mid, int):
                        continue
                    kb = _kb_for(cid, base_mid, base_mid)  # callback يستخدم base_mid مباشرة
    ...
    # 2) حدّث رسائل الدعوة/التذكير (فقط إذا الوضع يسمح)
    if attach_to_prompt and prompt_list:
        for (cid, mid) in list(prompt_list):
            try:
                base_mid = _find_base_mid_for_chat(cid)
                kb = _kb_for(cid, mid, base_mid)  # إن وُجد base_mid نستخدمه في callback
...
async def send_post_one_chat(
    ...
    *,
    ...
    # لو attach_to_base: ضَع الأزرار على المنشور نفسه
    if attach_to_base:
        try:
            await context.bot.edit_message_reply_markup(
                chat_id=chat_id,
                message_id=base_id_for_buttons,  # عادة هو base_mid
                reply_markup=kb
            )
        except Exception:
            pass
...
async def _auto_setup_and_warmup():
    ...
    if not url:
        logger.error("AUTO-WEBHOOK: BASE_URL/PUBLIC_URL غير متوفر")
        return
    try:
        # لا نكرر الضبط إن كان مضبوطًا
        ...
        if getattr(info, "url", "") != url:
            ...
            logger.info("AUTO-WEBHOOK: already set")
    except Exception as e:
        logger.exception("AUTO-WEBHOOK: set failed: %s", e)
...
# ===== مراقب دوري للويبهوك (كل 5 دقائق) =====
async def _webhook_watchdog():
    ...
    while True:
        try:
            info = await application.bot.get_webhook_info()
            if getattr(info, "url", "") != url:
                logger.warning("WATCHDOG: webhook drifted. resetting…")
                await application.bot.set_webhook(
                    url=url,
                    allowed_updates=None,
                    drop_pending_updates=False,
                )
                logger.info("WATCHDOG: reset done")
        except Exception as e:
...
async def setup_webhook(request: Request):
    ...
    async with _WEBHOOK_LOCK:
        now = time.monotonic()
        if not force and (now - _LAST_SET_TS) < 5:
            info = await application.bot.get_webhook_info()
            return {"ok": True, "skipped": "debounced", "url": url, "current": getattr(info, "url", "")}

        # مسح اختياري قبل الضبط
        if force:
            try:
                await application.bot.delete_webhook(drop_pending_updates=True)
                logger.info("setup-webhook: deleted current webhook (force)")
            except Exception as e:
                logger.warning("delete_webhook failed: %s", e)

        # إذا كان مضبوطًا أصلًا على نفس الرابط، لا تعيد الضبط (إلا لو force)
        if not force:
            try:
                info = await application.bot.get_webhook_info()
                if getattr(info, "url", "") == url:
        ...
        # set_webhook مع إعادة واحدة عند RetryAfter
        try:
            ok = await application.bot.set_webhook(
                url=url,
                allowed_updates=None,         # كل التحديثات
                drop_pending_updates=True,    # تنظيف الطابور
            )
        except RetryAfter as e:
        ...
        # أعد معلومات الويبهوك الحالية للمراقبة
        try:
            info = await application.bot.get_webhook_info()
            current = {
            ...
            current = {}
        return {"ok": ok, "url": url, "current": current}
...
def panel_main_keyboard() -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [InlineKeyboardButton("🗄️ نسخ احتياطي", callback_data="panel:backup")],
        # ⏱️ الإعادة
        [InlineKeyboardButton(
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel:backup":
        ...
        # 2) لو ما في قناة أرشيف
            if not TG_BACKUP_CHAT_ID:
                await query.answer(
                    "💾 تم الحفظ محليًا.\nاضبط TG_BACKUP_CHAT_ID لإرسال النسخة إلى القناة.",
                    show_alert=True
                )
                try:
                    await context.bot.send_message(
                        chat_id=query.from_user.id,
                        text="💾 تم الحفظ محليًا فقط.\nℹ️ اضبط TG_BACKUP_CHAT_ID لإرسال النسخة إلى قناة الأرشيف."
                    )
                except:
                    pass
                return
        ...
        # 3) النسخ اليدوي
            ok = await backup_state_to_tg(context.bot)

        # 4) إشعار النجاح/الفشل — Alert + DM + بديل في نفس الدردشة
            if ok:
                await query.answer("✅ تم إنشاء النسخة الاحتياطية وإرسالها إلى قناة الأرشيف.", show_alert=True)
                try:
                    from datetime import datetime
                    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    await context.bot.send_message(
                        chat_id=query.from_user.id,
                        text=f"✅ تم إرسال النسخة الاحتياطية الآن.\n🕒 {ts}\n📦 قناة الأرشيف: {TG_BACKUP_CHAT_ID}"
                    )
                except:
                    try:
                        await query.message.reply_text("✅ تم إرسال النسخة الاحتياطية الآن.")
                    except:
                        pass
            else:
                await query.answer("❌ فشل إرسال النسخة إلى قناة الأرشيف.", show_alert=True)
                try:
                    await context.bot.send_message(
                        chat_id=query.from_user.id,
                        text="❌ فشل إرسال النسخة إلى قناة الأرشيف. جرّب ثانية."
                    )
                except:
                    try:
                        await query.message.reply_text("❌ فشل إرسال النسخة إلى قناة الأرشيف.")
                    except:
                        pass

        except Exception:
            await query.answer("❌ فشل النسخ الاحتياطي.", show_alert=True)
            try:
                await context.bot.send_message(
                    chat_id=query.from_user.id,
...
async def _startup():
    # اضبط الويبهوك وسخّن الخدمة تلقائيًا
    asyncio.create_task(_auto_setup_and_warmup())
    # راقب الويبهوك دوريًا
    asyncio.create_task(_webhook_watchdog())

    # 1) حمّل أي حالة محلية (إن وُجدت)
    load_state()

    # 2) ثبّت الهاندلرات وابدأ تليجرام
    register_handlers()
    await application.initialize()

    # 3) استرجاع حالة احتياطية من تيليجرام (Pinned) إن كانت متاحة
    try:
        await restore_state_from_tg(application.bot)
    except Exception:
        logger.exception("Telegram backup restore failed")

    # 4) اضبط الويبهوك (مع معالجة Rate Limit)
    try:
        try:
            await application.bot.delete_webhook(drop_pending_updates=True)
        except Exception:
    ...
    # 6) استعادة مهام إعادة النشر المجدولة
    try:
