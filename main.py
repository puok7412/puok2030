...
class Session:
    ...
    pin_enabled: bool = True       # تفعيل/تعطيل تثبيت المنشور
...
async def publish_to_chats(context, user_id, sess, *, is_rebroadcast: bool, hide_links: bool):
    ...
    for cid in list(sess.chosen_chats):
        ...
        try:
            ...
            if mid:
                # pin + schedule unpin (فقط إذا مفعّل وليس أثناء الإعادة)
                if getattr(sess, "pin_enabled", True) and not is_rebroadcast:
                    try:
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel:schedule:stop_now":
        # أوقف أي JobQueue/منطق داخلي للإعادات عبر وضع عدّادات الحملات للانتهاء
        for job in getattr(context.application, "job_queue", []).jobs() if hasattr(context.application, "job_queue") else []:
