    ...
    for k, v in d.items():
        try:
            if isinstance(k, tuple) and len(k) >= 2:
                ...
                continue
            out[f"{int(str(a).strip())}|{int(str(b).strip())}"] = v
        except Exception:
            # نتجاهل المفتاح السيئ ولا نفشل الحفظ
            continue
    return out
...
async def handle_reactions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ ردّ فوري على الكولباك لمنع التعليق (spinner)
    try:
        if data.startswith("like:"):
            await query.answer("تم تسجيل إعجابك ✅", show_alert=False)
        elif data.startswith("dislike:"):
            await query.answer("تم تسجيل عدم الإعجاب ✅", show_alert=False)
        else:
            await query.answer()  # ردّ صامت إن تغيّر النمط
    except Exception:
        pass  # لا نسمح بأي خطأ هنا يوقف الدالة

    # نفس منطقك تمامًا من هنا:
    try:
    ...
    # تحصين بسيط لو كانت voters غير dict بالخطأ
