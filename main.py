...
def save_state():
    ...
    try:
        ...
        # لا شيء بعد هنا—لا إرسال لتليجرام
...
async def backup_state_to_tg(bot) -> None:
    ...
    # تبريد لمنع الضغط المزدوج على الزر
    try:
    ...
    try:
        ...
        # (اختياري) تثبيت هادئ بدون unpin_all لتفادي Flood
        try:
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel:backup":
        try:
        # 1) حفظ الحالة محليًا فقط
            save_state()

        # 2) إرسال نسخة إلى قناة الأرشيف يدويًا فقط (إن كانت مضبوطة)
            if not TG_BACKUP_CHAT_ID:
                await query.answer(
        ...
        # دالة النسخ اليدوي (فيها تبريد، وبدون unpin_all)
            await backup_state_to_tg(context.bot)
