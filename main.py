...
async def handle_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg and msg.text:
        ...
        if raw in ("ok25s", "ok"):
            # تم التقاطهما بهاندلرات مخصصة:
            # ok25s → يفتح لوحة التحكم عبر Regex handler
            # ok    → يبدأ جلسة النشر عبر start_publishing_keyword
            return
    # --- نهاية سلوك ok / ok25s ---
...
def register_handlers():
    ...
    # ========= PRIVATE shortcuts =========
    # ok25s: افتح لوحة التحكم مباشرة في الخاص
    application.add_handler(
        MessageHandler(
