...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    # حالة تفعيل نص الدعوة من الإعدادات العامة
    prompt_enabled = global_settings.get("reactions_prompt_enabled", True)
    ...
    # ===== حالة تعطيل نص الدعوة: نحدّث منشورات الأساس مباشرة في كل المجموعات =====
    if not prompt_enabled:
        for key, base_mid in list(base_map.items()):
            ...
            try:
                ...
                # نتجاهل الرسائل المحذوفة / القديمة
                continue
        return

    # ===== من هنا وما تحت: السلوك القديم عندما نص الدعوة مفعّل =====

    # -------- mode: prompt_only --------
    if mode == "prompt_only":
        # نظّف الأزرار من المنشور الأساسي
        for key, base_mid in list(base_map.items()):
            camp_k, cid = _parse_key_to_pair(key)
            if camp_k != campaign_id or not isinstance(base_mid, int):
                continue
    ...
    # تحديث رسائل الدعوة (إن كان مفعلا)
    if attach_to_prompt:
        for (cid, mid) in prompt_list:
            try:
                base_mid = _find_base_mid_for_chat(cid)
    ...
    # تحديث منشورات الأساس في حال base_only أو both
    if attach_to_base:
        for key, base_mid in list(base_map.items()):
            camp_k, cid = _parse_key_to_pair(key)
            if camp_k != campaign_id or not isinstance(base_mid, int):
