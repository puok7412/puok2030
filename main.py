...
async def start_publishing_keyword(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🛠️ إشعار فوري عند كتابة OK إذا كان وضع الصيانة مُفعّلاً
    if global_settings.get("maintenance_mode", False):
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "publish":
        # 🛠️ احترم وضع الصيانة
        if global_settings.get("maintenance_mode", False):
            await query.message.reply_text("🛠️ نظام النشر تحت الصيانة والتحديث حاليًا.")
            return
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel:schedule":
        ...
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(
                f"{'⏹️ إيقاف' if on else '▶️ تشغيل'} الإعادة",
                callback_data="panel:sched_toggle"
            )],
        ...
        try:
            await query.message.reply_text(
                f"⏱️ الإعادة: *{'مفعّلة' if on else 'معطّلة'}*",
                parse_mode="Markdown",
                reply_markup=kb
            )
        except Exception:
    ...
    if data == "panel:exit":
        try:
            ...
            pass
        try:
            await query.message.reply_text("👋 تم الخروج من لوحة التحكّم.")
        except Exception:
            pass
        return
...
def register_handlers():
    ...
    # ========= Auto-register أي مجموعة/قناة على أي رسالة =========
    ...
    # ========= استقبال محتوى/نصوص في الخاص (آخر نص هاندلر) =========
    application.add_handler(
