...
global_settings = {
    ...
    "rebroadcast_interval_seconds": 2700,  # الافتراضي: كل 2 ساعة
...
def build_session_schedule_keyboard(sess) -> InlineKeyboardMarkup:
    ...
    current_h = current_secs // 3600
    current_m = (current_secs // 60) if current_secs else 0
    ...
    # عرض الفاصل الحالي بصيغة h/m وإضافة زر دقيقة للاختبار
    rows.append([InlineKeyboardButton(f"⏲️ الفاصل (الحالي {current_h}h/{current_m}m)", callback_data="noop")])

    # الخيارات بالساعات كما هي
...
def build_chats_keyboard(chat_ids: List[int], chosen: Set[int], settings: Dict[str, Any]) -> InlineKeyboardMarkup:
    ...
    for cid in chat_ids:
        ...
        mark  = "✅" if cid in chosen else "🚫"      # مفعّل/غير مفعّل (دائرة بخط أحمر)
