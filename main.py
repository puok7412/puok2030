...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    def _kb_for(cid: int, mid_for_buttons: int, base_mid: int | None) -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup([[
            ...
            InlineKeyboardButton(
                f"{neg_emo} {tot_dislike}",
                callback_data=f"dislike:{cid}:{use_id}"
            ),
        ]])

    # ✅ وضع prompt_only: نظّف كيبورد منشور الأساس، وحدّث الدعوات فقط، ثم اخرج
    if mode == "prompt_only":
        try:
    ...
    #    مع احترام رغبة المعلن: لا تضيف أزرار لمنشور لم تُفعَّل له التفاعلات أساسًا
    if attach_to_base:
        for key, base_mid in list(base_map.items()):
            camp_k, cid = _parse_key_to_pair(key)
            if camp_k != campaign_id or not isinstance(base_mid, int):
            ...
            # 👈 هذا هو الشرط الذي يحترم "استخدام التفاعلات":
            # لو ما فيه سجل في reactions_counters، لا نركّب أزرار لهذا المنشور.
            rec_key = (cid, base_mid)
            if rec_key not in reactions_counters:
                continue
