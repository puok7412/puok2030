...
OK25S_REGEX = re.compile(r'(?i)^\s*ok25s\s*$')   # فتح لوحة التحكم المركزية
...
def keyboard_collecting() -> InlineKeyboardMarkup:
    # نعرض زر إدراج القوالب دوماً؛ وإن لم توجد قوالب سيظهر تنبيه مناسب عند الضغط
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📑 إدراج قالب", callback_data="insert_template")],
...
def build_session_schedule_keyboard(sess: "Session") -> InlineKeyboardMarkup:
    ...
    rows.append([
        ...
        InlineKeyboardButton("8س", callback_data="ssched_int:28800"),
        InlineKeyboardButton("12س", callback_data="ssched_int:43200"),
    ])

    # 🔁 عدد الإعادات 2-4-6-8-10-12
    for chunk in [(2,4,6), (8,10,12)]:
        rows.append([InlineKeyboardButton(f"{c}×", callback_data=f"ssched_total:{c}") for c in chunk])
    ...
    try:
        ...
        cur_hrs = (cur_int + 3599)//3600 if cur_int>0 else 0
        rows.append([InlineKeyboardButton(f"ℹ️ الفاصل: {cur_hrs} ساعة • الإعادات: {int(getattr(sess,'rebroadcast_total',0) or 0)}×", callback_data="noop")])
    except Exception:
        pass
...
def keyboard_ready_options(sess: Session, settings: Dict[str, Any]) -> InlineKeyboardMarkup:
    ...
    # لا نظهر زر التفاعلات إلا إذا كان الافتراضي مفعّل AND العلم المركزي مفعّل
    if settings.get("default_reactions_enabled", True) and global_settings.get("reactions_feature_enabled", True):
        btns.append([
            InlineKeyboardButton(
...
async def start_publishing_keyword(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تحقّق من إداريته في مجموعات/قنوات وأنه غير محظور منها مركزياً
    admin_ids = await chats_where_user_is_admin(context, user_id)
    if admin_ids:
        allowed_any = False
        for cid in admin_ids:
            perm = group_permissions.get(cid) or {}
            blocked = set(perm.get("blocked_admins") or [])
    ...
    await update.message.reply_text("🔒 هذا الأمر متاح فقط بتصريح فعّال أو إذا كنت مشرفاً مسموحاً به في إحدى الوجهات.")
...
async def cmd_temp_ok(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_admin_in_chat(context, chat.id, granter.id):
        try: await update.message.reply_text("🚫 هذا الأمر للمشرفين فقط.")
        except Exception: pass
        return

    # مشرف «مُعطّل» في هذه الوجهة؟
    gp = group_permissions.setdefault(chat.id, {"blocked_admins": set()})
    if granter.id in gp.get("blocked_admins", set()):
        try: await update.message.reply_text("⛔ تم تعطيل صلاحياتك لمنح /ok في هذه الوجهة.")
        except Exception: pass
        return
...
async def handle_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- حالات إدخال اللوحة المركزية أولاً ---
    st = panel_state.get(user_id)
    if st == PANEL_WAIT_REACTION_PROMPT and update.message and update.message.text:
        new_txt = sanitize_text(update.message.text).strip()
        if new_txt:
            s["reaction_prompt_text"] = new_txt  # موجود افتراضياً في get_settings :contentReference[oaicite:7]{index=7}
            save_state()
            panel_state[user_id] = PANEL_MAIN
            # ارجع لشاشة التفاعلات
            await context.bot.send_message(
                chat_id=user_id,
                text=f"✅ تم حفظ نص الدعوة الجديد:\n`{new_txt}`",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ رجوع", callback_data="panel:reactions")]])
            )
            return
        # نص فارغ
        await context.bot.send_message(chat_id=user_id, text="⚠️ النص فارغ. أعد المحاولة أو اضغط رجوع.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ رجوع", callback_data="panel:reactions")]]))
        return

    if isinstance(st, str) and st.startswith("tpl:") and update.message and update.message.text:
        payload = sanitize_text(update.message.text).strip()
        if not payload:
            await context.bot.send_message(chat_id=user_id, text="⚠️ النص فارغ. أعد الإرسال أو اضغط رجوع.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ رجوع", callback_data="panel:templates")]]))
            return
        # فصل العنوان عن النص (اختياري)
        ...
        if st == "tpl:add":
            ...
            s["templates"] = tpls
            msg = "✅ تم إضافة القالب."
        elif st.startswith("tpl:edit:"):
            try:
                idx = int(st.split(":")[-1])
            except Exception:
                idx = -1
            if 0 <= idx < len(tpls):
                # دعم القيم النصية القديمة بتحويلها لقاموس
                if not isinstance(tpls[idx], dict):
                    tpls[idx] = {"name": "", "text": str(tpls[idx])}
                tpls[idx]["name"] = name
                tpls[idx]["text"] = text
                msg = "✅ تم تحديث القالب."
            else:
                msg = "⚠️ لم يتم العثور على القالب المحدد."
            s["templates"] = tpls
        else:
            msg = "⚠️ حالة غير معروفة."

        panel_state[user_id] = PANEL_MAIN
        save_state()
        await context.bot.send_message(chat_id=user_id, text=msg, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ رجوع", callback_data="panel:templates")]]))
        return
    # --- نهاية حالات اللوحة المركزية ---

    # صلاحيات أبيض/أسود (من منطقك الحالي)
    ...
    # سلوك ok/ok25s في الخاص (كما هو لديك)
    if msg.text:
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data or ""
    # تجاهل ما يُعالج في هاندلرز أخرى
    ...
    # لا توجد جلسة
    if not sess and data not in ("insert_template",):  # السماح بفتح منتقي القوالب يعرض تنبيه لاحقًا
        await query.message.reply_text("⚠️ لا توجد جلسة حالية. أرسل ok لبدء جلسة نشر جديدة.")
        return

    # ===== إنهاء الجمع والانتقال للخيارات =====
    if data == "done":
    ...
    # ===== التفاعلات: احترام التعطيل المركزي =====
    if data == "toggle_reactions":
        if not global_settings.get("reactions_feature_enabled", True):
            await query.answer("💬 ميزة التفاعلات مُعطّلة مركزيًا.", show_alert=True)
            return
    ...
    # ===== التثبيت: احترام التعطيل المركزي =====
    if data == "toggle_pin":
    ...
    # دعم خلفي للنسخ القديمة (ssched_count) + النسخة المعتمدة (ssched_total)
    if data.startswith("ssched_count:") or data.startswith("ssched_total:"):
    ...
    if data == "choose_chats":
        ...
        glob_disabled = global_settings.get("disabled_chats_global", set()) or set()
        active_ids = [cid for cid in admin_chat_ids if cid not in s["disabled_chats"] and cid not in glob_disabled]
        if not active_ids:
            await query.message.reply_text("🚫 كل الوجهات المصرّح بها معطّلة حاليًا من اللوحة (شخصيًا أو مركزيًا).")
            return
    ...
    if data.startswith("toggle_chat:"):
        ...
        # احترام التعطيل المركزي/الشخصي
        glob_disabled = global_settings.get("disabled_chats_global", set()) or set()
        if cid in s["disabled_chats"] or cid in glob_disabled:
            await query.answer("هذه الوجهة معطّلة حاليًا.", show_alert=True)
            return
        ...
        active_ids = [i for i in source_ids if i not in s["disabled_chats"] and i not in glob_disabled]
        rows: List[List[InlineKeyboardButton]] = []
    ...
    if data == "select_all":
        ...
        source_ids = list(sess.allowed_chats) if sess.is_temp_granted else await list_authorized_chats(context, user_id)
        glob_disabled = global_settings.get("disabled_chats_global", set()) or set()
        sess.chosen_chats = set(i for i in source_ids if i not in s["disabled_chats"] and i not in glob_disabled)
    ...
    if data == "publish":
        ...
        # احترام التعطيل المركزي للتفاعلات وقت النشر
        sess.use_reactions = bool(sess.use_reactions) \
                             and bool(s.get("default_reactions_enabled", True)) \
                             and bool(global_settings.get("reactions_feature_enabled", True))
        ...
        # تشغيل الإرسال على شكل مهمة غير حاجبة
    ...
    # ===== قوالب: منتقي وإدراج =====
    if data == "insert_template":
        settings = get_settings(user_id)
        tpls = settings.get("templates") or []
        if not tpls:
            await query.answer("لا توجد قوالب حتى الآن.", show_alert=True)
            return
        ...
        def _tpl_name(i, t):
            if isinstance(t, dict):
                ...
                name = ""
                text = str(t).strip()
            return name or (text.splitlines()[0][:40] if text else f"قالب {i+1}")

        rows: List[List[InlineKeyboardButton]] = []
        for i, t in enumerate(tpls):
            rows.append([InlineKeyboardButton(f"📄 {_tpl_name(i, t)}", callback_data=f"ins_tpl:{i}")])
        rows.append([InlineKeyboardButton("⬅️ إغلاق", callback_data="close_picker")])

        delete_picker_if_any(context, user_id, sess)
        m = await context.bot.send_message(chat_id=user_id, text="📑 اختر قالباً لإدراجه في نص المنشور:", reply_markup=InlineKeyboardMarkup(rows))
        sess.picker_msg_id = m.message_id
        await query.answer()
        return

    if data.startswith("ins_tpl:"):
        try:
        ...
        if not (0 <= idx < len(tpls)):
            await query.answer("قالب غير موجود.", show_alert=True)
            return

        t = tpls[idx]
        tpl_text = (t.get("text") if isinstance(t, dict) else str(t)) or ""
        tpl_text = sanitize_text(tpl_text).strip()

        if tpl_text:
            sess.text = (f"{sess.text}\n\n{tpl_text}".strip() if sess.text else tpl_text)
            save_state()

        delete_picker_if_any(context, user_id, sess)
        await push_panel(context, user_id, sess, "✅ تم إدراج القالب في النص.")
        await query.answer()
        return

    if data == "close_picker":
        if sess:
            delete_picker_if_any(context, user_id, sess)
        await query.answer()
        return
...
async def publish_to_chats(context: ContextTypes.DEFAULT_TYPE, user_id: int, sess: Session, *, is_rebroadcast: bool, hide_links: bool):
    ...
    # فلترة الوجهات المعطّلة لهذا المستخدم
    s = get_settings(user_id)
    disabled = s.get("disabled_chats", set()) or set()
    original = set(sess.chosen_chats)
    sess.chosen_chats = {cid for cid in original if cid not in disabled}

    if not sess.chosen_chats:
        try:
            await context.bot.send_message(chat_id=user_id, text="⚠️ لا توجد وجهات صالحة للنشر (ربما كلها معطّلة من لوحتك).")
        except Exception:
            pass
        return

    # ... (تابع الدالة كما هي عندك لإرسال الرسائل على chosen_chats)
...
def panel_main_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🗂️ الوجهات (شخصي)", callback_data="panel:destinations")],
        [InlineKeyboardButton("🗂️ الوجهات (مركزي)", callback_data="panel:destinations_global")],
...
# =============================
# أوامر فتح اللوحة (مطابقة للاسم القديم)
# =============================
...
# مساعدة: جلب المجموعات التي يملكها المستخدم (وفق منطق الملف القديم)
...
# =============================
# المعالج المركزي لأزرار اللوحة (قديم)
# =============================
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ====== الإحصاءات (موجودة لديك؛ أبقِها كما هي) ======
    if data == "panel:stats":
        tot_like = sum(v.get("like", 0) for v in campaign_counters.values())
        tot_dislike = sum(v.get("dislike", 0) for v in campaign_counters.values())
        txt = (
            "📊 *إحصاءات عامة*\n"
            f"إجمالي 👍: *{tot_like}* • إجمالي 👎: *{tot_dislike}*\n"
            "اختر طريقة العرض:"
        )
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("📈 حسب الحملة", callback_data="panel:stats_by_campaign")],
            [InlineKeyboardButton("🏷️ حسب الوجهة (المجموعة/القناة)", callback_data="panel:stats_by_chat")],
    ...
    if data == "panel:stats_by_campaign":
        rows = []
        for cid, cnt in campaign_counters.items():
            rows.append([InlineKeyboardButton(f"حملة #{cid} • 👍{cnt.get('like',0)}/👎{cnt.get('dislike',0)}", callback_data=f"panel:noop")])
        rows.append([InlineKeyboardButton("⬅️ رجوع", callback_data="panel:stats")])
        await query.edit_message_text("📈 *حسب الحملة:*", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(rows))
        await query.answer()
        return

    if data == "panel:stats_by_chat":
        rows = []
        for chat_id, meta in known_chats.items():
            like = 0; dislike = 0
            # تجميع تقديري من لوحات الحملة
            for cc in campaign_counters.values():
                like += cc.get("like", 0)
                dislike += cc.get("dislike", 0)
            title = meta.get("title") or str(chat_id)
            rows.append([InlineKeyboardButton(f"{title} — (تراكمي إجمالي) 👍{like}/👎{dislike}", callback_data="panel:noop")])
        rows.append([InlineKeyboardButton("⬅️ رجوع", callback_data="panel:stats")])
        await query.edit_message_text("🏷️ *حسب الوجهة:*", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(rows))
        await query.answer()
        return

    # ====== الوجهات / السجل / الإعدادات … (أبقِها كما هي) ======
    if data == "panel:destinations":
        # منطقك الحالي لاختيار الوجهات المركزية
        # ...
        pass
    ...
    # ====== التفاعلات (مفتاح مركزي + نص الدعوة) ======
    if data == "panel:reactions":
        enabled = bool(global_settings.get("reactions_feature_enabled", True))
        cur_prompt = s.get("reaction_prompt_text", "قيّم المنشور 👇")  # موجود في get_settings الافتراضي :contentReference[oaicite:6]{index=6}
        txt = (
            "💬 *التفاعلات — تحكم مركزي*\n"
            f"الحالة: *{'مفعّل' if enabled else 'معطّل'}*\n"
            f"نص الدعوة الحالي:\n`{cur_prompt}`"
        )
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🔁 تفعيل/تعطيل", callback_data="panel:reactions_toggle")],
            [InlineKeyboardButton("✏️ تعديل نص الدعوة", callback_data="panel:reactions_prompt")],
            [InlineKeyboardButton("💾 حفظ", callback_data="panel:back"), InlineKeyboardButton("⬅️ رجوع", callback_data="panel:back")]
        ])
        await query.edit_message_text(txt, parse_mode="Markdown", reply_markup=kb)
        await query.answer()
        return

    if data == "panel:reactions_toggle":
        cur = bool(global_settings.get("reactions_feature_enabled", True))
        global_settings["reactions_feature_enabled"] = not cur
        save_state()
        await query.answer("تم تحديث الحالة.")
        # أعد فتح شاشة التفاعلات لتحديث العرض
        await context.application.create_task(handle_control_buttons(update, context))
        return

    if data == "panel:reactions_prompt":
        panel_state[user_id] = PANEL_WAIT_REACTION_PROMPT
        await query.edit_message_text(
            "✏️ أرسل الآن نص الدعوة للتقييم (مثال: *من فضلك قيّم هذا المنشور*)\n\n"
            "بعد الإرسال سيتم الحفظ مباشرة.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ إلغاء", callback_data="panel:reactions")]])
        )
        await query.answer()
        return

    # ====== القوالب (إدارة مركزية) ======
    if data == "panel:templates":
        ...
        def _title(i, t):
            ...
            return name or (text.splitlines()[0][:40] if text else f"قالب {i+1}")
        for i, t in enumerate(tpls):
            rows.append([InlineKeyboardButton(f"📄 {_title(i,t)}", callback_data="panel:noop")])
            rows.append([
                InlineKeyboardButton("✏️ تحرير", callback_data=f"panel:tpl_edit:{i}"),
                InlineKeyboardButton("🗑️ حذف", callback_data=f"panel:tpl_del:{i}")
            ])
        rows.append([InlineKeyboardButton("➕ إضافة قالب", callback_data="panel:tpl_add")])
        rows.append([InlineKeyboardButton("⬅️ رجوع", callback_data="panel:back")])
        await query.edit_message_text("📑 *القوالب — إدارة*", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(rows))
        await query.answer()
        return

    if data == "panel:tpl_add":
        panel_state[user_id] = "tpl:add"
        await query.edit_message_text(
            "🆕 أرسل نص القالب الآن.\n"
            "يمكنك وضع *العنوان في السطر الأول* ثم سطر جديد وباقي النص.\n"
            "مثال:\n"
            "_عنوان القالب_\n"
            "النص…",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ رجوع", callback_data="panel:templates")]])
        )
        await query.answer()
        return

    if data.startswith("panel:tpl_edit:"):
        try:
            idx = int(data.split(":")[-1])
        except Exception:
            await query.answer()
            return
        panel_state[user_id] = f"tpl:edit:{idx}"
        await query.edit_message_text(
            "✏️ أرسل النص الجديد للقالب بنفس الطريقة:\n"
            "*سطر أول = عنوان (اختياري)* ثم سطر جديد لنص القالب.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ رجوع", callback_data="panel:templates")]])
        )
        await query.answer()
    ...
    if data.startswith("panel:tpl_del:"):
        try:
            idx = int(data.split(":")[-1])
        except Exception:
            await query.answer()
        ...
        if 0 <= idx < len(tpls):
            ...
            s["templates"] = tpls
            save_state()
        await query.answer("تم حذف القالب.")
        await handle_control_buttons(update, context)  # إعادة فتح شاشة القوالب
        return

    # رجوع عام
    if data in ("panel:back", "panel:noop"):
        await cmd_panel(update, context)
        await query.answer()
        return
...
def register_handlers():
    ...
    application.add_handler(ChannelPostHandler(auto_register_chat))
    ...
    # ok25s في الخاص → افتح لوحة التحكم المركزية
    application.add_handler(
        MessageHandler(
...
async def on_startup():
    # 1) حمّل الحالة من التخزين
    load_state()

    # 2) سجّل الهاندلرز مرة واحدة فقط (تحاشي التكرار عند إعادة التشغيل)
    if not application.bot_data.get("handlers_registered"):
        register_handlers()
        application.bot_data["handlers_registered"] = True

    # 3) تهيئة التطبيق ثم تشغيله
    await application.initialize()
    await application.start()

    # 4) إعادة ضبط الويبهوك: احذف القديم ثم اضبط الجديد
    try:
        # نحذف القديم مع إسقاط التحديثات المتراكمة لتفادي تراكم رسائل قديمة
        await application.bot.delete_webhook(drop_pending_updates=True)
    except Exception:
        pass
    ...
    url = build_webhook_url()
    if url:
        # قلّل قائمة التحديثات المقبولة للي تحتاجه فعلاً
        allowed = [
        ...
        # سرّ الويبهوك (إن كان مٌعرّفًا في البيئة)
        secret = None
        try:
            import os  # لا نضيف دوال جديدة؛ مجرد قراءة متغيّر بيئة إن وُجد
            secret = os.environ.get("WEBHOOK_SECRET") or None
        except Exception:
            pass

        try:
            # بعد الحذف أعلاه لا نستخدم drop_pending_updates هنا
    ...
    # 5) استرجاع وظائف الإعادة (Jobs) بعد تشغيل التطبيق
    try:
        ...
        for name, rec in active_rebroadcasts.items():
            ...
            try:
                # first: نضبطها قصيرة ومعقولة لتبدأ قريبًا بدون انتظار طويل
                jq.run_repeating(
                    rebroadcast_job,
                    interval=interval,
                    first=max(10, min(60, interval)),  # بين 10 و60 ثانية
                    data=payload,
                    name=name
                )
            except Exception:
