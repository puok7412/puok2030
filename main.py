...
async def send_preview(update: Update, context: ContextTypes.DEFAULT_TYPE, sess: Session, *, hide_links: bool):
    ...
    # أولاً: المرفق المنفرد (صوتي / مستند / فويس)
    ...
    # ثانياً: الألبوم / الوسائط المتعددة
    if getattr(sess, "media_list", None):
        if len(sess.media_list) > 1:
            media_group = []
            for t, fid, cap in sess.media_list:
                c = cap
                if c:
                    c = hidden_links_or_plain(c, hide_links)
                if t == "photo":
    ...
    # ثالثاً: نص المنشور (رسالة مستقلة في النهاية)
...
async def send_post_one_chat(
    ...
    *,
    ...
    # --- أولاً: مرفق منفرد (صوت / مستند / فويس) ---
    ...
    # --- ثانياً: وسائط متعددة (ألبوم/صور/فيديو) ---
    if sess.media_list:
        if len(sess.media_list) > 1:
            media_group = []
            for t, fid, cap in sess.media_list:
            ...
            if msgs:
                first_message_id = first_message_id or msgs[0].message_id
        else:
            t, fid, cap = sess.media_list[0]
            c = cap
            if c:
    ...
    # --- ثالثاً: نص المنشور كرسالة مستقلة في النهاية ---
    if main_text:
        m = await context.bot.send_message(
            chat_id=chat_id,
            text=hidden_links_or_plain(main_text, hide_links),
            parse_mode=("HTML" if hide_links else None),
