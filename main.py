...
TG_BACKUP_CHAT_ID = int(os.getenv("TG_BACKUP_CHAT_ID", "0") or "0")
_LAST_BACKUP_TS = 0.0
...
def save_state():
    ...
    try:
        ...
        if not chat_id_str:
            return  # لم يحدد المستخدم قناة النسخ — نتخطى بهدوء
        ...
        async def _tg_backup():
            try:
                ...
                # ثبّت آخر نسخة (إن أمكن)
                try:
                    await application.bot.unpin_all_chat_messages(chat_id=int(chat_id_str))
                except Exception:
                    pass
                try:
...
def build_persist_snapshot() -> Dict[str, Any]:
    # اختصرنا على الأشياء الخاصة بالنشر/الحملات/التفاعلات فقط
    return {
        "known_chats": known_chats,  # عناوين فقط (مفيد للعرض)
        "campaign_messages": campaign_messages,               # {camp_id: [(cid, mid), ...]}
        "campaign_base_msg": _pack_tuple_keys(campaign_base_msg),  # {(camp_id,cid)->mid} لكن نحن نستخدم (camp_id,cid)؟ عندك أصلاً (camp_id,cid)-> mid
        "message_to_campaign": _pack_tuple_keys(message_to_campaign),  # {(cid, mid)-> camp_id}
        "campaign_counters": campaign_counters,               # {camp_id: {like, dislike, voters}}
        "reactions_counters": _pack_tuple_keys(reactions_counters),    # {(cid, base_mid)-> {like, dislike, voters}}
...
async def backup_state_to_tg(bot) -> None:
    ...
    if now - _LAST_BACKUP_TS < 3.0:
        return
    _LAST_BACKUP_TS = now
    try:
        payload = json.dumps(build_persist_snapshot(), ensure_ascii=False, separators=(",", ":")).encode("utf-8")
        ...
        try:
            # بيئة النسخ: قناتك الخاصة — مسموح نمسح كل تثبيتات ثم نثبّت آخر نسخة
            await bot.unpin_all_chat_messages(chat_id=TG_BACKUP_CHAT_ID)
        except Exception:
            pass
        try:
            await bot.pin_chat_message(chat_id=TG_BACKUP_CHAT_ID, message_id=msg.message_id, disable_notification=True)
        except Exception:
            pass
...
async def restore_state_from_tg(bot) -> None:
    ...
    try:
        ...
        if not pm or not getattr(pm, "document", None):
            return
        file = await bot.get_file(pm.document.file_id)
        path = "/tmp/_restore.json"
        await file.download_to_drive(path)
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
        apply_persist_snapshot(data)
        logger.info("State restored from pinned backup in %s", TG_BACKUP_CHAT_ID)
    except Exception:
...
async def cmd_mychats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # نعرضها في الخاص فقط حتى لا ننشر IDs علنًا
    if update.effective_chat.type != ChatType.PRIVATE:
        try:
            await update.message.reply_text("افتح الخاص مع البوت واكتب /mychats لعرض القنوات/المجموعات مع أرقامها.")
        except Exception:
            pass
        return
    ...
    # احصل على الوجهات التي أنت مشرف فيها فعليًا
    try:
        admin_chat_ids = set(await list_authorized_chats(context, user_id))
    except Exception:
        admin_chat_ids = set()

    # لا يوجد أي وجهات معروفة
    if not known_chats:
        await context.bot.send_message(
            chat_id=user_id,
            text="لا توجد وجهات مسجّلة بعد.\nادخل كل مجموعة/قناة وأرسل /register ثم أعد /mychats."
        )
        return

    # نبني القائمة: الاسم + ID + النوع (+ وسم 👑 إذا كنت مشرفًا)
    rows = []
    for cid, meta in sorted(known_chats.items(), key=lambda kv: (str(kv[1].get("title") or ""), kv[0])):
        title = meta.get("title") or str(cid)
        typ = meta.get("type") or ("channel" if str(cid).startswith("-100") else "group")
        typ_ar = "قناة" if typ == "channel" else "مجموعة"
        crown = "👑" if cid in admin_chat_ids else "—"
        rows.append(f"{crown} *{title}*\nID: `{cid}` • النوع: {typ_ar}")

    header = "📋 *قائمة الوجهات المسجّلة*\n- 👑: لديك صلاحية مشرف\n- — : لست مشرفًا\n\n"
    text = header + "\n".join(rows)

    # تيليجرام يحدّ 4096 حرفًا؛ نقسّم لو احتجنا
    MAX = 3800
    parts = [text[i:i+MAX] for i in range(0, len(text), MAX)]
    for part in parts:
        try:
            await context.bot.send_message(chat_id=user_id, text=part, parse_mode=ParseMode.MARKDOWN)
        except Exception:
            # محاولة بدون Markdown لو حصل خطأ بالترميز
