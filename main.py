...
async def handle_reactions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ حدِّث زر الرسالة الحالية فورًا:
    try:
        if campaign_id is not None:
            # في الحملة نعرض الإجمالي
            tot = campaign_counters.get(campaign_id, {"like": 0, "dislike": 0})
            like_cnt, dislike_cnt = int(tot.get("like", 0)), int(tot.get("dislike", 0))
        else:
            # في الرسالة المفردة نعرض عدّادها
            like_cnt, dislike_cnt = int(rec.get("like", 0)), int(rec.get("dislike", 0))
    ...
    # إن كانت حملة: زامن باقي رسائلها (best effort)
    if campaign_id is not None:
        try:
    ...
    try:
        ...
        logger.exception("save_state after reaction failed")
...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    if not style:
        try:
            # حاول استنتاج النمط من أي رسالة أساسية
            for (cid, base_mid) in (globals().get("campaign_base_msg", {}) or {}).keys():
                # المفاتيح هنا هي tuples (camp_id, cid) → نبحث عن camp_id المطابق
                pass
        except Exception:
            pass
    ...
    # مصادر الرسائل:
    campaign_msgs = (globals().get("campaign_messages") or {}).get(campaign_id, [])
    if not campaign_msgs:
        # في بعض الأكواد تُحفظ رسائل الحملة في مفاتيح نصية
        campaign_msgs = (globals().get("campaign_messages") or {}).get(str(campaign_id), []) or []

    # لوحة الزر بالإجمالي
    def _kb_for(cid, mid):
        return InlineKeyboardMarkup([[
            InlineKeyboardButton(f"{pos_emo} {tot_like}",    callback_data=f"like:{cid}:{mid}"),
            InlineKeyboardButton(f"{neg_emo} {tot_dislike}", callback_data=f"dislike:{cid}:{mid}"),
        ]])

    # 1) حدّث الرسالة الأساسية لكل وجهة
    try:
        base_map = globals().get("campaign_base_msg") or {}
        for key, base_mid in list(base_map.items()):
            try:
                # key قد يكون tuple أو "camp|cid"
                if isinstance(key, tuple) and len(key) >= 2:
                    camp_k, cid = int(key[0]), int(key[1])
                elif isinstance(key, str) and ("|" in key or "::" in key):
                    sep = "|" if "|" in key else "::"
                    a, b = key.split(sep, 1)
                    camp_k, cid = int(a), int(b)
                else:
                    continue
                if camp_k != campaign_id:
                    continue
                kb = _kb_for(cid, base_mid)
    ...
    # 2) حدّث رسائل الدعوة/التذكير (إن كانت محفوظة في campaign_messages)
    for (cid, mid) in list(campaign_msgs or []):
        try:
            kb = _kb_for(cid, mid)
