...
# =============================
OK_REGEX = re.compile(r'(?i)^\s*ok\s*$')        # ← خاص ببدء جلسة النشر
OK25S_REGEX = re.compile(r'(?i)^\s*ok25s\s*$')  # ← خاص بفتح لوحة التحكّم
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel:back":
        ...
        await query.answer()
        return
async def open_central_panel_keyword(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type != ChatType.PRIVATE:
        return
    # (اختياري) تأكد أنه مشرف في أي وجهة مسجّلة
    admin_ids = await chats_where_user_is_admin(context, update.effective_user.id)
    if not admin_ids and not _user_has_active_grant(update.effective_user.id):
        await update.message.reply_text("لوحة التحكّم المركزية متاحة للمشرفين فقط.")
        return
    # افتح لوحة التحكّم المركزية
    await cmd_panel(update, context)
