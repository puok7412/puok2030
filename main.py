...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    مزامنة أزرار التفاعل لجميع رسائل الحملة وفق الوضع المختار:
      - REACTIONS_ATTACH_MODE = "prompt_only": الأزرار على رسائل الدعوة فقط
      - REACTIONS_ATTACH_MODE = "base_only"  : الأزرار على رسائل الأساس (المنشور) فقط
    ...
    if not style:
        try:
            # يمكن لاحقًا استنتاج النمط من أي رسالة أساسية إن دُعِم التخزين
            pass
        except Exception:
            pass
        style = style or "thumbs"
    ...
    def _find_base_mid_for_chat(cid: int) -> int | None:
        ...
        try:
            # محاولات متعددة لاستخراج base_mid
            # 1) مفاتيح tuple
