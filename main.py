...
# مكان ظهور أزرار التفاعل للحملات – الآن نعتمد على المنشور الأساسي فقط
REACTIONS_ATTACH_MODE = "base_only"
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "reactions_menu":
        ...
        try:
            await query.message.reply_text(
                "🎭 اختر نمط التفاعلات:",
                reply_markup=build_reactions_menu_keyboard(sess, s)
            )
        except Exception:
            pass
        return
    ...
    if data == "back_main":
        ...
        try:
            if getattr(sess, "panel_msg_id", None) and query.message and (
                query.message.message_id != sess.panel_msg_id
            ):
                await context.bot.delete_message(
    ...
    if data == "preview":
        await send_preview(
            ...
            context,
            sess,
            hide_links=get_settings(user_id).get("hide_links_default", False)
        )
        return
...
async def handle_reactions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # شكل الداتا: like:cid:mid أو dislike:cid:mid
    try:
    ...
    if campaign_id is not None:
        ...
        # عداد تفصيلي لكل رسالة (اختياري لعرض إحصاءات لاحقًا)
        ...
        # --- دالة مساعدة لتحويل مفاتيح القامبين إلى (campaign_id, chat_id)
        ...
        # 1) حدّث الرسالة الحالية التي ضغط عليها العضو
        try:
            current_mid = getattr(query.message, "message_id", None)
            if current_mid:
                kb_current = InlineKeyboardMarkup([[
                    InlineKeyboardButton(
                        f"{pos_emo} {like_cnt}",
                ...
                await context.bot.edit_message_reply_markup(
                    ...
                    reply_markup=kb_current,
                )
        except Exception:
            pass

        # 2) تزامن كل رسائل الحملة في كل المجموعات
        for key, base_mid in list(base_map.items()):
            ...
            try:
                kb = InlineKeyboardMarkup([[
                    InlineKeyboardButton(
                        f"{pos_emo} {like_cnt}",
                        callback_data=f"like:{cid}:{base_mid}",
                    ),
                    InlineKeyboardButton(
                        f"{neg_emo} {dislike_cnt}",
                        callback_data=f"dislike:{cid}:{base_mid}",
                    ),
                ]])
                await context.bot.edit_message_reply_markup(
                    chat_id=cid,
                ...
                # لو رسالة محذوفة/قديمة نتجاهلها
                pass
    ...
    # منع التكرار على مستوى الرسالة فقط
...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    """
    مزامنة أزرار التفاعل لجميع رسائل الحملة:
    - لا نستخدم الآن رسائل دعوة نهائيًا
    - نعتمد فقط على رسائل الأساس المخزّنة في campaign_base_msg
    """
    # إجمالي عدّادات الحملة
    ...
    # خريطة رسائل الأساس لكل حملة/مجموعه
    base_map = globals().get("campaign_base_msg") or {}
    ...
    # كيبورد واحد موحّد لجميع رسائل الحملة
    def _kb(cid: int, base_mid: int) -> InlineKeyboardMarkup:
    ...
    # مرّ على كل رسائل الأساس التي تنتمي لهذه الحملة وحدّث الأزرار
    for key, base_mid in list(base_map.items()):
        camp_k, cid = _parse_key_to_pair(key)
        if camp_k != campaign_id or not isinstance(base_mid, int):
            continue
        try:
            ...
            # ممكن تكون الرسالة محذوفة أو قديمة – نتجاوزها
            continue
...
async def send_post_one_chat(
    ...
    *,
    is_rebroadcast: bool,
    hide_links: bool,
    reaction_prompt: str  # يبقى في التوقيع لكن لن نستخدمه
) -> Optional[int]:
    logger = logging.getLogger("publisher")

    first_message_id: Optional[int] = None
    caption = sess.text
    # هل المنشور عبارة عن ألبوم (أكثر من وسيط واحد)؟
    is_album = bool(sess.media_list and len(sess.media_list) > 1)
    logger.info(
        f"[DEBUG] send_post_one_chat chat={chat_id} "
        f"is_album={is_album} media_len={len(sess.media_list or [])} "
        f"text_len={len(caption or '')}"
    )
    ...
    if sess.media_list:
        if len(sess.media_list) > 1:
            ...
            # وسيط واحد (صورة/فيديو) في media_list
            t, fid, cap = sess.media_list[0]
            c = caption or cap
            c = hidden_links_or_plain(c, hide_links)
            if t == "photo":
    ...
    if caption:
        m = await context.bot.send_message(
            ...
            parse_mode=("HTML" if hide_links else None),
            disable_web_page_preview=True,  # لا نريد معاينة الروابط
        )
        first_message_id = first_message_id or m.message_id

    if not first_message_id:
        logger.error(f"[ERROR] send_post_one_chat no first_message_id chat={chat_id}")
        return None

    # --- إعداد الأساس لأزرار التفاعل ---
    base_id_for_buttons = first_message_id

    # في حالة الألبوم: نتعامل مع التفاعلات كرسالة مفردة (بدون حملة موحدة بين المجموعات)
    effective_campaign_id = sess.campaign_id if (sess.campaign_id is not None and not is_album) else None

    if effective_campaign_id is not None:
        key = (effective_campaign_id, chat_id)
        if key not in campaign_base_msg:
            campaign_base_msg[key] = first_message_id
            try:
                save_state()
            except Exception as e:
                logger.error(f"[ERROR] save_state campaign_base_msg failed: {e}")
        base_id_for_buttons = campaign_base_msg[key]

    # --- التفاعلات ---
    # احترم التعطيل المركزي + تفعيل الجلسة
    if not global_settings.get("reactions_feature_enabled", True):
        logger.info("[DEBUG] reactions disabled globally")
        return first_message_id
    if not getattr(sess, "use_reactions", True):
        logger.info("[DEBUG] reactions disabled for this session")
        return first_message_id
    ...
    try:
        ...
        if effective_campaign_id is not None:
            if style is None:
                style = campaign_styles.get(effective_campaign_id, "thumbs")
            campaign_styles[effective_campaign_id] = style
        else:
            if style is None:
        ...
        logger.error(f"[ERROR] save_state reactions/style failed: {e}")
        style = "thumbs"
    ...
    # ربط رسالة الأساس بالحملة بالتفاعلات – فقط إذا استخدمنا campaign
    if effective_campaign_id is not None:
        try:
            message_to_campaign[(chat_id, base_id_for_buttons)] = effective_campaign_id
            save_state()
        except Exception as e:
            logger.error(f"[ERROR] save_state message_to_campaign failed: {e}")

    # 🔹 حالة الألبوم: لا نحاول تعديل كيبورد رسالة الألبوم
    # بدلًا من ذلك نرسل رسالة جديدة أسفل الألبوم تحتوي فقط على الأزرار
    if is_album:
        logger.info(
            f"[DEBUG] album reactions → sending separate buttons message "
            f"chat={chat_id} base_id={base_id_for_buttons}"
        )
        try:
            await context.bot.send_message(
                chat_id=chat_id,
                text="\u200b",  # حرف مخفي لكن يعتبر نص غير فارغ
                reply_markup=kb,
            )
        except Exception as e:
            logger.error(f"[ERROR] album reactions send_message failed: {e}")
        return first_message_id

    # 🔹 بقية الحالات (نص / وسيط مفرد): نحاول تركيب الأزرار تحت نفس الرسالة
    try:
        await context.bot.edit_message_reply_markup(
            chat_id=chat_id,
            message_id=base_id_for_buttons,
            reply_markup=kb,
        )
    except Exception as e:
        logger.error(
            f"[ERROR] edit_message_reply_markup failed: {e} "
            f"chat={chat_id} msg={base_id_for_buttons}"
        )
        # في بعض الحالات النادرة ن fallback لرسالة جديدة
        try:
            await context.bot.send_message(
                chat_id=chat_id,
                text="\u200b",  # نفس الفكرة هنا اذا احتجنا رسالة منفصلة
                reply_markup=kb,
            )
        except Exception as e2:
            logger.error(
                f"[ERROR] fallback reactions send_message failed: {e2} "
                f"chat={chat_id} msg={base_id_for_buttons}"
            )
...
async def publish_to_chats(
    ...
    *,
    ...
    target_chats = [
        cid for cid in list(getattr(sess, "chosen_chats", []))
    ...
    tasks = [
        _safe_send_one(
            ...
            is_rebroadcast=is_rebroadcast,
            hide_links=hide_links,
            # لم نعد نستخدم نص الدعوة، نمرّر نصًا فارغًا
            reaction_prompt="",
            sem=sem,
        )
        for cid in target_chats
    ...
    for cid, mid in zip(target_chats, results):
        if mid:
            if (
                not is_rebroadcast
                and global_settings.get("pin_feature_enabled", True)
            ...
            errors.append(
                f"تعذّر الإرسال إلى {known_chats.get(cid, {}).get('title', cid)}"
            )
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel:reactions":
        ...
        text = f"👍 حالة التفاعلات: *{'مفعّلة' if on else 'معطّلة'}*"
