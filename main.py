...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel:stats":
    # نجمع المعرّفات من المصدرين: campaign_messages و campaign_base_msg
        def _camp_id_from_base(key):
        # يدعم المفاتيح كـ (camp_id, chat_id) أو كسلاسل "camp|chat" / "camp::chat"
        ...
        ids_from_msgs  = set(campaign_messages.keys())
        ids_from_bases = {cid for cid in (_camp_id_from_base(k) for k in campaign_base_msg.keys()) if cid is not None}
        ids = sorted(ids_from_msgs | ids_from_bases)
