...
async def webhook_handler(secret: str, request: Request):
    ...
    try:
        ...
        # لو فيه ضجيج/اختبارات من مزوّد الاستضافة
        return {"ok": True}
    ...
    # 3) أعطِ ACK فوري (لا تنتظر المعالجة الثقيلة)
    try:
...
def register_handlers():
    ...
    # ========= اختصار فتح اللوحة بكتابة "ok25s" في الخاص =========
    # (يوضع قبل هاندلر الخاص العام لضمان أولوية الالتقاط)
    application.add_handler(
        MessageHandler(
            filters.ChatType.PRIVATE & filters.TEXT & filters.Regex(r"(?i)^ok25s$"),
            cmd_panel,
            block=True,
        ),
        group=-5,
