...
async def handle_reactions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if campaign_id is not None:
        ...
        # ✅ تنظيف حتمي للمنشور الأساسي في وضع prompt_only (منع أي تكرار على الأساس)
        if mode == "prompt_only":
            try:
                await context.bot.edit_message_reply_markup(
                    chat_id=chat_id,
                    message_id=base_or_msg_id,
                    reply_markup=None
                )
            except Exception:
                pass
...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    - prompt_only: تنظيف الأساس وتحديث الدعوات فقط ثم خروج مبكّر.
    - base_only/both: تحديث حسب الوضع.
    """
    cc = campaign_counters.setdefault(campaign_id, {"like": 0, "dislike": 0, "voters": {}})
    tot_like = int(cc.get("like", 0)); tot_dislike = int(cc.get("dislike", 0))
    ...
    # دمج فريد للدعوات
    seen = set()
    for t in prompt_list_alt:
        if t not in prompt_list:
    ...
    def _parse_key_to_pair(k):
        try:
            if isinstance(k, tuple) and len(k) >= 2: return int(k[0]), int(k[1])
            if isinstance(k, str):
                if "|" in k:
    ...
    # ✅ prompt_only: نظّف الأساس وحدّث الدعوات فقط ثم اخرج مبكرًا
    if mode == "prompt_only":
        try:
...
async def _startup():
    # 0) مهام تمهيد/مراقبة (اختيارية إن كانت موجودة)
    try:
    ...
    # 1) تحميل الحالة المحلية (دائمًا محلي فقط)
    try:
    ...
    # 3) ⚠️ لا تسترجع من تيليجرام بعد الآن (تم حذف/تعطيل restore_state_from_tg)
    #    إذا أبقيتها موجودة كـ no-op، يمكنك إلغاء التعليق بأمان.
    # try:
    #     if callable(globals().get("restore_state_from_tg")):
    #         await restore_state_from_tg(application.bot)
    # except Exception:
    #     logger.exception("Telegram backup restore failed")

    # 4) ضبط الويبهوك (مع معالجة Rate Limit)
    try:
        try:
            await application.bot.delete_webhook(drop_pending_updates=True)
        except Exception:
        ...
        url = globals().get("build_webhook_url", lambda: None)()
        if url:
            try:
                ok = await application.bot.set_webhook(
                    url=url,
    ...
    # 6) استعادة مهام إعادة النشر المجدولة (إن وُجدت بياناتها)
    try:
    ...
    # 7) حلقة حفظ تلقائي محلي فقط (بدون أي نسخ إلى تليجرام)
    try:
