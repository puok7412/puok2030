...
# =============================
# خرائط مزامنة الحملة للتفاعلات (جديدة)
# =============================

# ربط (chat_id, base_message_id) → campaign_id
try:
    message_to_campaign
except NameError:
    message_to_campaign: Dict[Tuple[int, int], int] = {}

# حفظ رسائل زرّ التفاعل لكل حملة: campaign_id → [(chat_id, prompt_message_id)]
try:
    campaign_prompt_msgs
except NameError:
    campaign_prompt_msgs: Dict[int, List[Tuple[int, int]]] = {}

# عدادات موحّدة للحملة + مانع تكرار التصويت على مستوى الحملة
try:
    campaign_counters
except NameError:
...
global_settings = {
    ...
    "rebroadcast_interval_seconds": 7200,  # الافتراضي: كل 2 ساعة (7200s)
...
def build_session_schedule_keyboard(sess) -> InlineKeyboardMarkup:
    ...
    # القيمة الحالية للفاصل بالساعات (تقريب للأعلى)
    ...
    # عدد الإعادات
    rows.append([
        InlineKeyboardButton(f"🔁 عدد الإعادات (الحالي {getattr(sess, 'rebroadcast_total', 0)})", callback_data="noop")
    ])
    rows.append([InlineKeyboardButton(str(c), callback_data=f"ssched_count:{c}") for c in counts])

    # الفاصل الحالي بالساعات فقط
    rows.append([
        InlineKeyboardButton(f"⏲️ الفاصل الحالي: {current_hrs} ساعة", callback_data="noop")
    ])
...
async def handle_campaign_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # == عرض الإحصاءات من العدّاد الموحّد للحملة ==
    if data.startswith("show_stats:"):
        ...
        # المجاميع من campaign_counters
        cc = campaign_counters.get(campaign_id)
        if not cc:
            text = (
                f"📊 إحصاءات الحملة #{campaign_id}\n"
                f"——————————————\n"
                f"لا توجد تفاعلات حتى الآن."
            )
            try:
        ...
        tot_like = cc.get("like", 0)
        tot_dislike = cc.get("dislike", 0)
        voters_count = len(cc.get("voters", {}))

        # (اختياري) تفصيل كل وجهة من العدادات المحليّة الحالية
        per_chat_lines = []
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # لا تتعامل هنا مع أزرار لها هاندلرات خاصة
    if data.startswith(("like:", "dislike:", "panel:", "perm:", "show_stats:", "stop_rebroadcast:")):
        return

    # أجب فورًا لتجنب "تحميل" وأخطاء Query is too old/invalid
    try:
        await query.answer()
    ...
    if data == "back_main":
        # إبقِ لوحة المنشور فقط: احذف أي لوحة مؤقتة (اختيار/جدولة) إن كانت هي الرسالة الحالية
        try:
        ...
        # احذف لوحة اختيار الوجهات إن كانت مسجلة
        delete_picker_if_any(context, user_id, sess)
    ...
    if data == "publish":
        ...
        # احترام الحالة المركزية للجدولة
        ...
        # أرسل لوحة الحملة لصاحب الجلسة وربما لمن منح التصريح
...
def build_chats_keyboard(chat_ids: List[int], chosen: Set[int], settings: Dict[str, Any]) -> InlineKeyboardMarkup:
    ...
    for cid in chat_ids:
        ...
        badge = "📢" if typ == "channel" else "👥"   # قناة/مجموعة
        mark  = "✅" if cid in chosen else "🚫"      # مفعّل/غير مفعّل

        rows.append([
            InlineKeyboardButton(f"{badge} {mark} {title}", callback_data=f"toggle_chat:{cid}")
        ])

    selected_cnt = len(chosen)
    rows.append([
        InlineKeyboardButton("✅ تحديد الكل", callback_data="select_all"),
        InlineKeyboardButton(f"💾 حفظ الاختيار ({selected_cnt})", callback_data="done_chats"),
    ])

    # كان: "⬅️ رجوع" —> صار: "▶️ متابعة" (نفس callback)
    rows.append([InlineKeyboardButton("▶️ متابعة", callback_data="back_main")])
...
async def handle_reactions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # لا نجاوب هنا عشان ما نضيع محاولة التنبيه لاحقاً
        return

    # سجلّ هذا المنشور (لا يعتمد على آخر منشور فقط)
    ...
    # 🔎 تحديد الحملة من خلال (chat_id, base_message_id)
    campaign_id = message_to_campaign.get((chat_id, message_id))

    # ✅ منع التكرار على مستوى الحملة (إن وُجدت)
    ...
    # ✅ منع التكرار المحلي (نفس سلوكك القديم تمامًا)
    ...
    if campaign_id is not None:
        # حدّث عدادات الحملة
        if action == "like":
            campaign_counters[campaign_id]["like"] += 1
        else:
            campaign_counters[campaign_id]["dislike"] += 1
        campaign_counters[campaign_id]["voters"][user.id] = action

        # مزامنة كل أزرار الحملة فورًا
        try:
            await _sync_campaign_keyboards(context, campaign_id)
        except Exception:
            pass
    else:
        # لا توجد حملة: حدّث أزرار هذه الرسالة فقط (سلوكك القديم)
        kb = InlineKeyboardMarkup([[
    ...
    # أجب callback (نجاح) — نفس رسالتك
    try:
        await query.answer("تم تسجيل تفاعلك. شكرًا لك 🌟", show_alert=False)
    except BadRequest:
        pass

    # رسالة شكر مقتبسة لرفع المنشور للأعلى — نفس سلوكك
    try:
...
async def _sync_campaign_keyboards(context: ContextTypes.DEFAULT_TYPE, campaign_id: int):
    ...
    # حرّك عبر كل رسالة زرّ محفوظة للحملة
    for (cid, prompt_mid) in list(campaign_prompt_msgs.get(campaign_id, [])):
        try:
            ...
            # نتجاهل الأخطاء الفردية حتى لو كانت رسالة قد حُذفت
            pass
...
async def send_post_one_chat(
    ...
    *,
    ...
    if sess.single_attachment:
        ...
        if a_type == "document":
            m = await context.bot.send_document(
                chat_id=chat_id, document=file_id, caption=c,
                parse_mode=("HTML" if hide_links else None)
            )
        elif a_type == "audio":
            m = await context.bot.send_audio(
                chat_id=chat_id, audio=file_id, caption=c,
                parse_mode=("HTML" if hide_links else None)
            )
        elif a_type == "voice":
            m = await context.bot.send_voice(
                chat_id=chat_id, voice=file_id, caption=c,
                parse_mode=("HTML" if hide_links else None)
            )
        else:
            m = None
    ...
    if sess.media_list:
        if len(sess.media_list) > 1:
            ...
            for idx, (t, fid, cap) in enumerate(sess.media_list):
                ...
                if t == "photo":
                    media_group.append(
                        InputMediaPhoto(media=fid, caption=c, parse_mode=("HTML" if hide_links else None))
                    )
                else:
                    media_group.append(
                        InputMediaVideo(media=fid, caption=c, parse_mode=("HTML" if hide_links else None))
                    )
            ...
            if t == "photo":
                m = await context.bot.send_photo(
                    chat_id=chat_id, photo=fid, caption=c,
                    parse_mode=("HTML" if hide_links else None)
                )
            else:
                m = await context.bot.send_video(
                    chat_id=chat_id, video=fid, caption=c,
                    parse_mode=("HTML" if hide_links else None)
                )
    ...
    # === التفاعلات (يشمل منطق الخطوة 3) ===
    if sess.use_reactions:
        # عدّاد محلي (متوافق مع النظام القديم)
        ...
        try:
            prompt_msg = await context.bot.send_message(
                chat_id=chat_id, text=reaction_prompt, reply_markup=kb
            )

            # ربط هذا الـ base_id بالحملة + حفظ رسالة زرّ التفاعل للتحديث المتزامن لاحقًا
            if sess.campaign_id is not None:
                # يربط نسخة هذا المنشور (الأساس) بالحملة
                message_to_campaign[(chat_id, base_id_for_buttons)] = sess.campaign_id
                # يحفظ رسالة زرّ التفاعل لهذه الوجهة ضمن الحملة
...
# =============================
# Global singletons / state (مرة واحدة فقط)
# ضعه قبل تسجيل أي هاندلرز وقبل بدء التطبيق
# =============================
global_settings = {
...
# لكل مستخدم إعداداته الإدارية
admin_settings: Dict[int, Dict[str, Any]] = {}

# أذونات/قيود كل مجموعة
group_permissions: Dict[int, Dict[str, Any]] = {}

# كاش للمحادثات المعروفة + مشرفيها
known_chats: Dict[int, Dict[str, Any]] = {}
known_chats_admins: Dict[int, Dict[int, str]] = {}

# جلسات النشر النشطة في الخاص
sessions: Dict[int, "Session"] = {}

# منح مؤقتة للناشرين
temp_grants: Dict[int, Dict[str, Any]] = {}

# عدادات التفاعل لكل (chat_id, message_id)
reactions_counters: Dict[Tuple[int, int], Dict[str, Any]] = {}

# تتبّع رسائل كل حملة: campaign_id -> [(chat_id, base_message_id), ...]
campaign_messages: Dict[int, List[Tuple[int, int]]] = {}

# أوّل رسالة للمنشور في كل وجهة: (campaign_id, chat_id) -> base_message_id
campaign_base_msg: Dict[Tuple[int, int], int] = {}
...
# حالة انتظار نص دعوة التفاعل في لوحة التحكم
PANEL_WAIT_REACTION_PROMPT = "panel_wait_reaction_prompt"
...
def get_settings(user_id: int) -> Dict[str, Any]:
    ...
    # خيارات تشغيل افتراضية للمستخدم
    s.setdefault("permissions_mode", "all")   # أو "whitelist"
    s.setdefault("whitelist", set())
    s.setdefault("hide_links_default", False)
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # أجب فوراً لتجنب "تحميل" ولتلافي خطأ Query is too old...
    try:
        await query.answer()
    except BadRequest as e:
        msg = str(e).lower()
    ...
    if data == "panel:stats":
        ...
        # كان هنا await query.answer() — أبقيناه لكن بشكل آمن (ولو شلته نهائياً ما يضرّ لأننا جاوبنا بالأعلى)
        try:
            await query.answer()
        except BadRequest as e:
            msg = str(e).lower()
...
def build_status_block(sess, *, allow_schedule: bool | None = None) -> str:
    ...
    def _fmt_interval(sec):
        try:
            ...
            return None
        if sec < 60: return f"كل {sec} ثانية"
        mins = sec // 60
        if mins < 60: return f"كل {mins} دقيقة"
        hrs = (sec + 3599) // 3600
        return f"كل {hrs} ساعة"
    ...
    lines = []
    if likes_line: lines.append(likes_line)
    if pin_line:   lines.append(pin_line)
...
async def publish_to_chats(context, user_id, sess, *, is_rebroadcast: bool, hide_links: bool):
    ...
    if global_settings.get("maintenance_mode", False):
        try:
            await context.bot.send_message(chat_id=user_id, text="🛠️ نظام النشر تحت الصيانة والتحديث حاليًا. الرجاء المحاولة لاحقًا.")
        except:
            pass
        ...
        for cid in target_chats:
            try:
                ...
                mid = await send_post_one_chat(
                    ...
                    reaction_prompt=s.get("reaction_prompt_text", "")
                )
                if mid:
                    # تثبيت فقط إذا الخدمة مفعلة والجلسة مفعلة وليس أثناء الإعادة
                    if gs.get("pin_feature_enabled", True) and getattr(sess, "pin_enabled", True) and not is_rebroadcast:
                        try:
                        ...
                        if gs.get("scheduling_enabled", True) and getattr(sess, "schedule_active", False):
                            total_secs = max(1, getattr(sess, "rebroadcast_total", 1)) * max(1, getattr(sess, "rebroadcast_interval_seconds", 60))
                        else:
                            total_secs = 6 * 3600
                        if "_schedule_unpin" in globals():
                ...
                try:
                    logging.getLogger("publisher").exception("Failed to send to %s", cid)
                except:
                    pass
                errors.append(f"❌ فشل الإرسال إلى {known_chats.get(cid, {}).get('title', cid)}: {e}")

    except Exception as e:
        # حماية عُلوية — دائمًا نرجع زوج قيم
        try:
            logging.getLogger("publisher").exception("publish_to_chats crashed")
        except:
            pass
        errors.append(f"⚠️ خطأ داخلي أثناء النشر: {e}")
...
async def answer_unknown_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
...
async def on_error(update: object, context: ContextTypes.DEFAULT_TYPE):
    err = context.error
    # كتم أخطاء ضغط الأزرار القديمة/الملغية
    if isinstance(err, BadRequest):
        msg = str(err).lower()
        if "query is too old" in msg or "invalid" in msg:
            return
    # أي أخطاء أخرى: سجّلها
    logger.exception("Unhandled exception", exc_info=err)

# سجّل الخطّاف مرّة واحدة فقط
application.add_error_handler(on_error)

# =========================
# تسجيل الهاندِرات (مرة واحدة وفي ذيل الملف)
# =========================
...
application.add_handler(CommandHandler("panel", cmd_open_panel))  # يفتح لوحة التحكم في الخاص

# مفاتيح سريعة في الخاص
application.add_handler(MessageHandler(
    filters.ChatType.PRIVATE & filters.TEXT & ~filters.COMMAND & filters.Regex(r"(?i)^\s*ok\s*$"),
    start_publishing_keyword
), group=0)
application.add_handler(MessageHandler(
    filters.ChatType.PRIVATE & filters.TEXT & ~filters.COMMAND & filters.Regex(r"(?i)^ok25s$"),
    open_panel
), group=0)

# نصوص لوحة التحكم في الخاص
application.add_handler(MessageHandler(
    filters.ChatType.PRIVATE & filters.TEXT & ~filters.COMMAND,
    handle_panel_text
), group=1)

# إدخال المحتوى في الخاص (نص/وسائط/ملفات) — مع استثناء ok/ok25s
application.add_handler(MessageHandler(
    filters.ChatType.PRIVATE
    & ~filters.COMMAND
    & ~filters.Regex(r"(?i)^\s*(ok|ok25s)\s*$"),
    handle_admin_input
), group=2)

# ---- Callback buttons (ORDER MATTERS)
# 1) تفاعلات 👍👎
application.add_handler(
    CallbackQueryHandler(handle_reactions, pattern=r"^(like|dislike):"),
    group=10
)

# 2) أزرار الحملة (إحصاءات/إيقاف)
application.add_handler(
    CallbackQueryHandler(handle_campaign_buttons, pattern=r"^(show_stats:|stop_rebroadcast:)"),
    group=11
)
...
# 4) أزرار جلسة النشر كلها ما عدا ما سبق
application.add_handler(
    CallbackQueryHandler(on_button, pattern=r"^(?!like:|dislike:|panel:|perm:|show_stats:|stop_rebroadcast:).+"),
    group=99
)

# 5) فول-باك لأي شيء سقط من الفلاتر (يُنهي "تحميل")
application.add_handler(
    CallbackQueryHandler(answer_unknown_callback),
    group=100
)

# تتبّع انضمام/ترقية البوت في المحادثات
application.add_handler(ChatMemberHandler(on_my_chat_member, ChatMemberHandler.MY_CHAT_MEMBER), group=20)

# تتبّع رسائل الجروبات/القنوات (لتخزين بيانات الشات)
