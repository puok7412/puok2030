...
def def build_persist_snapshot() -> Dict[str, Any]:
    ...
    # تطبيع المفاتيح إلى نصوص أينما كانت بدائية لتوافق JSON
    ...
    campaign_styles_norm     = {str(k): v for k, v in _campaign_styles.items()}  # ✅
    campaign_prompt_msgs_norm= {str(k): _pairs_list(v) for k, v in _campaign_prompt_msgs.items()}  # ✅
    ...
    return {
        ...
        "campaign_styles": campaign_styles_norm,  # ✅
        "reaction_style_by_message": _pack_tuple_keys(_reaction_style_by_msg, name="reaction_style_by_message"),
        "campaign_prompt_msgs": campaign_prompt_msgs_norm,  # ✅ مهم للمزامنة عبر المجموعات
        "global_settings": {
...
def apply_persist_snapshot(data: Dict[str, Any]) -> None:
    ...
    # ✅ استعادة قوائم رسائل الدعوات لكل حملة (ضروري للمزامنة عبر كل المجموعات)
    cpm_raw = data.get("campaign_prompt_msgs", {})
    restored = {}
    if isinstance(cpm_raw, dict):
        for camp_id, pairs in cpm_raw.items():
            try:
                key = str(camp_id).strip()
                restored[key] = _pairs_to_tuples(pairs)
            except Exception:
                continue
    globals()["campaign_prompt_msgs"] = restored
